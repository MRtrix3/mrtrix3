...
public:
  ...
  template <class VecType, class MatType> MatType &V2M(const VecType &, MatType &) const;

  // Convenience functions to avoid having to pre-define the output class
  template <class MatType> vector_type M2V(const MatType &) const;
  template <class VecType> matrix_type V2M(const VecType &) const;

private:
  const node_t dim;
};

template <class MatType, class VecType> VecType &Mat2Vec::M2V(const MatType &m, VecType &v) const {
...
template <class MatType> vector_type Mat2Vec::M2V(const MatType &m) const {
  vector_type v;
  M2V(m, v);
  return v;
}

template <class VecType> matrix_type Mat2Vec::V2M(const VecType &v) const {
  matrix_type m;
  V2M(v, m);
  return m;
}
