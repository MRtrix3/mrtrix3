...
namespace MR {
  namespace DWI {
    ...
    namespace {
      const size_t WM_response_size[] = { 6, 8 };
      const float WM_response_coefs[] = { 
    ...
    {
      return Math::Matrix<float> (const_cast<float*> (WM_response_coefs), WM_response_size[0], WM_response_size[1]);
    }

    Math::Matrix<float> default_CSF_response ()
    {
      return Math::Matrix<float> ();
    }
