...
const OptionGroup SIFTModelOption =
    OptionGroup("Options affecting the SIFT model")

    + Option("fd_scale_gm",
             "provide this option (in conjunction with -act) to heuristically downsize the fibre density estimates "
    ...
    + Option("no_dilate_lut",
             "do NOT dilate FOD lobe lookup tables; only map streamlines to FOD lobes if the precise tangent lies "
             "within the angular spread of that lobe")

    + Option("make_null_lobes",
             "add an additional FOD lobe to each voxel, with zero integral, that covers all directions with zero / "
             "negative FOD amplitudes")
    ...
    Option(
        "fd_thresh",
        "fibre density threshold; exclude an FOD lobe from filtering processing if its integral is less than this "
        "amount "
...
const OptionGroup SIFTOutputOption =
    OptionGroup("Options to make SIFT provide additional output files")

    + Option("csv", "output statistics of execution per iteration to a .csv file") + Argument("file").type_file_out()

    + Option("out_mu", "output the final value of SIFT proportionality coefficient mu to a text file") +
    Argument("file").type_file_out()

    + Option("output_debug", "write to a directory various output images for assessing & debugging performance etc.") +
