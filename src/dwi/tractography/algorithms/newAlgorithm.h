...
#ifndef __dwi_tractography_algorithms_new_algorithm_h__
#define __dwi_tractography_algorithms_new_algorithm_h__
...
class NewAlgorithm : public MethodBase { MEMALIGN(NewAlgorithm)
  public:
      ...
      public:
            ...
            lmax (Math::SH::LforN (source.size(3)))
        {
          try {
            ...
            e.display();
            throw Exception ("New Algorithm expects as input a spherical harmonic (SH) image");
          }
          ...
          properties["method"] = "newAlgorithm";
    ...
    NewAlgorithm (const Shared& shared) :
      MethodBase (shared),
      S (shared),
      source (S.source) { }

    NewAlgorithm (const NewAlgorithm& that) :
      MethodBase (that.S),
      S (that.S),
      source (S.source) { }
