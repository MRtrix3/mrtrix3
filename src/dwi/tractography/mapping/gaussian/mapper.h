      ...
      namespace Mapping {
        namespace Gaussian {
          ...
          { MEMALIGN(TrackMapper)
            ...
            inline void add_to_set (SetVoxel&   , const Eigen::Vector3i&, const tangent_type&, const default_type, const default_type) const;
            inline void add_to_set (SetVoxelDEC&, const Eigen::Vector3i&, const tangent_type&, const default_type, const default_type) const;
            inline void add_to_set (SetDixel&   , const Eigen::Vector3i&, const tangent_type&, const default_type, const default_type) const;
            inline void add_to_set (SetVoxelTOD&, const Eigen::Vector3i&, const tangent_type&, const default_type, const default_type) const;
          ...
          template <class Cont>
            void TrackMapper::voxelise (const Streamline<>& tck, Cont& output) const
            {
              ...
              for (size_t i = 0; i != last; ++i) {
                ...
                if (check (vox, info)) {
                  const tangent_type dir ((tck[i+1] - tck[prev]).normalized());
              ...
              if (check (vox, info)) {
                const tangent_type dir ((tck[last] - tck[prev]).normalized());
            ...
            void TrackMapper::voxelise_ends (const Streamline<>& tck, Cont& out) const
            {
              for (size_t end = 0; end != 2; ++end) {
                ...
                if (check (vox, info))
                  add_to_set (out,
                              vox,
                              Tractography::tangent (tck, end ? (tck.size()-1) : 0),
                              1.0,
                              end ? factors.back() : factors.front());
              }
            }



          inline void TrackMapper::add_to_set (SetVoxel&    out, const Eigen::Vector3i& v, const tangent_type& d, const default_type l, const default_type f) const
          {
            out.insert (v, l, f);
          }
          inline void TrackMapper::add_to_set (SetVoxelDEC& out, const Eigen::Vector3i& v, const tangent_type& d, const default_type l, const default_type f) const
          {
            out.insert (v, d, l, f);
          }
          inline void TrackMapper::add_to_set (SetDixel&    out, const Eigen::Vector3i& v, const tangent_type& d, const default_type l, const default_type f) const
          {
            assert (dixel_plugin);
            const size_t bin = (*dixel_plugin) (d);
            out.insert (v, bin, l, f);
          }
          inline void TrackMapper::add_to_set (SetVoxelTOD& out, const Eigen::Vector3i& v, const tangent_type& d, const default_type l, const default_type f) const
          {
            assert (tod_plugin);
