...
void BufferScratchDump<value_type>::dump_to_file(const std::string &path, const Header &H) const {
  ...
  if (H.transform().is_set()) {
    out_header << "\ntransform: " << H.transform()(0, 0) << "," << H.transform()(0, 1) << "," << H.transform()(0, 2)
               << "," << H.transform()(0, 3);
    out_header << "\ntransform: " << H.transform()(1, 0) << "," << H.transform()(1, 1) << "," << H.transform()(1, 2)
               << "," << H.transform()(1, 3);
    out_header << "\ntransform: " << H.transform()(2, 0) << "," << H.transform()(2, 1) << "," << H.transform()(2, 2)
               << "," << H.transform()(2, 3);
  }
  ...
  if (H.DW_scheme().is_set()) {
    for (size_t i = 0; i < H.DW_scheme().rows(); i++)
      out_header << "\ndw_scheme: " << H.DW_scheme()(i, 0) << "," << H.DW_scheme()(i, 1) << "," << H.DW_scheme()(i, 2)
                 << "," << H.DW_scheme()(i, 3);
  }
