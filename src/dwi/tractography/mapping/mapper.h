  ...
  namespace DWI {
    namespace Tractography {
      namespace Mapping {
        ...
        template <class Cont>
          void TrackMapperBase::voxelise (const Streamline<>& tck, Cont& output) const
          {
            ...
            for (auto i = tck.cbegin(); i != last; ++i) {
              ...
              if (check (vox, info)) {
                const tangent_type dir = (*(i+1) - *prev).normalized();
            ...
            if (check (vox, info)) {
              const tangent_type dir = (*last - *prev).normalized();
              if (dir.allFinite() && !dir.isZero())
                add_to_set (output, vox, dir, 1.0);
            }
        ...
        template <class Cont>
          void TrackMapperBase::voxelise_precise (const Streamline<>& tck, Cont& out) const
          {
            ...
            do {
              ...
              const point_type traversal_vector = (p_voxel_exit - p_voxel_entry).normalized();
        ...
        template <class Cont>
          void TrackMapperBase::voxelise_ends (const Streamline<>& tck, Cont& out) const
          {
            ...
            if (tck.size() == 1) {
              ...
              if (check (vox, info))
                add_to_set (out, vox, Eigen::Vector3f(NaN, NaN, NaN), 1.0);
              return;
            }
            for (size_t end = 0; end != 2; ++end) {
              const auto vox = round (scanner2voxel * (end ? tck.back() : tck.front()));
              if (check (vox, info))
                add_to_set (out, vox, Tractography::tangent (tck, end ? (tck.size()-1) : 0), 1.0);
            }
          }
        ...
        inline void TrackMapperBase::add_to_set (SetVoxel&    out, const Eigen::Vector3i& v, const tangent_type& d, const default_type l) const
        {
          out.insert (v, l);
        }
        inline void TrackMapperBase::add_to_set (SetVoxelDEC& out, const Eigen::Vector3i& v, const tangent_type& d, const default_type l) const
        {
          out.insert (v, d, l);
        }
        inline void TrackMapperBase::add_to_set (SetVoxelDir& out, const Eigen::Vector3i& v, const tangent_type& d, const default_type l) const
        {
          out.insert (v, d, l);
        }
        inline void TrackMapperBase::add_to_set (SetDixel&    out, const Eigen::Vector3i& v, const tangent_type& d, const default_type l) const
        {
          assert (dixel_plugin);
          const DWI::Directions::index_type bin = (*dixel_plugin) (d);
          out.insert (v, bin, l);
        }
        inline void TrackMapperBase::add_to_set (SetVoxelTOD& out, const Eigen::Vector3i& v, const tangent_type& d, const default_type l) const
        {
          assert (tod_plugin);
