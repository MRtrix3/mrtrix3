...
public:
  ...
  Matrix(const node_t max_node_index, const stat_edge stat, const bool vector_output, const bool track_assignments)
      : statistic(stat),
        ...
        mat2vec(vector_output ? nullptr : new MR::Connectome::Mat2Vec(max_node_index + 1)),
        data(vector_type::Zero(vector_output ? (max_node_index + 1) : mat2vec->vec_size())),
        counts(stat == stat_edge::MEAN ? vector_type::Zero(vector_output ? (max_node_index + 1) : mat2vec->vec_size())
                                       : vector_type()) {
    if (statistic == stat_edge::MIN)
      data = vector_type::Constant(vector_output ? (max_node_index + 1) : mat2vec->vec_size(),
