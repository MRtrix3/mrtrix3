...
public:
  ...
  Point_t getPosition() const { return pos; }

  void setPosition(const Point_t &p) { pos = p; }

  Point_t getDirection() const { return dir; }
  ...
  Point_t getEndPoint(const int a) const { return (pos + a * L * dir); }

  bool hasPredecessor() const { return (predecessor != nullptr); }

  Particle *getPredecessor() const { return predecessor; }
  ...
  Particle *getSuccessor() const { return successor; }
