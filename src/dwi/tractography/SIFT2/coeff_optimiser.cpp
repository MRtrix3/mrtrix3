...
CoefficientOptimiserBase::~CoefficientOptimiserBase() {
  ...
  fprintf(
      stderr,
      "%ld of %ld initial searches failed, %ld in wrong direction, %ld steps truncated, %ld coefficients truncated\n",
      failed,
      total,
      wrong_dir,
      step_truncated,
...
CoefficientOptimiserQLS::CoefficientOptimiserQLS(TckFactor &tckfactor,
                                                 StreamlineStats &step_stats,
                                                 StreamlineStats &coefficient_stats,
                                                 unsigned int &nonzero_streamlines,
                                                 BitSet &fixels_to_exclude,
...
CoefficientOptimiserQLS::CoefficientOptimiserQLS(const CoefficientOptimiserQLS &that)
    : CoefficientOptimiserBase(that), qls(-master.max_coeff_step, master.max_coeff_step) {
  qls.set_exit_if_outside_bounds(false);
  qls.set_value_tolerance(0.001);
}
...
CoefficientOptimiserIterative::CoefficientOptimiserIterative(TckFactor &tckfactor,
                                                             StreamlineStats &step_stats,
                                                             StreamlineStats &coefficient_stats,
                                                             unsigned int &nonzero_streamlines,
                                                             BitSet &fixels_to_exclude,
