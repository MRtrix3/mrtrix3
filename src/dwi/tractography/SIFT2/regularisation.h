...
template <typename value_type> inline value_type dtvreg_dcoeff(const value_type coeff, const value_type base) {
  return ((coeff <= base) ? (value_type(2.0) * (coeff - base))
                          : (value_type(2.0) * std::exp(coeff) * (std::exp(coeff) - std::exp(base))));
}
template <typename value_type> inline value_type d2tvreg_dcoeff2(const value_type coeff, const value_type base) {
  return ((coeff <= base)
              ? (value_type(2.0))
              : (value_type(2.0) * std::exp(coeff) * ((value_type(2.0) * std::exp(coeff)) - std::exp(base))));
}
template <typename value_type> inline value_type d3tvreg_dcoeff3(const value_type coeff, const value_type base) {
  return ((coeff <= base)
              ? (value_type(0.0))
...
inline void dxtvreg_dcoeffx(LineSearchFunctor::Result &result,
                            const double coeff,
