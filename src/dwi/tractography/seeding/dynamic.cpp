  ...
  namespace DWI
  {
    namespace Tractography
    {
      ...
      Dynamic::~Dynamic()
      {
        ...
        H[Image::Sparse::Legacy::name_key] = str(typeid(Image::Sparse::Legacy::FixelMetric).name());
        H[Image::Sparse::Legacy::size_key] = str(sizeof(Image::Sparse::Legacy::FixelMetric));
        Image::BufferSparse<Image::Sparse::Legacy::FixelMetric> buffer_probs ("final_seed_probs.msf", H), buffer_logprobs ("final_seed_logprobs.msf", H), buffer_ratios ("final_fixel_ratios.msf", H);
        ...
        for (loop.start (v, out_probs, out_logprobs, out_ratios); loop.ok(); loop.next (v, out_probs, out_logprobs, out_ratios)) {
          if (v.value()) {
            ...
            for (Fixel_map<Fixel_TD_seed>::ConstIterator i = begin (v); i; ++i, ++index) {
              Image::Sparse::Legacy::FixelMetric fixel (i().get_dir(), i().get_FOD(), i().get_old_prob());
      ...
      void Dynamic::output_fixel_images()
      {
        ...
        H[Image::Sparse::Legacy::name_key] = str(typeid(Image::Sparse::Legacy::FixelMetric).name());
        H[Image::Sparse::Legacy::size_key] = str(sizeof(Image::Sparse::Legacy::FixelMetric));
        Image::BufferSparse<Image::Sparse::Legacy::FixelMetric> buffer_probs ("mid_seed_probs.msf", H), buffer_logprobs ("mid_seed_logprobs.msf", H), buffer_ratios ("mid_fixel_ratios.msf", H), buffer_FDs ("mid_FDs.msf", H), buffer_TDs ("mid_TDs.msf", H);
      ...
      void Dynamic::perform_fixel_masking()
      {
        ...
        H[Image::Sparse::Legacy::name_key] = str(typeid(Image::Sparse::Legacy::FixelMetric).name());
        H[Image::Sparse::Legacy::size_key] = str(sizeof(Image::Sparse::Legacy::FixelMetric));
        Image::BufferSparse<Image::Sparse::Legacy::FixelMetric> buffer ("fixel_mask.msf", H);
        ...
        for (loop.start (v, out); loop.ok(); loop.next (v, out)) {
          if (v.value()) {
            ...
            for (Fixel_map<Fixel_TD_seed>::ConstIterator f = begin(v); f; ++f, ++index) {
              Image::Sparse::Legacy::FixelMetric fixel (f().get_dir(), f().get_FOD(), (f().can_update() ? 1.0f : 0.0f));
