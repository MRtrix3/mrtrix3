...
const OptionGroup SeedMechanismOption =
    ...
    + Option("seed_random_per_voxel",
          ...
          .allow_multiple() +
    Argument("image").type_image_in() + Argument("num_per_voxel").type_integer(1)
    ...
    +
    Option("seed_rejection", "seed from an image using rejection sampling (higher values = more probable to seed from)")
        .allow_multiple() +
    Argument("image").type_image_in()
    ...
    + Option("seed_dynamic",
             "determine seed points dynamically using the SIFT model (must not provide any other seeding mechanism). "
...
const OptionGroup SeedParameterOption =
    ...
    + Option("seed_direction",
             "specify a seeding direction for the tracking (this should be "
