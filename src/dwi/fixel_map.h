...
public:
  Fixel_map(const Header &H) : _header(H), _accessor(Image<MapVoxel *>::scratch(_header, "fixel map voxels")) {
...
private:
  ...
  public:
    HeaderHelper(const ::MR::Header &H) : ::MR::Header(H) { ndim() = 3; }
...
public:
  Iterator(const MapVoxel *const voxel, Fixel_map<Fixel> &parent)
      : index(voxel ? voxel->first_index() : 0), last(voxel ? (index + voxel->num_fixels()) : 0), fixel_map(parent) {}
...
template <class Fixel> class Fixel_map<Fixel>::ConstIterator {
public:
...
template <class Fixel> bool Fixel_map<Fixel>::operator()(const FMLS::FOD_lobes &in) {
  if (in.empty())
    return true;
  auto v = accessor();
