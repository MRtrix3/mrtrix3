...
namespace MR {
  namespace DWI {
    ...
    RenderFrame::RenderFrame (QWidget* parent) :
      ...
      view_angle (40.0), distance (0.3), line_width (1.0), scale (1.0), lod (3), lmax (12), 
    ...
    { 
      ...
      modelview[0] = M[0]; modelview[1] = M[1]; modelview[2] = M[2]; modelview[3] = 0.0;
      modelview[4] = M[3]; modelview[5] = M[4]; modelview[6] = M[5]; modelview[7] = 0.0;
      modelview[8] = M[6]; modelview[9] = M[7]; modelview[10] = M[8]; modelview[11] = 0.0;
      modelview[12] = modelview[13] = modelview[14] = 0.0; modelview[15] = 1.0;
      ...
      if (values.size()) {
        ...
        if (values_changed) renderer.calculate (values, lmax);
        ...
        renderer.draw (use_lighting, hide_neg_lobes, color_by_dir ? NULL : lighting->object_color);
...
*/
    ...
    void RenderFrame::mousePressEvent (QMouseEvent *event) { last_pos = event->pos(); }
