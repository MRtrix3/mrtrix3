...
namespace MR
{
  namespace Surface
  {
    ...
    void Mesh::load_vtk (const std::string& path)
    {
      ...
      // Second line: header description
      MR::getline (in, line);
    ...
    void Mesh::save_vtk (const std::string& path, const bool binary) const
    {
      ...
      description << "Generated by MRtrix";
      size_t description_size = description.tellp();
      for (size_t iteration = 0; iteration != 2; ++iteration) {
        switch (iteration) {
          case 0: description << " " << App::mrtrix_version; break;
          case 1: description << ", built " << App::build_date; break;
        }
        if (description.tellp() > 256)
          break;
        description_size = description.tellp();
      }
      out << description.str().substr(0, description_size) << "\n";
      if (binary)
        out << "BINARY\n";
      else
        out << "ASCII\n";
