...
void Transform::onActivate(bool onoff) { setActive(onoff); }

void Transform::showEvent(QShowEvent *) { setActive(false); }

void Transform::closeEvent(QCloseEvent *) { setActive(false); }

void Transform::hideEvent(QHideEvent *) { setActive(false); }
...
bool Transform::panthrough_event(const ModelViewProjection &projection) {
  auto move =
      window().get_current_mode()->get_through_plane_translation_FOV(window().mouse_displacement().y(), projection);
