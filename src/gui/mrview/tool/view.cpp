...
public:
  ...
  void reset(QModelIndex &index, const Image &image, int proj) { reset(planes[index.row()], image, proj); }
...
View::View(Dock *parent) : Base(parent) {
  ...
  connect(clip_planes_list_view,
          SIGNAL(customContextMenuRequested(const QPoint &)),
          this,
          SLOT(clip_planes_right_click_menu_slot(const QPoint &)));
  ...
  clip_intersectionmode_check_box->setToolTip(
      "Generated volume is the intersection of individual clipped volumes, rather than the union.");
  clip_intersectionmode_check_box->setChecked(false);
  connect(
      clip_intersectionmode_check_box, SIGNAL(toggled(bool)), this, SLOT(clip_planes_toggle_intersectionmode_slot()));
...
void View::closeEvent(QCloseEvent *) { window().disconnect(this); }
...
void View::onPlaneChanged() { plane_combobox->setCurrentIndex(window().plane()); }
...
bool View::get_cliphighlightstate() const { return clip_highlight_check_box->isChecked(); }
...
void View::clip_planes_toggle_shown_slot() { window().updateGL(); }

void View::clip_planes_toggle_highlight_slot() { window().updateGL(); }

void View::clip_planes_toggle_intersectionmode_slot() { window().updateGL(); }

// Light box related functions

void View::light_box_slice_inc_reset_slot() { reset_light_box_gui_controls(); }

void View::light_box_toggle_volumes_slot(bool) { reset_light_box_gui_controls(); }
