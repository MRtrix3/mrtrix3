...
Edge::Edge(const node_t one, const node_t two, const Eigen::Vector3f &c_one, const Eigen::Vector3f &c_two)
  ...
  if (is_diagonal()) {
    ...
    rot_matrix[0] = matrix(0, 0);
    rot_matrix[1] = matrix(0, 1);
    rot_matrix[2] = matrix(0, 2);
    rot_matrix[3] = matrix(1, 0);
    rot_matrix[4] = matrix(1, 1);
    rot_matrix[5] = matrix(1, 2);
    rot_matrix[6] = matrix(2, 0);
    rot_matrix[7] = matrix(2, 1);
    rot_matrix[8] = matrix(2, 2);
  }
}
...
void Edge::Streamtube::render() const {
  ...
  normal_buffer.bind(gl::ARRAY_BUFFER);
  vertex_array_object.bind();
  gl::MultiDrawElements(gl::TRIANGLE_STRIP,
                        shared.element_counts,
                        gl::UNSIGNED_INT,
                        (const GLvoid *const *)shared.element_indices,
                        count - 1);
  GL::assert_context_is_current();
}
