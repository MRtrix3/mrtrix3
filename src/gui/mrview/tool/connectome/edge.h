    ...
    namespace MRView
    {
      ...
      class Edge
      {
        ...
        public:
          ...
          void create_exemplar (const std::string& path) { assert (!exemplar); exemplar.reset (new Exemplar (path, *this)); }
          void buffer_exemplar() { assert (exemplar); exemplar->assign_buffers(); }
          void clear_exemplar() { delete exemplar.release(); }
          void render_exemplar() const { assert (exemplar); glColor3f (colour[0], colour[1], colour[2]); exemplar->render(); }
        ...
        private:
          ...
          // Class to store data relating to storing and displaying streamtubes
          // For now, just store the mean streamline path; down the track, may try to
          //   also capture the spatial dispersion of the pathway cross-section
          // Try to have a generic class that will allow display of both
          //   exemplar streamlines and streamtubes
          class Exemplar
          {
            public:
              Exemplar (const std::string&, const Edge&);
              Exemplar () = delete;
              void assign_buffers();
              void render();
            private:
              // These will be generated by the worker thread on construction
              std::unique_ptr< std::vector< Point<float> > > vertices;
              std::unique_ptr< std::vector< Point<float> > > tangents;
              // The master thread must then assign the VBOs
              GLuint count;
