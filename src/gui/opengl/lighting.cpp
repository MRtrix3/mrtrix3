  ...
  namespace GUI
  {
    namespace GL
    {
      ...
      void Lighting::load_defaults ()
      {
        ...
        File::Config::get_RGB ("LightPosition", lightpos, 0.0, 0.0, 1.0);

        Math::normalise (lightpos, 3);

        ambient = File::Config::get_float ("AmbientIntensity", 0.4);
        diffuse = File::Config::get_float ("DiffuseIntensity", 0.1);
        specular = File::Config::get_float ("SpecularIntensity", 0.6);
