...
public:
  ...
  template <class C> void load(C &data) const {
  ...
  bool operator<(const Min_mem_array<T> &that) const {
    ...
    // Therefore, only return true for '<' if the length of that is longer than this (if they are equivalent in length,
    // '<' does not hold so return false)
    return (n < that.n);
  }
