      ...
      void Writer<MatrixType>::save (const std::string& path) const
      {
        ...
        for (size_t stream_index = 0; stream_index != 2; ++stream_index) {
          ...
          if (stream_index)
            ...
            stream << DataType::from<fixel_index_type>().specifier();
        ...
        size_t data_count = 0;
        vector<fixel_index_type> fixel_buffer;
        ...
        for (size_t fixel_index = 0; fixel_index != matrix.size(); ++fixel_index) {
          ...
          connectivity_value_type sum_connectivity = connectivity_value_type(0);
          ...
          fixel_stream.write (reinterpret_cast<const char*>(fixel_buffer.data()), fixel_buffer.size() * sizeof (fixel_index_type));
