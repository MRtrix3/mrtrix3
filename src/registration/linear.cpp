...
const OptionGroup adv_init_options =
    ...
    + Option("init_rotation.unmasked1",
             "disregard mask1 for the rotation initialisation (affects 'search' and 'moments')") +
    Option("init_rotation.unmasked2",
           "disregard mask2 for the rotation initialisation (affects 'search' and 'moments')") +
...
const OptionGroup rigid_options =
    ...
    + Option("rigid_1tomidway",
             "the output text file containing the rigid transformation that "
    ...
    Option(
        "rigid_niter",
        "the maximum number of gradient descent iterations per stage. This can be specified either as a single number "
    ...
    + Option("rigid_log", "write gradient descent parameter evolution to log file") + Argument("file").type_file_out();
...
const OptionGroup affine_options =
    ...
    + Option("affine_1tomidway",
             "the output text file containing the affine transformation that "
             "aligns image1 to image2 in their common midway space as a 4x4 matrix") +
    Argument("file").type_file_out()
    ...
    + Option("affine_lmax",
             "explicitly set the lmax to be used per scale factor in affine FOD registration. By default FOD "
             "registration will "
             "use lmax 0,2,4 with default scale factors 0.25,0.5,1.0 respectively. Note that no reorientation will be "
             "performed with lmax = 0.") +
    Argument("num").type_sequence_int()

    + Option("affine_log", "write gradient descent parameter evolution to log file") + Argument("file").type_file_out();
...
const OptionGroup fod_options =
    ...
    +
    Option("directions",
           "the directions used for FOD reorientation using apodised point spread functions (Default: 60 directions)") +
