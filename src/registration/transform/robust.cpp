...
void calc_median_trafo(const MR::vector<transform_type> &candid_trafo,
                       const Eigen::Matrix<default_type, 4, 4> &vertices_4,
                       const transform_type &trafo_before,
                       transform_type &median_trafo,
                       MR::vector<default_type> &scores) {
  ...
  if (scores.size()) {
    ...
    Eigen::MatrixXd P, pn;
    Eigen::Matrix<default_type, 3, n_vertices> R =
        median_vertices_4.template block<3, n_vertices>(0, 0) - start_vertices;
    for (size_t i = 0; i < n_vertices; ++i) {
