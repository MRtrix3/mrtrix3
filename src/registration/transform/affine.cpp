      ...
      bool AffineUpdate::operator() (Eigen::Matrix<default_type, Eigen::Dynamic, 1>& newx,
          ...
          if (control_points.size()) {
            ...
            while (true) {
              ...
              } else if (projection_type == TransformProjectionType::affine_nonsym || projection_type == TransformProjectionType::rigid_nonsym) {
                DEBUG ("nonsymmetric affine update");
                newx = x - step_size * g;
                Registration::Transform::param_vec2mat(newx, Xnew);
                if (Xnew.determinant() <= 0.0){ step_size *= step_down_factor; continue; }
              } else {
              ...
              if (projection_type == TransformProjectionType::rigid_nonsym) {
                ...
                assert((R * R.transpose().eval()).isApprox(Eigen::Matrix<default_type, Eigen::Dynamic, Eigen::Dynamic>::Identity(3,3)) && "projection of affine to rotation orthonormal?");
