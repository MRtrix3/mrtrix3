...
bool RigidLinearNonSymmetricUpdate::operator()(Eigen::Matrix<default_type, Eigen::Dynamic, 1> &newx,
  ...
  if (control_points.size()) {
    ...
    if (cnt > 0)
      INFO("rigid: gradient descent step size was too large. Multiplied by factor " + str(std::pow(factor, cnt), 4) +
           " (now: " + str(step_size, 4) + ")");
