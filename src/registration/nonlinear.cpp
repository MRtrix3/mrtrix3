...
const OptionGroup nonlinear_options =
    ...
    + Option("nl_warp",
             "the non-linear warp output defined as two deformation fields, where warp1 can be used to transform "
             "image1->image2 and warp2 to transform image2->image1. The deformation fields also encapsulate any linear "
             "transformation estimated prior to non-linear registration.") +
    Argument("warp1").type_image_out() + Argument("warp2").type_image_out()

    + Option("nl_warp_full",
             "output all warps used during registration. This saves four different warps that map each image to a "
             "midway space and their inverses in a single 5D image file. "
    ...
    + Option("nl_init",
             "initialise the non-linear registration with the supplied warp image. The supplied warp must be in the "
             "same format as output using the -nl_warp_full option "
    ...
    + Option("nl_niter",
             "the maximum number of iterations. This can be specified either as a single number "
    ...
    + Option("diagnostics_image", "write intermediate images for diagnostics purposes") + Argument("path");
