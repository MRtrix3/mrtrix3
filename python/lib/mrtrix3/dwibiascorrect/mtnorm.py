...
def usage(base_parser, subparsers): #pylint: disable=unused-variable
  ...
  parser.add_description('This algorithm bases its operation almost entirely on the utilisation of multi-tissue '
                         ...
                         'as is implemented in the MRtrix3 command "mtnormalise". '
                         'Its typical usage is however slightly different, '
                         'in that the primary output of the command is not the bias-field-corrected FODs, '
                         'but a bias-field-corrected version of the DWI series.')
  parser.add_description('The operation of this script is a subset of that performed by the script "dwibiasnormmask". '
                         ...
                         'this dwibiascorrect algorithm is nevertheless provided to demonstrate '
                         'specifically the bias field correction portion of that command.')
...
def execute(): #pylint: disable=unused-variable
  ...
  class Tissue(object): #pylint: disable=useless-object-inheritance
    def __init__(self, name):
      self.name = name
      self.rffile = f'response_{name}.txt'
  ...
  run.command(['mrcalc', 'in.mif', 'field.mif', '-div', '-', '|',
              'mrconvert', '-', app.ARGS.output],
              mrconvert_keyval=app.ARGS.input,
              force=app.FORCE_OVERWRITE)

  if app.ARGS.bias:
    run.command(['mrconvert', 'field.mif', app.ARGS.bias],
                mrconvert_keyval=app.ARGS.input,
                force=app.FORCE_OVERWRITE)
