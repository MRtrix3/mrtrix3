...
def usage(cmdline): #pylint: disable=unused-variable
  ...
  cmdline.flag_mutually_exclusive_options( [ 'topup_field', 'se_epi' ], False )
  cmdline.flag_mutually_exclusive_options( [ 'topup_files', 'align_seepi' ], False )
  cmdline.flag_mutually_exclusive_options( [ 'topup_field', 'align_seepi' ], False )
  cmdline.flag_mutually_exclusive_options( [ 'topup_files', 'topup_options' ], False )
  cmdline.flag_mutually_exclusive_options( [ 'topup_field', 'topup_options' ], False )
...
def execute(): #pylint: disable=unused-variable
  ...
  if app.ARGS.topup_field and not app.ARGS.topup_files:
    raise MRtrixError('-topup_field cannot have any effect if -topup_files is not also provided')
  ...
  if app.ARGS.eddy_options:
    ...
    if eddy_topup_entry:
      ...
      topup_files_userpath = eddy_topup_entry[0][len('--topup='):]
  ...
  if topup_files_userpath:
    ...
    def find_fieldcoef(fieldcoef_prefix):
      ...
      if not fieldcoef_candidates:
        raise MRtrixError(f'No topup field coefficient image found based on path "{topup_files_userpath}"')
      if len(fieldcoef_candidates) > 1:
        raise MRtrixError(f'Multiple topup field coefficient images found based on path "{topup_files_userpath}": {fieldcoef_candidates}')
      return fieldcoef_candidates[0]

    if os.path.isfile(topup_files_userpath):
      if topup_files_userpath.endswith('_movpar.txt'):
        topup_input_movpar = app.Parser.FileIn()(topup_files_userpath)
        topup_input_fieldcoef = app.Parser.ImageIn()(find_fieldcoef(topup_files_userpath[:-len('_movpar.txt')]))
      ...
      if topup_files_userpath[-1] == '_':
        topup_files_userpath = topup_files_userpath[:-1]
      try:
        topup_input_movpar = app.Parser.FileIn()(f'{topup_files_userpath}_movpar.txt')
        topup_input_fieldcoef = app.Parser.ImageIn()(find_fieldcoef(topup_files_userpath))
      except argparse.ArgumentTypeError as exc:
        raise argparse.ArgumentTypeError(f'Unable to find topup files from user specification "{topup_files_userpath}"') from exc
  ...
  # Need to un-permute volumes before proceeding with dwirecon
  run.command(f'mrconvert {eddy_output_image_path} dwi_post_eddy.mif'
              f' -fslgrad {bvecs_path} bvals'
              f' -import_pe_eddy eddy_config.txt eddy_indices.txt'
              f'{dwi_permvols_posteddy_option}')
  ...
  if pe_design in (PEDesign.SplitRPE, PEDesign.AllRPE, PEDesign.InferFromHeader) \
    ...
    # 2. Generated by us from topup
    if topup_input_fieldimage:
      field_map_image = 'field_map.mif'
    elif execute_topup:
      field_map_image = fsl.find_image('field_map')
