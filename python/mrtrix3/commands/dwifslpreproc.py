...
def usage(cmdline): #pylint: disable=unused-variable
  ...
  cmdline.set_synopsis('Perform diffusion image pre-processing using FSL\'s eddy tool; '
                       'including inhomogeneity distortion correction using FSL\'s topup tool if possible')
  cmdline.add_description('This script is intended to provide convenience of use of the FSL software tools'
                          ' topup and eddy for performing DWI pre-processing,'
                          ' by encapsulating some of the surrounding image data and metadata processing steps.'
                          ' It is intended to simply these processing steps for most commonly-used DWI acquisition strategies,'
                          ' whilst also providing support for some more exotic acquisitions.'
                          ' The "example usage" section demonstrates the ways in which the script can be used'
                          ' based on the (compulsory) -rpe_* command-line options.')
  cmdline.add_description('More information on use of the dwifslpreproc command can be found at the following link: \n'
                          f'https://mrtrix.readthedocs.io/en/{version.TAG}/dwi_preprocessing/dwifslpreproc.html')
  cmdline.add_description('Note that the MRtrix3 command dwi2mask will automatically be called'
                          ' to derive a processing mask for the FSL command "eddy",'
                          ' which determines which voxels contribute to the estimation of geometric distortion parameters'
                          ' and possibly also the classification of outlier slices.'
                          ' If FSL command "topup" is used to estimate a susceptibility field,'
                          ' then dwi2mask will be executed on the resuts of running FSL command "applytopup" to the input DWIs;'
                          ' otherwise it will be executed directly on the input DWIs.'
                          ' Alternatively, the -eddy_mask option can be specified in order to manually provide such a processing mask.'
  ...
  cmdline.add_description('The "-topup_options" and "-eddy_options" command-line options allow the user'
                          ' to pass desired command-line options directly to the FSL commands topup and eddy.'
                          ' The available options for those commands may vary between versions of FSL;'
                          ' users can interrogate such by querying the help pages of the installed software,'
                          ' and/or the FSL online documentation: \n'
  ...
  cmdline.add_description('The script will attempt to run the CUDA version of eddy;'
                          ' if this does not succeed for any reason,'
                          ' or is not present on the system,'
                          ' the CPU version will be attempted instead.'
                          ' By default, the CUDA eddy binary found that indicates compilation'
                          ' against the most recent version of CUDA will be attempted;'
                          ' this can be over-ridden by providing a soft-link "eddy_cuda" within your path'
                          ' that links to the binary you wish to be executed.')
  cmdline.add_description('Note that this script does not perform any explicit registration'
                          ' between images provided to topup via the -se_epi option,'
                          ' and the DWI volumes provided to eddy.'
                          ' In some instances (motion between acquisitions)'
                          ' this can result in erroneous application of the inhomogeneity field during distortion correction.'
                          ' Use of the -align_seepi option is advocated in this scenario,'
                          ' which ensures that the first volume in the series provided to topup'
                          ' is also the first volume in the series provided to eddy,'
                          ' guaranteeing alignment.'
                          ' But a prerequisite for this approach is that the image contrast'
                          ' within the images provided to the -se_epi option'
                          ' must match the b=0 volumes present within the input DWI series:'
                          ' this means equivalent TE, TR and flip angle'
                          ' (note that differences in multi-band factors between two acquisitions'
                          ' may lead to differences in TR).')
  cmdline.add_example_usage('A basic DWI acquisition,'
                            ' where all image volumes are acquired in a single protocol with fixed phase encoding',
                            'dwifslpreproc DWI_in.mif DWI_out.mif -rpe_none -pe_dir ap -readout_time 0.55',
                            'Due to use of a single fixed phase encoding,'
                            ' no EPI distortion correction can be applied in this case.')
  cmdline.add_example_usage('DWIs all acquired with a single fixed phase encoding; '
                            'but additionally a pair of b=0 images with reversed phase encoding'
                            ' to estimate the inhomogeneity field',
                            'mrcat b0_ap.mif b0_pa.mif b0_pair.mif -axis 3;'
                            ' dwifslpreproc DWI_in.mif DWI_out.mif -rpe_pair -se_epi b0_pair.mif -pe_dir ap -readout_time 0.72 -align_seepi',
                            'Here the two individual b=0 volumes are concatenated into a single 4D image series,'
                            ' and this is provided to the script via the -se_epi option.'
                            ' Note that with the -rpe_pair option used here,'
                            ' which indicates that the SE-EPI image series contains'
                            ' one or more pairs of b=0 images with reversed phase encoding,'
                            ' the FIRST HALF of the volumes in the SE-EPI series must possess'
                            ' the same phase encoding as the input DWI series,'
                            ' while the second half are assumed to contain the opposite'
  ...
  cmdline.add_example_usage('All DWI directions & b-values are acquired twice,'
                            ...
                            'mrcat DWI_lr.mif DWI_rl.mif DWI_all.mif -axis 3;'
                            ' dwifslpreproc DWI_all.mif DWI_out.mif -rpe_all -pe_dir lr -readout_time 0.66',
                            'Here the two acquisition protocols are concatenated'
                            ' into a single DWI series containing all acquired volumes.'
                            ' The direction indicated via the -pe_dir option should be the direction of'
                            ' phase encoding used in acquisition of the FIRST HALF of volumes in the input DWI series;'
                            ' ie. the first of the two files that was provided to the mrcat command.'
                            ' In this usage scenario,'
                            ' the output DWI series will contain the same number of image volumes'
                            ' as ONE of the acquired DWI series'
                            ' (ie. half of the number in the concatenated series);'
                            ' this is because the script will identify pairs of volumes that possess'
                            ' the same diffusion sensitisation but reversed phase encoding,'
                            ' and perform explicit recombination of those volume pairs in such a way'
                            ' that image contrast in regions of inhomogeneity is determined'
                            ' from the stretched rather than the compressed image.')
  cmdline.add_example_usage('Any acquisition scheme that does not fall into one of the example usages above',
                            'mrcat DWI_*.mif DWI_all.mif -axis 3;'
                            ' mrcat b0_*.mif b0_all.mif -axis 3;'
                            ' dwifslpreproc DWI_all.mif DWI_out.mif -rpe_header -se_epi b0_all.mif -align_seepi',
                            'With this usage,'
                            ' the relevant phase encoding information is determined'
                            ' entirely based on the contents of the relevant image headers,'
                            ' and dwifslpreproc prepares all metadata for the executed FSL commands accordingly. '
                            ' This can therefore be used if the particular DWI acquisition strategy used'
                            ' does not correspond to one of the simple examples as described in the prior examples.'
                            ' This usage is predicated on the headers of the input files'
                            ' containing appropriately-named key-value fields'
                            ' such that MRtrix3 tools identify them as such.'
                            ' In some cases,'
                            ' conversion from DICOM using MRtrix3 commands will automatically extract and embed this information;'
                            ' however this is not true for all scanner vendors and/or software versions.'
                            ' In the latter case it may be possible to manually provide these metadata;'
                            ' either using the -json_import command-line option of dwifslpreproc,'
                            ' or the -json_import or one of the -import_pe_* command-line options of MRtrix3\'s mrconvert command'
                            ' (and saving in .mif format)'
                            ' prior to running dwifslpreproc.')
  cmdline.add_citation('Andersson, J. L. & Sotiropoulos, S. N. '
  ...
  cmdline.add_citation('Smith, S. M.; Jenkinson, M.; Woolrich, M. W.; Beckmann, C. F.; Behrens, T. E.; Johansen-Berg, H.; Bannister, P. R.; De Luca, M.; Drobnjak, I.; Flitney, D. E.; Niazy, R. K.; Saunders, J.; Vickers, J.; Zhang, Y.; De Stefano, N.; Brady, J. M. & Matthews, P. M. '
                       ...
                       is_external=True)
  cmdline.add_citation('Skare, S. & Bammer, R. '
                       'Jacobian weighting of distortion corrected EPI data. '
                       'Proceedings of the International Society for Magnetic Resonance in Medicine, 2010, 5063',
                       condition='If performing recombination of diffusion-weighted volume pairs'
                                 ' with opposing phase encoding directions',
                       is_external=True)
  cmdline.add_citation('Andersson, J. L.; Skare, S. & Ashburner, J. '
                       ...
                       condition='If performing EPI susceptibility distortion correction',
                       is_external=True)
  cmdline.add_citation('Andersson, J. L. R.; Graham, M. S.; Zsoldos, E. & Sotiropoulos, S. N. '
                       ...
                       condition='If including "--repol" in -eddy_options input',
                       is_external=True)
  cmdline.add_citation('Andersson, J. L. R.; Graham, M. S.; Drobnjak, I.; Zhang, H.; Filippini, N. & Bastiani, M. '
                       'Towards a comprehensive framework for movement and distortion correction of diffusion MR images: '
                       'Within volume movement. '
                       'NeuroImage, 2017, 152, 450-466',
                       condition='If including "--mporder" in -eddy_options input',
                       is_external=True)
  cmdline.add_citation('Bastiani, M.; Cottaar, M.; Fitzgibbon, S.P.; Suri, S.; Alfaro-Almagro, F.; Sotiropoulos, S.N.; Jbabdi, S.; Andersson, J.L.R. '
...
def execute(): #pylint: disable=unused-variable
  ...
  if pe_design == 'None':
    ...
    if not os.path.isfile(topup_config_path):
      raise MRtrixError(f'Could not find necessary default config file for FSL topup command '
                        f'(expected location: {topup_config_path})')
  ...
  if app.ARGS.eddyqc_text:
    eddyqc_path = app.ARGS.eddyqc_text
  elif app.ARGS.eddyqc_all:
    eddyqc_path = app.ARGS.eddyqc_all
  ...
  if app.ARGS.eddy_options:
    ...
    if any(entry.startswith('--mask=') for entry in eddy_manual_options):
      raise MRtrixError('Cannot provide eddy processing mask via -eddy_options "--mask=..." as manipulations are required; '
                        'use -eddy_mask option instead')
    if any(entry.startswith('--slspec=') for entry in eddy_manual_options):
      raise MRtrixError('Cannot provide eddy slice specification file via -eddy_options "--slspec=..." as manipulations are required; '
                        'use -eddy_slspec option instead')
    if '--resamp=lsr' in eddy_manual_options:
      raise MRtrixError('dwifslpreproc does not currently support least-squares reconstruction; '
                        'this cannot be simply passed via -eddy_options')
    ...
    if eddy_topup_entry:
      ...
      topup_file_userpath = eddy_topup_entry[0][len('--topup='):]
  ...
  if app.ARGS.topup_files:
    ...
    topup_file_userpath = app.ARGS.topup_files
  ...
  if topup_file_userpath:
    ...
    def find_fieldcoef(fieldcoef_prefix):
      fieldcoef_candidates = glob.glob(f'{fieldcoef_prefix}_fieldcoef.nii*')
      ...
      if len(fieldcoef_candidates) > 1:
        raise MRtrixError(f'Multiple topup field coefficient images found based on path "{topup_file_userpath}": {fieldcoef_candidates}')
  ...
  run.command(['mrconvert', app.ARGS.input, 'dwi.mif']
              + (['-json_import', app.ARGS.json_import] if app.ARGS.json_import else [])
              + ['-json_export', 'dwi.json']
              + app.dwgrad_import_options(),
              preserve_pipes=True)
  if app.ARGS.se_epi:
    image.check_3d_nonunity(app.ARGS.se_epi)
    run.command(f'mrconvert {app.ARGS.se_epi} se_epi.mif',
                preserve_pipes=True)
  if topup_file_userpath:
    run.function(shutil.copyfile, topup_input_movpar, 'field_movpar.txt')
  ...
  if app.ARGS.eddy_mask:
    run.command(['mrconvert', app.ARGS.eddy_mask, 'eddy_mask.mif', '-datatype', 'bit'],
                preserve_pipes=True)
  ...
  app.debug(f'Number of DWI volumes: {dwi_num_volumes}')
  dwi_num_slices = dwi_header.size()[2]
  app.debug(f'Number of DWI slices: {dwi_num_slices}')
  ...
  if not len(grad) == dwi_num_volumes:
    raise MRtrixError(f'Number of lines in gradient table ({len(grad)}) '
                      f'does not match input image ({dwi_num_volumes} volumes); '
  ...
  if eddy_mporder:
    ...
    if app.ARGS.eddy_slspec:
      try:
        slice_groups = matrix.load_numeric(app.ARGS.eddy_slspec, dtype=int)
        app.debug(f'Slice groups: {slice_groups}')
      except ValueError:
        try:
          ...
          if len(slice_timing) != dwi_num_slices:
            raise MRtrixError(f'Cannot use slice timing information from file "{app.ARGS.eddy_slspec}" for slice-to-volume correction: ' # pylint: disable=raise-missing-from
                              f'number of entries ({len(slice_timing)}) does not match number of slices ({dwi_num_slices})')
        except ValueError:
          raise MRtrixError(f'Error parsing eddy "slspec" file "{app.ARGS.eddy_slspec}" ' # pylint: disable=raise-missing-from
                            f'(please see FSL eddy help page, specifically the --slspec option)')
    else:
      if 'SliceTiming' not in dwi_header.keyval():
        raise MRtrixError('Cannot perform slice-to-volume correction in eddy: '
                          '-eddy_slspec option not specified, '
                          'and no slice timing information present in input DWI header')
      slice_timing = dwi_header.keyval()['SliceTiming']
      app.debug(f'Initial slice timing contents from header: {slice_timing}')
      if slice_timing in ['invalid', 'variable']:
        raise MRtrixError(f'Cannot use slice timing information in image header for slice-to-volume correction: '
                          f'data flagged as "{slice_timing}"')
      # Fudges necessary to manipulate nature of slice timing data in cases where
  ...
  if not app.ARGS.eddy_options or not any(s.startswith('--slm=') for s in app.ARGS.eddy_options.split()):
    ...
    if len(shell_bvalues) == len(shell_asymmetries) + 1:
      ...
      raise MRtrixError(f'Number of b-values reported by mrinfo ({len(shell_bvalues)}) '
                        f'does not match number of outputs provided by dirstat ({len(shell_asymmetries)})')
    peak_asymmetry = max(shell_asymmetries)
    if peak_asymmetry >= 0.1:
      severity = 'Strongly' if peak_asymmetry >= 0.4 else 'Moderately'
      app.warn(f'{severity} unbalanced diffusion sensitisation gradient scheme detected; '
               'eddy current distortion correction may benefit from '
               'presence of second-level model, eg.: '
  ...
  app.debug(f'Manual PE direction: {manual_pe_dir}')
  manual_trt = None
  if app.ARGS.readout_time:
    manual_trt = float(app.ARGS.readout_time)
  app.debug(f'Manual readout time: {manual_trt}')
  ...
  if manual_pe_dir:
    ...
    #   that volumes must be matched between the first and second halves of the DWI data.
      ...
      grad_pairs = [ ]
      app.debug(f'Commencing gradient direction matching; {dwi_num_volumes} volumes')
          ...
          for index2 in range(int(dwi_num_volumes/2), dwi_num_volumes):
            if grads_matched[index2] == dwi_num_volumes: # Also as yet unpaired
              if grads_match(index1, index2):
                ...
                app.debug(f'Matched volume {index1} with {index2}: {grad[index1]} {grad[index2]}')
                break
          else:
            raise MRtrixError(f'Unable to determine matching reversed phase-encode direction volume for DWI volume {index1}')
  ...
  if dwi_pe_scheme:
    if manual_pe_dir:
      ...
      if not scheme_dirs_match(dwi_pe_scheme, dwi_manual_pe_scheme):
        app.warn('User-defined phase-encoding direction design does not match what is stored in DWI image header; '
    ...
    if dwi_auto_trt_warning:
      app.console(f'Total readout time not provided at command-line; '
                  f'assuming sane default of {auto_trt}')
  ...
  app.debug(f'Index of first b=0 image in DWIs is {dwi_first_bzero_index}')
  ...
  if app.ARGS.se_epi:
    ...
    if not image.match(dwi_header, se_epi_header, up_to_dim=3):
      ...
      run.command(f'mrtransform {se_epi_path} - -reorient_fod no -interp sinc -template dwi.mif | '
                  f'mrcalc - 0.0 -max {new_se_epi_path}')
      ...
      if se_epi_pe_scheme:
        if manual_pe_dir:
          if not scheme_dirs_match(se_epi_pe_scheme, se_epi_manual_pe_scheme):
            app.warn('User-defined phase-encoding direction design does not match what is stored in SE EPI image header; '
                     'proceeding with user specification')
      ...
      if not se_epi_pe_scheme_has_contrast:
        if app.ARGS.align_seepi:
          ...
          if se_epi_pe_scheme_has_contrast:
            ...
            raise MRtrixError('No phase-encoding contrast present in SE-EPI images, '
                              'even after concatenating with b=0 images due to -align_seepi option; '
    ...
    if app.ARGS.align_seepi:
      ...
      for field_name, description in { 'EchoTime': 'echo time',
        ...
        if dwi_value and se_epi_value and dwi_value != se_epi_value:
          app.warn(f'It appears that the spin-echo EPI images used for inhomogeneity field estimation '
                   f'have a different {description} to the DWIs being corrected; '
                   f'this may cause issues in estimation of the field, '
                   f'as the first DWI b=0 volume will be added to the input series to topup '
                   f'due to use of the -align_seepi option.')
      ...
      if dwi_first_bzero_index == len(grad) and not dwi_bzero_added_to_se_epi:
        ...
        app.warn('Unable to find b=0 volume in input DWIs to provide alignment between topup and eddy; '
                 'script will proceed as though the -align_seepi option were not provided')
      ...
      #   absence of phase-encoding contrast in the latter, we don't need to perform the following
        ...
        run.command(f'mrconvert dwi.mif dwi_first_bzero.mif -coord 3 {dwi_first_bzero_index} -axes 0,1,2')
        ...
        if (se_epi_pe_sum == [ 0, 0, 0 ]) and (se_epi_volume_to_remove < len(se_epi_pe_scheme)):
          ...
          seepi_volumes_to_preserve = ','.join(str(index) for index in range(len(se_epi_pe_scheme)) if not index == se_epi_volume_to_remove)
          run.command(f'mrconvert {se_epi_path} - -coord 3 {seepi_volumes_to_preserve} | '
                      f'mrcat dwi_first_bzero.mif - {new_se_epi_path} -axis 3')
          ...
          if se_epi_pe_sum == [ 0, 0, 0 ] and se_epi_volume_to_remove == len(se_epi_pe_scheme):
            app.console('Phase-encoding scheme of -se_epi image is balanced, '
                        'but could not find appropriate volume with which to substitute first b=0 volume from DWIs; '
                        'first b=0 DWI volume will be inserted to start of series, '
                        'resulting in an unbalanced scheme')
          else:
            app.console('Unbalanced phase-encoding scheme detected in series provided via -se_epi option; '
                        'first DWI b=0 volume will be inserted to start of series')
          run.command(f'mrcat dwi_first_bzero.mif {se_epi_path} {new_se_epi_path} -axis 3')
    ...
    run.command(f'mrconvert dwi.mif {import_dwi_pe_table_option} - | '
                f'dwiextract - {se_epi_path} -bzero')
    se_epi_header = image.Header(se_epi_path)
  ...
  if execute_topup:
    ...
    for axis_size in dwi_header.size()[:3]:
      if int(axis_size%2):
        odd_axis_count += 1
    if odd_axis_count:
      app.console(f'{odd_axis_count} spatial {"axes of DWIs have" if odd_axis_count > 1 else "axis of DWIs has"} non-even size; '
                  f'this will be automatically padded for compatibility with topup, '
                  f'and the extra {"slices" if odd_axis_count > 1 else "slice"} erased afterwards')
      for axis, axis_size in enumerate(dwi_header.size()[:3]):
        if int(axis_size%2):
          new_se_epi_path = f'{os.path.splitext(se_epi_path)[0]}_pad{axis}.mif'
          run.command(f'mrconvert {se_epi_path} -coord {axis} {axis_size-1} - | '
                      f'mrcat {se_epi_path} - {new_se_epi_path} -axis {axis}')
          app.cleanup(se_epi_path)
          se_epi_path = new_se_epi_path
          ...
          dwi_post_eddy_crop_option += f' -coord {axis} 0:{axis_size-1}'
    ...
    if app.VERBOSITY > 1:
      ...
      sys.stderr.write(topup_terminal_output)
  ...
  if execute_applytopup:
    ...
    app.debug(f'applytopup_config: {applytopup_config}')
  ...
  if eddy_mporder:
    ...
    #   SliceEncodingDirection to be present.
      ...
      app.debug(f'Original: {slice_groups}')
      app.debug(f'New: {new_slice_groups}')
      slice_groups = new_slice_groups
  ...
  if eddy_cuda_cmd:
    ...
    try:
      ...
      try:
        ...
        eddy_cuda_header = f'{"="*len(eddy_cuda_cmd)}\n{eddy_cuda_cmd}\n{"="*len(eddy_cuda_cmd)}\n'
  ...
  if app.VERBOSITY > 1:
    ...
    sys.stderr.write(eddy_terminal_output)
  ...
  if not os.path.isfile(bvecs_path):
    app.warn('eddy has not provided rotated bvecs file; '
             'using original gradient table. '
             'Recommend updating FSL eddy to version 5.0.9 or later.')
    bvecs_path = 'bvecs'
  ...
  if eddyqc_path:
    if shutil.which('eddy_quad'):
      ...
      if dwi_post_eddy_crop_option:
        ...
        for eddy_filename in eddyqc_files:
          if os.path.isfile('dwi_post_eddy.' + eddy_filename):
            if slice_padded and eddy_filename in [ 'eddy_outlier_map', 'eddy_outlier_n_sqr_stdev_map', 'eddy_outlier_n_stdev_map' ]:
              ...
              run.function(os.symlink, f'dwi_post_eddy.{eddy_filename}', f'dwi_post_eddy_unpad.{eddy_filename}')
            app.cleanup(f'dwi_post_eddy.{eddy_filename}')
          progress.increment()

        if eddy_mporder and slice_padded:
          app.debug(f'Current slice groups: {slice_groups}')
          ...
          app.debug(f'Slice groups after removal: {slice_groups}')
          try:
        ...
        run.command(f'mrconvert eddy_mask.nii eddy_mask_unpad.nii {dwi_post_eddy_crop_option}')
      ...
      eddyqc_options = f' -idx eddy_indices.txt -par eddy_config.txt -b bvals -m {eddyqc_mask}'
      if os.path.isfile(f'{eddyqc_prefix}.eddy_residuals.nii.gz'):
        eddyqc_options += f' -g {bvecs_path}'
      if execute_topup:
        eddyqc_options += f' -f {eddyqc_fieldmap}'
      if eddy_mporder:
        eddyqc_options += f' -s {eddyqc_slspec}'
      if app.VERBOSITY > 2:
        eddyqc_options += ' -v'
      try:
        run.command(f'eddy_quad {eddyqc_prefix} {eddyqc_options}')
        ...
        app.warn(f'Error running automated EddyQC tool "eddy_quad"; '
                 f'QC data written to "{eddyqc_path}" will be files from "eddy" only')
        # Delete the directory if the script only made it partway through
        try:
          shutil.rmtree(f'{eddyqc_prefix}.qc')
        except OSError:
          pass
    else:
      ...
      for index2 in range(index1+1, dwi_num_volumes):
        if volume_matchings[index2] == dwi_num_volumes: # Also as yet unpaired
          ...
          if not any(dwi_pe_scheme[index1][i] + dwi_pe_scheme[index2][i] for i in range(0,3)) and grads_match(index1, index2):
            ...
            app.debug(f'Matched volume {index1} with {index2}\n'
                      f'Phase encoding: {dwi_pe_scheme[index1]} {dwi_pe_scheme[index2]}\n'
                      f'Gradients: {grad[index1]} {grad[index2]}')
            break
  ...
  if len(volume_pairs) != int(dwi_num_volumes/2):
    ...
    app.console('Detected matching DWI volumes with opposing phase encoding; '
                'performing explicit volume recombination')
    ...
    if not image.match('topup_in.nii', field_map_header, up_to_dim=3):
      app.warn('topup output field image has erroneous header; '
               'recommend updating FSL to version 5.0.8 or later')
      new_field_map_image = 'field_map_fix.mif'
      run.command(f'mrtransform {field_map_image} -replace topup_in.nii {new_field_map_image}')
    ...
    # The first volume is however the expected field offset image
      ...
      new_field_map_image = 'field_map_fix.mif'
      run.command(f'mrconvert {field_map_image} -coord 3 0 -axes 0,1,2 {new_field_map_image}')
      app.cleanup(field_map_image)
      field_map_image = new_field_map_image
    app.cleanup('topup_in.nii')
    ...
    app.debug(f'EDDY config: {eddy_config}')
    app.debug(f'EDDY indices: {eddy_indices}')
    ...
    for index, config in enumerate(eddy_config):
      ...
      field_derivative_path = f'field_deriv_pe{index+1}.mif'
      total_readout_time = config[3]
      run.command(f'mrcalc {field_map_image} {total_readout_time} -mult {sign_multiplier} - | '
                  f'mrfilter - gradient - | '
                  f'mrconvert - {field_derivative_path} -coord 3 {pe_axis} -axes 0,1,2')
      jacobian_path = f'jacobian{index+1}.mif'
      run.command(f'mrcalc 1.0 {field_derivative_path} -add 0.0 -max {jacobian_path}')
      app.cleanup(field_derivative_path)
      run.command(f'mrcalc {jacobian_path} {jacobian_path} -mult weight{index+1}.mif')
    ...
    if dwi_permvols_posteddy_option:
      new_eddy_output_image_path = f'{os.path.splitext(eddy_output_image_path)[0]}_volpermuteundo.mif'
      run.command(f'mrconvert {eddy_output_image_path} {new_eddy_output_image_path} {dwi_permvols_posteddy_option}')
      app.cleanup(eddy_output_image_path)
      eddy_output_image_path = new_eddy_output_image_path
    ...
    for index, volumes in enumerate(volume_pairs):
      ...
      run.command(f'mrconvert {eddy_output_image_path} volume0.mif -coord 3 {volumes[0]}')
      run.command(f'mrconvert {eddy_output_image_path} volume1.mif -coord 3 {volumes[1]}')
      # Volume recombination equation described in Skare and Bammer 2010
      combined_image_path = f'combined{index}.mif'
      run.command(f'mrcalc volume0.mif weight{pe_indices[0]}.mif -mult volume1.mif weight{pe_indices[1]}.mif -mult -add weight{pe_indices[0]}.mif weight{pe_indices[1]}.mif -add -divide 0.0 -max {combined_image_path}')
    ...
    for index in range(0, len(eddy_config)):
      app.cleanup(f'weight{index+1}.mif')
  ...
  if eddyqc_path:
    ...
    if app.ARGS.eddyqc_all:
      if dwi_post_eddy_crop_option:
        mask_export_path = shlex.quote(os.path.join(eddyqc_path, 'eddy_mask.nii'))
        run.command(f'mrconvert eddy_mask.nii {mask_export_path} {dwi_post_eddy_crop_option}',
                    force=app.FORCE_OVERWRITE)
      else:
