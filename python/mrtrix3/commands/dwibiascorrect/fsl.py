...
from mrtrix3 import app, fsl, run, utils
...
MRTRIX_DEPENDENCIES |= {'dwiextract', 'mrcalc', 'mrconvert', 'mrmath'}
...
def usage(base_parser, subparsers): #pylint: disable=unused-variable
  ...
  parser.add_citation('Smith, S. M.; Jenkinson, M.; Woolrich, M. W.; Beckmann, C. F.; Behrens, T. E.; Johansen-Berg, H.; Bannister, P. R.; De Luca, M.; Drobnjak, I.; Flitney, D. E.; Niazy, R. K.; Saunders, J.; Vickers, J.; Zhang, Y.; De Stefano, N.; Brady, J. M. & Matthews, P. M. '
                      ...
                      is_external=True)
  parser.add_description('The FSL "fast" command only estimates the bias field within a brain mask, '
                         'and cannot extrapolate this smoothly-varying field beyond the defined mask. '
                         'As such, this algorithm by necessity introduces a hard masking of the input DWI. '
                         'Since this attribute may interfere with the purpose of using the command '
...
def execute(): #pylint: disable=unused-variable
  ...
  run.command('mrcalc mean_bzero.mif mask.mif -mult - | '
              'mrconvert - mean_bzero_masked.nii -strides -1,+2,+3')
  run.command(f'{fast_cmd} -t 2 -o fast -n 3 -b mean_bzero_masked.nii')
  ...
  run.command(f'mrcalc in.mif {bias_path} -div mask.mif -mult result.mif')
  run.command(['mrconvert', 'result.mif', app.ARGS.output],
              mrconvert_keyval=app.ARGS.input,
              force=app.FORCE_OVERWRITE,
              preserve_pipes=True)
  if app.ARGS.bias:
    run.command(['mrconvert', bias_path, app.ARGS.bias],
                mrconvert_keyval=app.ARGS.input,
                force=app.FORCE_OVERWRITE,
                preserve_pipes=True)
