...
def usage(base_parser, subcommands): #pylint: disable=unused-variable
  ...
  parser.add_citation('Tustison, N.; Avants, B.; Cook, P.; Zheng, Y.; Egan, A.; Yushkevich, P. & Gee, J. '
                      ...
                      is_external=True)
  ants_options = parser.add_option_group('Options for ANTs N4BiasFieldCorrection command')
  for key in sorted(OPT_N4_BIAS_FIELD_CORRECTION):
    ants_options.add_option(f'ants_{key}',
                            f'N4BiasFieldCorrection option -{key}: {OPT_N4_BIAS_FIELD_CORRECTION[key][1]}',
                            type=str,
