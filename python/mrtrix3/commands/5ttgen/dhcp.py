...
from mrtrix3 import app, run, utils
...
def usage(base_parser, subparsers): #pylint: disable=unused-variable
  ...
  parser.set_synopsis('Use ANTs commands, the output of the dHCP pipeline, and the M-CRIB atlas,'
                      ' to generate the 5TT image of a neonatal subject based on a T2-weighted image')
  parser.add_description('Derivation of the 5TT image is principally based on the segmentation already performed in the dHCP pipeline.'
                         ' The M-CRIB atlas will only be used to introduce'
                         ' additional sub-cortical grey matter parcels into the tissue segmentation.'
                         ' By default, the algorithm will use the tissue probability maps obtained from the dHCP.'
                         ' However, if the pipeline was executed without the -additional command-line flag,'
  ...
  parser.add_citation('Alexander, B.; Murray, A.L.; Loh, W.Y.; Matthews, L.G.; Adamson, C.; Beare, R.; Chen, J.; Kelly, C.E.; Rees, S.; Warfield, S.K.; Anderson, P.J.; Doyle, L.W.; Spittle, A.J.; Cheong, J.L.Y; Seal, M.L. & Thompson, D.K.'
                      ...
                      is_external=True)
  parser.add_citation('Makropoulos, A., Robinson, E.C., Schuh, A., Wright, R., Fitzgibbon, S., Bozek, J., Counsell, S.J., Steinweg, J., Vecchiato, K., Passerat-Palmbach, J., Lenz, G., Mortari, F., Tenev, T., Duff, E.P., Bastiani, M., Cordero-Grande, L., Hughes, E., Tusor, N., Tournier, J.D., Hutter, J., Price, A.N., Teixeira, R.P.A.G., Murgasova, M., Victor, S., Kelly, C., Rutherford, M.A., Smith, S.M., Edwards, A.D., Hajnal, J.V., Jenkinson, M. & Rueckert, D.'
                      ' The developing human connectome project:'
                      ' A minimal processing pipeline for neonatal cortical surface reconstruction.'
                      ' NeuroImage, 2018, 173, 88-112.')
  parser.add_argument('input',
                      help='The input path of the derivates folder (anat/) obtained from the dHCP pipeline')
...
def get_inputs(): #pylint: disable=unused-variable
  ...
  t2w_file = app.ARGS.input.glob('*_T2w_restore.nii.gz') + app.ARGS.input.glob('*restore_T2w.nii.gz')
  if not t2w_file:
    raise MRtrixError('Could not find a T2w image in the dHCP folder;'
                      ' please check that the file exists')
  if len(t2w_file) > 1:
    raise MRtrixError('Multiple candidate T2w images in the dHCP folder;'
                      ' please check directory contents')
  t2w_file = t2w_file[0]
  run.command(f'mrconvert {t2w_file} input_raw.mif')

  mask_file = app.ARGS.input.glob('*_brainmask_drawem.nii.gz') + app.ARGS.input.glob('**-brain_mask.nii.gz')
  if not mask_file:
    raise MRtrixError('Could not find a brain mask in the dHCP folder;'
                      ' please check that the file exists')
  if len(mask_file) > 1:
    raise MRtrixError('Multiple candidate brain mask image in the dHCP folder;'
                      ' please check directory contents')
  run.command(f'mrconvert {mask_file[0]} mask.mif -datatype bit -strides -1,+2,+3')

  mcrib_import = utils.RunList('Importing M-CRIB data to scratch directory', 20)
  for i in range(1, 11):
    mcrib_import.command(['mrconvert', mcrib_dir / f'M-CRIB_P{i:02d}_T2.nii.gz', f'template_{i:02d}.nii'])
    mcrib_import.command(['mrconvert', mcrib_dir / f'M-CRIB_orig_P{i:02d}_parc.nii.gz', f'template_labels_{i:02d}.nii'])

  dhcp_import = utils.RunList('Importing dHCP data to scratch directory', 87)
  dhcp_dir_posteriors = app.ARGS.input / 'posteriors'
  use_hard_segmentation = False
  if app.ARGS.hard_segmentation:
    use_hard_segmentation = True
  elif not dhcp_dir_posteriors:
    app.warn('No tissue posteriors found'
             ' (-additional flag not used in the dHCP pipeline);'
             ' output will be hard segmentation')
    use_hard_segmentation = True
  if use_hard_segmentation:
    labels = app.ARGS.input.glob('*_drawem_all_labels.nii.gz') + app.ARGS.input.glob('*-drawem87*.nii.gz')
    if not labels:
      raise MRtrixError('Could not find a binary segmentation in the dHCP folder;'
                        ' please check that the file exists')
    if len(labels) > 1:
      raise MRtrixError('Multiple candidate binary segmentations in the dHCP folder;'
                        ' please check directory contents')
    dhcp_import.command(f'mrconvert {labels[0]} all_labels_dHCP.mif -strides -1,+2,+3')
  else:
    for i in range(1, 88):
      images = dhcp_dir_posteriors.glob(f'*_drawem_seg{i}.nii.gz')
      if not images:
        raise MRtrixError(f'Unable to find dHCP image for parcel {i} in location "{dhcp_dir_posteriors}"')
      if len(images) > 1:
        raise MRtrixError(f'Multiple dHCP images found for parcel {i} in location "{dhcp_dir_posteriors}"')
      dhcp_import.command(f'mrconvert {images[0]} label_{i}.mif')
...
def execute(): #pylint: disable=unused-variable
  ...
  if use_hard_segmentation:
    ...
    run.command('mrcalc '
                + ' '.join(f'input_parcellation_Labels.nii {i} -eq' for i in MCRIB_SGM)
                + ' '
                + ' '.join(['-add'] * (len(MCRIB_SGM) - 1))
                + ' sGM_mcrib.mif')
  else:
    #extract sGM from M-CRIB
    run.command(['mrmath',
                 list('posterior{i:04d}.nii.gz' for i in MCRIB_SGM),
                 'sum',
                 'sGM_mcrib.mif'])
  ...
  for tissue, indices in { 'cGM': DHCP_CGM + ([] if app.ARGS.sgm_amyg_hipp else DHCP_AMYG_HIPP),
    ...
    if use_hard_segmentation:
      run.command('mrcalc '
                  + ' '.join(f'all_labels_dHCP.mif {i} -eq' for i in indices)
                  + ' '
                  + ' '.join(['-add'] * (len(indices) - 1))
                  + f' Pmap-{tissue}.mif')
    else:
      run.command(['mrmath',
                   list(f'label_{i}.mif' for i in indices),
                   'sum',
                   f'Pmap-{tissue}.mif'])

  #refine the WM
  run.command(f'mrcalc sGM_mcrib.mif {"0.5" if use_hard_segmentation else "0.1"} -lt Pmap-WM.mif -mult Pmap-WM-corr.mif')
  ...
  #impose CSF of dHCP over sGM, every voxel with CSF > 0.5 will be removed from the sGM,
  # to avoid issues later on at the normalization
  ...
  t2w_file = (app.ARGS.input.glob('*_T2w_restore.nii.gz') + app.ARGS.input.glob('*restore_T2w.nii.gz'))[0]
  run.command(f'mrconvert result.mif {app.ARGS.output}',
              mrconvert_keyval=t2w_file,
