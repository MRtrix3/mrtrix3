...
def usage(base_parser, subcommands): #pylint: disable=unused-variable
  parser = subcommands.add_subcommand('msmt_5tt', parent=base_parser)
  ...
  parser.add_argument('out_csf',
                      ...
                      type=app.Parser.FileOut())
  options = parser.add_option_group('Options specific to the "msmt_5tt" algorithm')
  ...
  options.add_option('fa',
                     ...
                     default=0.2)
  options.add_option('pvf',
                     'Partial volume fraction threshold for tissue voxel selection',
                     type=app.Parser.Float(0.0, 1.0),
                     metavar='fraction',
                     default=0.95)
  options.add_option('wm_algo',
                     'dwi2response algorithm to use for WM single-fibre voxel selection',
                     choices=WM_ALGOS,
                     metavar='algorithm',
                     default='tournier')
  options.add_option('sfwm_fa_threshold',
                     'Sets -wm_algo to fa and allows to specify a hard FA threshold for single-fibre WM voxels, '
