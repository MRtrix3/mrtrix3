...
const OptionGroup OutputOptions = OptionGroup ("Metric values for output fixel data files")
  ...
  + Option ("skew",
            "output a measure of FOD lobe skew as the angle between peak and weighted mean directions")
    + Argument ("image").type_image_out();
...
void usage() {
  ...
  + Argument ("fixel_directory", "the output fixel directory").type_directory_out(DirOutMode::EmptyOrAbsent);
...
void run() {
  ...
  opt = get_options("skew");
  if (!opt.empty())
    receiver.set_skew_output(opt[0][0]);
