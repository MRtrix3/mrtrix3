...
void run() {
  ...
  try {
    ...
    try {
      ...
      Exception e(fmt::format("Could not locate fixel data based on input string \"{}\"", std::string(argument[0])));
  ...
  if (!opt.empty()) {
    ...
    INFO(fmt::format("Maximum number of fixels in any given voxel: {}", max_fixel_count));
  }
