...
void run() {
  ...
  for (auto &in_data_header : in_headers) {
    in_data_images.push_back(in_data_header.get_image<float>(DirectIO{}));
    ...
    out_data_images.push_back(Image<float>::create(
        Path::join(out_fixel_directory, Path::basename(in_data_header.name())), out_data_header, DirectIO{}));
  }
