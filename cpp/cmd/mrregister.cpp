...
void run() {
  ...
  for (size_t i = 0; i < n_images; i++) {
    ...
    if (i > 0)
      check_dimensions(input1[i], input1[i - 1], 0, 3);
    if (i > 0)
    ...
    if (nvols1 != nvols2)
      throw Exception(fmt::format("input images do not have the same number of volumes: {} and {}",
                                  str(input2[i].name()),
                                  str(input1[i].name())));
  ...
  INFO(fmt::format("maximum input lmax: {}", str(max_mc_image_lmax)));
...

  {
    ...
    if (max_mc_image_lmax != 0) {
      if (do_rigid)
        max_requested_lmax = std::max(max_requested_lmax, rigid_registration.get_lmax());
      if (do_affine)
      ...
      INFO(fmt::format("maximum used lmax: {}", str(max_requested_lmax)));
    }
  ...
  if (mc_params.size() > 1) {
    if (do_rigid)
      rigid_registration.set_mc_parameters(mc_params);
    if (do_affine)
    ...
    if (images2.ndim() == 4) {
      ...
      if (rigid_metric == Registration::Diff) {
        if (rigid_estimator == Registration::None) {
          ...
          throw Exception("FIXME: estimator selection");
      } else
    ...
    if (!warp2_filename.empty()) {
      ...
      Registration::Warp::compute_full_deformation(nl_registration.get_im1_to_mid_linear().inverse(),
                                                   ...
                                                   *(nl_registration.get_im2_to_mid()),
                                                   nl_registration.get_im2_to_mid_linear(),
                                                   warp2);
    }
  }
  ...
  if (!input1_midway_transformed_paths.empty() and !input2_midway_transformed_paths.empty()) {
    ...
    if (do_nonlinear)
      ...
      midway_header = compute_minimum_average_header(
          input1[0], input2[0], affine.get_transform_half_inverse(), affine.get_transform_half());
    else // rigid
      midway_header = compute_minimum_average_header(
          input1[0], input2[0], rigid.get_transform_half_inverse(), rigid.get_transform_half());
    ...
    if (do_nonlinear) {
      ...
      Registration::Warp::compose_linear_deformation(
          nl_registration.get_im2_to_mid_linear(), *(nl_registration.get_im2_to_mid()), im2_deform_field);
    }
