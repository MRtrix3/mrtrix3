...
void run() {
  ...
  for (auto i =
    ...
    index_type const nfixels_template = template_index.value();
    template_index.index(3) = 1;
    index_type const template_offset = template_index.value();
    ...
    for (index_type t = 0; t < nfixels_template; ++t) {
      ...
      index_type const nfixels_subject = subject_index.value();
      subject_index.index(3) = 1;
      index_type const subject_offset = subject_index.value();
      ...
      for (index_type s = 0; s < nfixels_subject; ++s) {
        ...
        float const dp = std::fabs(templatedir.dot(subjectdir));
        if (dp > largest_dp) {
          largest_dp = dp;
