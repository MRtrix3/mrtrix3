...
std::vector<size_t> optimise(const Eigen::MatrixXd &directions, const size_t skip, const size_t first_volume) {
...
void run() {
  ...
  if (size_t(directions.rows()) <= Math::SH::NforL(2)) {
    ...
    WARN("Very few directions in input (" + str(directions.rows()) + ";" +
         ...
         (skip ?
              " (direction #" + str(skip + 1) + " will be first direction in output "
  ...
  for (size_t first_volume = skip; first_volume != last_candidate_first_volume; ++first_volume) {
    const std::vector<size_t> order = optimise(directions, skip, first_volume);
