...
template <typename T, class StreamType> void write(Image<T> image, Image<bool> &mask, StreamType &out) {
  if (!mask.valid()) {
    write(image, out);
    return;
  }
...
template <class StreamType> void write(Header &header, Image<bool> &mask, StreamType &out) {
  ...
  case DataType::UInt64:
    write(header.get_image<uint64_t>(), mask, out);
    break;
  case DataType::Int64:
    write(header.get_image<int64_t>(), mask, out);
    break;
