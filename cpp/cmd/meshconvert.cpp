...
void usage() {
  ...
  + Option ("transform", fmt::format("transform vertices from one coordinate space to another,"
                                    " based on a template image; options are: {}.", MR::Enum::join<TransformChoice>()))
...
void run() {
  ...
  MeshMulti meshes;
  try {
    MR::Surface::Mesh mesh(input_path);
  ...
  if (meshes.size() == 1)
    meshes[0].save(output_path, !get_options("binary").empty());
  else
    meshes.save(output_path);
}
