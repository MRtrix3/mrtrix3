...
inline bool check(Header &H, size_t num_axes) {
  ...
  case DataType::Float64:
    ...
    break;
  default:
    throw Exception("Unsupported data type for MGH format ({})", H.datatype().specifier());
  }
...
template <class Input> void read_other(Header &H, Input &in) {
  ...
  auto read_colourtable_V2 = [&](Input &in) {
    ...
    for (int32_t i = 0; i != num_entries_to_read; ++i) {
      ...
      if (structure < 0)
        throw Exception("Error reading colour table from file \"{}\":" //
...
template <class Output> void write_header(const Header &H, Output &out) {
  ...
  case DataType::Float32BE:
    ...
    break;
  default:
    throw Exception("Error in MGH file format header write:" //
                    " invalid datatype ({})",                //
...
template <class Output> void write_other(const Header &H, Output &out) {
  ...
  auto write_colourtable_V1 = [](std::string_view table, Output &out) {
    ...
    for (const auto &line : lines) {
      ...
      if (entries.size() != 5)
        throw Exception("Error writing colour table to file:" //
  ...
  for (auto entry : H.keyval()) {
    ...
    if (entry.first == "MGH_TR") {
      ...
      case tag_auto_align: {
        ...
        if (lines.size() != 4)
          throw Exception("Error parsing auto align header entry for MGH format:" //
                          " Invalid number of lines ({}; should be 4)",           //
                          lines.size());                                          //
        for (size_t row = 0; row != 4; ++row) {
          const auto entries = split(strip(lines[row]), ", ", true);
          if (entries.size() != 4)
            throw Exception("Error parsing auto align header entry for MGH format:"    //
                            " Invalid number of entries on line {} ({}; should be 4)", //
                            row,                                                       //
                            entries.size());                                           //
