...
template <typename ValueType, class Output> inline void store(ValueType value, Output &out) {
  value = ByteOrder::BE(value);
...
inline bool check(Header &H, size_t num_axes) {
  if (num_axes < 3)
    throw Exception("cannot create MGH image with less than 3 dimensions");
  if (num_axes > 4)
    throw Exception("cannot create MGH image with more than 4 dimensions");
  H.ndim() = num_axes;
  ...
  case DataType::Int16:
    H.datatype() = DataType::Int16BE;
    break;
  case DataType::UInt32:
  case DataType::Int32:
  case DataType::UInt64:
  case DataType::Int64:
    H.datatype() = DataType::Int32BE;
    break;
  ...
  case DataType::Float64:
    ...
    break;
  default:
    throw Exception(fmt::format("Unsupported data type for MGH format ({})", std::string(H.datatype().specifier())));
  }
...
typedef struct {
  ...
  // Gradient values
  int64_t D1_ramp, D1_flat;
  float64 D1_amp;
  int64_t D2_ramp, D2_flat;
  float64 D2_amp;
  int64_t D3_ramp, D3_flat;
  float64 D3_amp;
...
template <class Input> void read_header(Header &H, Input &in) {
  ...
  if (version != 1)
    throw Exception(fmt::format("image \"{}\" is not in MGH format (version != 1)", H.name()));
  ...
  case datatype_float:
    ...
    break;
  default:
    throw Exception(fmt::format("unknown data type for MGH image \"{}\" ({})", H.name(), type));
  }
  H.datatype() = dtype;
...
template <class Input> void read_other(Header &H, Input &in) {
  ...
  auto read_colourtable_V1 = [&](Input &in, const int32_t nentries) {
    if (!nentries)
      throw Exception(fmt::format("Error reading colour table from file \"{}\": No entries", H.name()));
    std::ostringstream table;
  ...
  try {
    ...
    do {
      ...
      if (id == tag_old_mgh_xform)
        ...
        throw Exception(fmt::format("Invalid tag ({}) in MGH format \"other data\"", id));
      else
      ...
      case tag_old_colortable: {
        ...
        if (version > 0) {
          ...
          throw Exception(
              fmt::format("Error reading colour table from file \"{}\": Unknown version ({})", H.name(), version));
        }
      } break;
...
template <class Output> void write_header(const Header &H, Output &out) {
  ...
  case DataType::Float32BE:
    ...
    break;
  default:
    throw Exception(fmt::format("Error in MGH file format header write: invalid datatype ({})",
...
template <class Output> void write_other(const Header &H, Output &out) {
  ...
  auto write_mri_frames = [&](std::string_view table, Output &out) {
    ...
    if (lines.size() != nframes) {
      WARN(fmt::format("Error writing MRI frame data to output image"
  ...
  auto write_colourtable_V1 = [](std::string_view table, Output &out) {
    ...
    for (const auto &line : lines) {
      ...
      if (entries.size() != 5)
        throw Exception(
            fmt::format("Error writing colour table to file: Line has {} fields, expected 5", entries.size()));
  ...
  auto write_colourtable_V2 = [](std::string_view table, Output &out) {
    ...
    for (auto line : lines) {
      ...
      if (entries.size() != 6)
        throw Exception(
            fmt::format("Error writing colour table to file: Line has {} fields, expected 6", entries.size()));
    ...
    if (entry.first == "MGH_TR") {
      ...
      case tag_auto_align: {
        ...
        for (size_t row = 0; row != 4; ++row) {
          ...
          if (entries.size() != 4)
            throw Exception(fmt::format("Error parsing auto align header entry for MGH format: Invalid number of "
  ...
  if (!colour_table.empty()) {
    ...
    switch (entries.size()) {
    case 5:
      write_colourtable_V1(colour_table, out);
      break;
    case 6:
      write_colourtable_V2(colour_table, out);
      break;
    default:
      WARN("Malformed colour table in header (incorrect number of columns); not written to output image");
      break;
    }
  }
