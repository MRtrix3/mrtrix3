...
std::ostream &operator<<(std::ostream &stream, const Frame &item) {
  stream << (item.instance == UINT_MAX ? 0 : item.instance) << "#" << (item.acq == UINT_MAX ? 0 : item.acq) << ":"
...
std::vector<size_t> Frame::count(const std::vector<Frame *> &frames) {
  ...
  for (auto frame_it = frames.cbegin() + 1; frame_it != frames.cend(); ++frame_it) {
    ...
    if ((!frame.ignore_series_num && frame.series_num != previous->series_num) || frame.acq != previous->acq ||
        frame.distance < previous->distance)
...
default_type Frame::get_slice_separation(const std::vector<Frame *> &frames, size_t nslices) {
  ...
  if (nslices < 2)
    return std::isfinite(frames[0]->slice_spacing) ? frames[0]->slice_spacing : frames[0]->slice_thickness;
...
Eigen::MatrixXd Frame::get_PE_scheme(const std::vector<Frame *> &frames, const size_t nslices) {
  ...
  for (size_t n = 0; n != num_volumes; ++n) {
    ...
    if (std::isfinite(frame.bandwidth_per_pixel_phase_encode)) {
      const default_type effective_echo_spacing =
          1.0 / (frame.bandwidth_per_pixel_phase_encode * frame.acq_dim[frame.pe_axis]);
