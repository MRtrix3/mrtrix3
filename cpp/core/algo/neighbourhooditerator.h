...
public:
  ...
  const [[nodiscard]] ssize_t &index(size_t axis) const { return pos[axis]; }
  ssize_t &index(size_t axis) { return pos[axis]; }

  const [[nodiscard]] Eigen::Matrix<ssize_t, 1, Eigen::Dynamic> get_pos() const { return pos; }

  const [[nodiscard]] ssize_t &extent(size_t axis) const { return dim[axis]; }
  const [[nodiscard]] ssize_t &centre(size_t axis) const { return offset[axis]; }
