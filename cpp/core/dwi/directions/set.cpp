...
void Set::load_predefined(Eigen::MatrixXd &az_in_pairs, const size_t i) {
  ...
  case 5000:
    ...
    return;
  default:
    throw Exception(fmt::format("No pre-defined data set of {} directions", str(i)));
  }
}
...
void Set::initialise_adjacency() {
  ...
  public:
    Vertex(const Set &set, const index_type index, const bool inverse)
        : dir(set[index] * (inverse ? -1.0 : 1.0)), index(index) {}
  ...
  for (index_type i = 0; i != 6; ++i) {
    if (i != distant_pair.first && i != distant_pair.second) {
      const default_type dist =
          (vertices[all_extrema[i]].dir - (vertices[all_extrema[distant_pair.first]].dir))
              .cross(vertices[all_extrema[i]].dir - vertices[all_extrema[distant_pair.second]].dir)
              .norm() /
          (vertices[all_extrema[distant_pair.second]].dir - vertices[all_extrema[distant_pair.first]].dir).norm();
      if (dist > max_dist) {
        max_dist = dist;
        third_point = i;
      }
    ...
    if (max_index == vertices.size()) {
      ...
      for (auto &p : all_planes) {
        for (size_t edge_index = 0; edge_index != 3; ++edge_index) {
          ...
          switch (edge_index) {
          case 0:
            edge = std::make_pair(p->indices[0], p->indices[1]);
            break;
          case 1:
            edge = std::make_pair(p->indices[1], p->indices[2]);
            break;
          case 2:
  ...
  for (auto &current : hull) {
    ...
    for (size_t edge = 0; edge != 6; ++edge) {
      ...
      case 0:
        ...
        to = vertices[current.indices[1]].index;
        break;
      case 1:
        from = vertices[current.indices[1]].index;
        to = vertices[current.indices[0]].index;
        break;
      case 2:
        from = vertices[current.indices[1]].index;
        to = vertices[current.indices[2]].index;
        break;
      case 3:
