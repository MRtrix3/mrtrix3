...
const App::Option EdgeStatisticOption
    ...
    + App::Argument("statistic").type_choice<stat_edge>().set_default("sum");
// clang-format on
...
template <typename T> void Matrix<T>::finalize() {
  switch (statistic) {
  case stat_edge::SUM:
    return;
  case stat_edge::MEAN:
...
template <typename T> void Matrix<T>::apply_data(const size_t index, const T value, const T weight) {
  T &target = data[index];
...
template <typename T> void Matrix<T>::apply_data(T &target, const T value, const T weight) {
  switch (statistic) {
  case stat_edge::SUM:
  case stat_edge::MEAN:
    target += value * weight;
...
template <typename T> void Matrix<T>::inc_count(const size_t index, const T weight) {
...
template <typename T> void Matrix<T>::inc_count(const size_t node_one, const size_t node_two, const T weight) {
  if (statistic != stat_edge::MEAN)
    return;
  assert(counts.size());
  assert(mat2vec);
