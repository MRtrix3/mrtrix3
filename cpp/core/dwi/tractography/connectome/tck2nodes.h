...
protected:
  ...
  public:
    ...
    bool operator<(const Eigen::Array<int, 3, 1> &that) const {
      return ((*this)[2] == that[2]) ? (((*this)[1] == that[1]) ? ((*this)[0] < that[0]) : ((*this)[1] < that[1]))
                                     : ((*this)[2] < that[2]);
    }
  };
};
...
public:
  Tck2nodes_radial(const Image<node_t> &nodes_data, const default_type radius)
      : Tck2nodes_base(nodes_data, true),
        max_dist(radius),
        max_add_dist(std::sqrt(Math::pow2(0.5 * nodes.spacing(2)) + Math::pow2(0.5 * nodes.spacing(1)) +
                               Math::pow2(0.5 * nodes.spacing(0)))) {
    initialise_search();
  }
