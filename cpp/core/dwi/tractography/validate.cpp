...
TckValidation validate_tck(const std::filesystem::path &path) {
  ...
  while (true) {
    ...
    if (seen_eof_barrier)
      throw Exception("Tractogram \"{}\": vertex data present after the end-of-file barrier triplet", //
                      path);                                                                          //
    ...
    if (p.allFinite()) {
      ...
      if (current_vertices > 0)
        throw Exception("Tractogram \"{}\": {}{} in the last streamline,"                         //
      ...
      // ---------------------------------------------------------------
      throw Exception("Tractogram \"{}\": invalid partially-finite triplet detected;" //
  ...
  if (result.header_count.has_value() && result.n_streamlines != *result.header_count)
    throw Exception("Tractogram \"{}\":"                                                     //
                    " header count ({}) does not match the number of streamlines read ({})", //
                    path,                                                                    //
                    *result.header_count,                                                    //
                    result.n_streamlines);                                                   //
...
void validate_tsf_properties(const Properties &a, const Properties &b, std::string_view file_types) {
  ...
  if (stamp_a == a.end() || stamp_b == b.end())
    throw Exception("Unable to verify {}: missing timestamp", file_types);
  if (stamp_a->second != stamp_b->second) {
    throw Exception("Invalid {}: timestamps do not match,"             //
  ...
  if ((count_a == a.end()) || (count_b == b.end()))
    throw Exception("Unable to validate {}: missing count field", file_types);
  if (to<size_t>(count_a->second) != to<size_t>(count_b->second))
    throw Exception("{} does not contain same number of streamlines", file_types);
}
...
void validate_tsf(const std::filesystem::path &tsf_path, const std::filesystem::path &tck_path) {
  ...
  if (tsf_count != header_count)
    throw Exception("Track scalar file \"{}\":"                                                       //
                    " header \"count\" ({}) does not match the number of scalar sequences read ({})", //
                    tsf_path,                                                                         //
                    header_count,                                                                     //
                    tsf_count);                                                                       //
  ...
  if (tsf_count != tck_count)
    throw Exception("Track scalar file \"{}\" contains {} scalar sequence(s)," //
                    " but tractogram \"{}\" contains {} streamline(s)",        //
                    tsf_path,                                                  //
                    tsf_count,                                                 //
                    tck_path,                                                  //
                    tck_count);                                                //
  ...
  if (n_length_mismatch > 0)
    throw Exception("Track scalar file \"{}\": {} of {} streamlines have a different number of vertices" //
