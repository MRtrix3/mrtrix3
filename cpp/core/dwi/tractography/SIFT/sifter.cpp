...
void SIFTer::perform_filtering() {
  ...
  try {
    gradient_vector.assign(
        num_tracks(),
        Cost_fn_gradient_sort(num_tracks(), std::numeric_limits<double>::max(), std::numeric_limits<double>::max()));
  } catch (...) {
  ...
  if (tracks_remaining < term_number)
    throw Exception("Filtering failed; desired number of filtered streamlines is greater than or equal to the size of "
                    "the input dataset");
  ...
  do {
    ...
    do {
      ...
      if (!output_at_counts.empty() && (tracks_remaining == output_at_counts.back())) {
        ...
        INFO(fmt::format("\\nProportionality coefficient at {} streamlines is {}", str(tracks_remaining), str(mu())));
      ...
      if (sum_noncontributing_length && ((contributing_length_removed / sum_contributing_length) >
        ...
        const double streamline_density_ratio =
            candidate->get_cost_gradient() / (sum_contributing_length - contributing_length_removed);
        ...
        if (this_actual_cf_change <
            std::min({required_cf_change_ratio, required_cf_change_quantisation, this_nonlinearity})) {
          ...
          if (!removed_this_iteration) {
            ...
            if (enforce_quantisation && (term_number || term_ratio || term_mu)) {
              ...
              CONSOLE("filtering has reached quantisation error, but user's desired termination criterion has not yet "
                      "been met;");
              CONSOLE("  disabling cost function quantisation check and continuing filtering to try to satisfy user's "
                      "request");
  ...
  end_iteration:
    ...
    if (!csv_path.empty()) {
      ...
      case NONLINEARITY:
        csv_out << "Non-linearity";
        break;
      case QUANTISATION:
        csv_out << "Quantisation";
        break;
      case TERM_COUNT:
        csv_out << "Target streamline count";
        break;
      case TERM_RATIO:
        csv_out << "Termination ratio";
        break;
      case TERM_MU:
        csv_out << "Target proportionality coefficient";
        break;
      case POS_GRADIENT:
        csv_out << "Positive gradient";
        break;
      }
      csv_out << ",\n";
    }
  ...
  case UNDEFINED:
    throw Exception("Encountered undefined recalculation at end of iteration!");
  case NONLINEARITY:
    INFO("Filtering terminated due to instability in cost function gradients");
    break;
  case QUANTISATION:
    INFO("Filtering terminated due to candidate streamline failing to exceed quantisation");
    break;
  case TERM_COUNT:
    INFO("Filtering terminated due to reaching desired streamline count");
    break;
  case TERM_RATIO:
    INFO("Filtering terminated due to cost function / streamline density decrease ratio");
    break;
  case TERM_MU:
    INFO("Filtering terminated due to reaching desired proportionality coefficient");
    break;
  case POS_GRADIENT:
    INFO("Filtering terminated due to candidate streamline having positive gradient");
    break;
  }
  ...
  INFO(fmt::format("Proportionality coefficient at end of filtering is {}", str(mu())));
}
  ...
  for (std::vector<size_t>::const_iterator i = block_sizes.begin(); i != block_sizes.end(); ++i) {
    ...
    try {
      ...
      for (size_t candidate_count = 0; candidate_count < num_tracks / 1000; ++candidate_count)
        sorter.get();
      std::cerr << "Time required for sorting " << num_tracks << " tracks, block size " << block_size << " = "
                << timer.elapsed() * 1000.0 << "ms\n";
    } catch (...) {
...
double SIFTer::calc_gradient(const track_t index, const double current_mu, const double current_roc_cost) const {
  ...
  for (size_t f = 0; f != tck_cont.dim(); ++f) {
    ...
    const double gradient_remove_tck =
        fixel.get_cost_wo_track(mu_if_removed, tck_cont[f].get_length()) - fixel.get_cost(current_mu);
