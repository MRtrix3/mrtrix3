...
const OptionGroup SIFTModelOption =
    ...
    + Option("remove_untracked",
             ...
             " (and you can no longer trivially do quantitative comparisons"
             " between reconstructions if this is enabled)")
    + Option("fd_thresh",
             "fibre density threshold;"
             " exclude an FOD lobe from filtering processing if its integral is less than this amount"
             " (streamlines will still be mapped to it,"
             " but it will not contribute to the cost function or the filtering)")
...
const OptionGroup SIFTTermOption =
    ...
    + Option("term_number",
             ...
             " continue filtering until this number of streamlines remain")
      + Argument("value").type_integer(1)
    + Option("term_ratio",
             "termination ratio;"
             " defined as the ratio between reduction in cost function,"
             " and reduction in density of streamlines."
