...
size_t determine_upsample_ratio(const Header &header, const std::filesystem::path &tck_path, const float ratio) {
  ...
  const Reader<> reader(tck_path, properties);
...
void generate_header(Header &header,
                     const std::filesystem::path &tck_file_path,
...
void oversample_header(Header &header, const std::vector<default_type> &voxel_size) {
  ...
  header.transform().translation() +=
      header.transform().rotation() * Eigen::Vector3d(0.5 * (voxel_size[0] - header.spacing(0)),
