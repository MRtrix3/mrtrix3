...
public:
  ...
  template <class InterpolatorType> FORCE_INLINE bool get_data(InterpolatorType &source) {
...
protected:
  ...
  FORCE_INLINE Eigen::Vector3f
  random_direction(const Eigen::Vector3f &d, const float max_angle, const float sin_max_angle) {
