...
const OptionGroup SeedMechanismOption =
    ...
    + Option("seed_gmwmi",
             ...
             " Input image should be a 3D seeding volume;"
             " seeds drawn within this image will be optimised to the interface"
             " using the 5TT image provided using the -act option.").allow_multiple()
      + Argument("image").type_image_in()
    + Option("seed_dynamic",
    ...
    + Option("seed_coordinates_fixed",
             "provide coordinates in a space-separated Mx3 matrix (XYZ per row) and seed a fixed number"
             " of streamlines per coordinate").allow_multiple()
      + Argument("cds_path").type_file_in()
      + Argument("num_per_coordinate").type_integer(1)

    + Option ("seed_coordinates_global",
              "provide coordinates in a space-separated Mx3 matrix (XYZ per row) and seed until global seed/streamline "
...
const OptionGroup SeedParameterOption =
    ...
    + Option("seeds",
             ...
             " the default number of seeds is set to 1000 x the number of selected streamlines."
             " If -select is NOT also specified,"
             " tckgen will continue tracking until this number of seeds has been attempted."
             " However, if -select is also specified,"
             " tckgen will stop when the number of seeds attempted reaches the number specified here,"
             " OR when the number of streamlines selected reaches the number requested with the -select option."
             " This can be used to prevent the program from running indefinitely "
             " when no or very few streamlines can be found that match the selection criteria"
             " Setting this to zero will cause tckgen to keep attempting seeds"
    ...
    + Option("max_attempts_per_seed",
             ...
             " This should be set high enough to ensure that an actual plausible seed point"
