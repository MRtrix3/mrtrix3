  ...
  for (size_t i = 0; i < in.size(); ++i) {
    ...
    if (i) {
      ...
      if (prev_s == state_t::AFTER_START && s == state_t::BEFORE_START)
        a = i;
      if (prev_s == state_t::BEFORE_END && s == state_t::AFTER_END)
        b = i;
      if (prev_s == state_t::AFTER_END && s == state_t::BEFORE_END)
        b = i - 1;
...
Arc::state_t Arc::state(const point_type &p) const {
  ...
  if (!after_start && !after_mid)
    return state_t::BEFORE_START;
  if (after_start && !after_mid)
    return state_t::AFTER_START;
  if (after_mid && !after_end)
    return state_t::BEFORE_END;
