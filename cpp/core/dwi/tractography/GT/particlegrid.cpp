...
ParticleGrid::ParticleGrid(const Header &H) {
  ...
  const Eigen::DiagonalMatrix<default_type, 3> newspacing(grid_spacing, grid_spacing, grid_spacing);
  const transform_type T_g2s = H.transform() * newspacing;
  T_s2g = T_g2s.inverse();
}

void ParticleGrid::add(const Point_t &pos, const Point_t &dir) {
  Particle *p = pool.create(pos, dir);
  const size_t gidx = pos2idx(pos);
  const std::lock_guard<std::mutex> lock(mutex);
  grid[gidx].push_back(p);
}

void ParticleGrid::shift(Particle *p, const Point_t &pos, const Point_t &dir) {
  const size_t gidx0 = pos2idx(p->getPosition());
  const size_t gidx1 = pos2idx(pos);
  const std::lock_guard<std::mutex> lock(mutex);
  grid[gidx0].remove(p);
  p->setPosition(pos);
  p->setDirection(dir);
...
void ParticleGrid::exportTracks(Tractography::Writer<float> &writer) {
  ...
  for (const ParticleContainer &gridvox : grid) {
