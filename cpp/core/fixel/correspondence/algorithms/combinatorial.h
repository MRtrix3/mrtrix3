...
public:
  Combinatorial(const size_t max_origins_per_target, const size_t max_objectives_per_source, const Header &H_cost)
    ...
    do {
                      ...
                      for (int32_t it = t.size() - 1; !skip && it >= 0; --it) {
                        ...
                        for (const auto& is : remapping_origins[src_fixel_list_index]) {
                          if (++objectives_per_source_fixel[is] > max_objectives_per_source) {
                            increment_index_for_skip = static_cast<uint32_t>(it);
                            skip = true;
                            break;
                          }
                        }
      ...
      */
      ...
      for (uint32_t is = 0; is != s.size(); ++is) {
        // TODO May be able to remove variable "objectives_per_source_fixel" now that the entire inverse mapping is
        // constructed
        if (static_cast<uint32_t>((objectives_per_source_fixel[is] = inv_mapping[is].size())) >
            max_objectives_per_source) {
          skip = true;
      ...
      if (skip) {
        ...
        cost = static_cast<const CostFunctor *const>(this)->calculate(
            s, rs, t, objectives_per_source_fixel, origins_per_remapped_fixel);
