...
public:
  ...
  bool operator()(const std::vector<index_type> &indices) const {
    ...
    for (const auto i : indices) {
      ...
      //   if *none* of those adjacent directions are also part of set "indices" (i.e. iterator != end, indicating a
      //   hit), then this fixel is disconnected from the rest of the fixel set in "indices", and so should be rejected
      //   as a candidate
      if (std::none_of(list.begin(), list.end(), [&](const DWI::Directions::index_type j) {
