...
class DP2Cost {
public:
  DP2Cost() : data(dp2cost_lookup_resolution + 2), multiplier(dp2cost_lookup_resolution) {
    for (size_t bin = 0; bin <= dp2cost_lookup_resolution; ++bin) {
      const default_type dp = static_cast<default_type>(bin) / static_cast<default_type>(dp2cost_lookup_resolution);
  ...
  float operator()(const float dp) const {
    ...
    const float mu = position - static_cast<float>(lower);
