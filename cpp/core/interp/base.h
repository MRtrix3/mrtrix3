...
public:
  ...
  static inline value_type default_out_of_bounds_value() { return std::numeric_limits<value_type>::quiet_NaN(); }
...
protected:
  ...
  template <class VectorType> bool set_out_of_bounds(const VectorType &pos) {
    return ((out_of_bounds = (pos[0] <= -0.5 || pos[0] >= bounds[0] || pos[1] <= -0.5 || pos[1] >= bounds[1] ||
                              pos[2] <= -0.5 || pos[2] >= bounds[2])));
  }

  void set_out_of_bounds(const bool value) { out_of_bounds = value; }
