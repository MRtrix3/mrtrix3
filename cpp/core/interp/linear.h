...
template <class C> struct value_type_of {
  using type = C;
};

template <class X> struct value_type_of<std::complex<X>> {
  using type = X;
};
...
template <class ImageType, LinearInterpProcessingType PType> class LinearInterpBase : public Base<ImageType> {
public:
  using typename Base<ImageType>::value_type;
  using coef_type = typename value_type_of<value_type>::type;

  LinearInterpBase(const ImageType &parent,
                   value_type value_when_out_of_bounds = Base<ImageType>::default_out_of_bounds_value())
      : Base<ImageType>(parent, value_when_out_of_bounds), zero(0.0), eps(1.0e-6) {}
...
public:
  ...
  LinearInterp(const ImageType &parent,
               value_type value_when_out_of_bounds = Base<ImageType>::default_out_of_bounds_value())
...
public:
  ...
  //! Returns the image gradient at the current position, defined with respect to the scanner coordinate frame of
  //! reference.
  ...
  //! Collectively interpolates gradients along axis 3, defined with respect to the scanner coordinate frame of
  //! reference.
...
public:
  ...
  LinearInterp(const ImageType &parent,
               coef_type value_when_out_of_bounds = Base<ImageType>::default_out_of_bounds_value())
  ...
  void value_and_gradient_row(Eigen::Matrix<value_type, Eigen::Dynamic, 1> &value,
                              Eigen::Matrix<value_type, Eigen::Dynamic, 3> &gradient) {
    if (Base<ImageType>::out_of_bounds) {
  ...
  void value_and_gradient_row_wrt_scanner(Eigen::Matrix<value_type, Eigen::Dynamic, 1> &value,
                                          Eigen::Matrix<value_type, Eigen::Dynamic, 3> &gradient) {
    if (Base<ImageType>::out_of_bounds) {
      value = out_of_bounds_vec;
      gradient = out_of_bounds_matrix;
...
template <typename ImageType> using Linear = LinearInterp<ImageType, LinearInterpProcessingType::Value>;
