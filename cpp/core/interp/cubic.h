...
public:
  ...
  SplineInterpBase(const ImageType &parent,
  ...
  template <class VectorType> bool voxel(const VectorType &pos) {
    ...
    for (ssize_t z = 0; z < 4; ++z) {
      for (ssize_t y = 0; y < 4; ++y) {
        value_type const partial_weight = H[1].weights[y] * H[2].weights[z];
...
public:
  ...
  template <class VectorType> bool voxel(const VectorType &pos) {
    ...
    for (ssize_t z = 0; z < 4; ++z) {
      for (ssize_t y = 0; y < 4; ++y) {
        value_type const partial_weight = H[1].weights[y] * H[2].weights[z];
        value_type const partial_weight_dy = H[1].deriv_weights[y] * H[2].weights[z];
        value_type const partial_weight_dz = H[1].weights[y] * H[2].deriv_weights[z];
...
using Cubic =
    SplineInterp<ImageType, Math::HermiteSpline<typename ImageType::value_type>, Math::SplineProcessingType::Value>;

template <typename ImageType>
using CubicUniform =
    SplineInterp<ImageType, Math::UniformBSpline<typename ImageType::value_type>, Math::SplineProcessingType::Value>;
