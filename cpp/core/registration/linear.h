...
public:
  ...
  void set_init_rotation_type(Transform::Init::InitType type) { init_rotation_type = type; }

  void use_robust_estimate(bool use) { robust_estimate = use; }

  void set_grad_tolerance(const float &tolerance) { grad_tolerance = tolerance; }

  void set_log_stream(std::streambuf *stream) { log_stream = stream; }
  ...
  void run(MetricType &metric, TransformType &transform, Im1ImageType &im1_image, Im2ImageType &im2_image) {
  ...
  void run_im2_mask(MetricType &metric,
  ...
  void run_im1_mask(MetricType &metric,
                    TransformType &transform,
                    Im1ImageType &im1_image,
                    Im2ImageType &im2_image,
  ...
  template <class MetricType,
            ...
            class Im1MaskType,
            class Im2MaskType>
  void run_masked(MetricType &metric,
                  TransformType &transform,
                  Im1ImageType &im1_image,
                  Im2ImageType &im2_image,
    ...
    if (init_translation_type == Transform::Init::mass)
      Transform::Init::initialise_using_image_mass(
          im1_image, im2_image, im1_mask, im2_mask, transform, init, contrasts);
    ...
    if (init_rotation_type == Transform::Init::moments)
      Transform::Init::initialise_using_image_moments(
          im1_image, im2_image, im1_mask, im2_mask, transform, init, contrasts);
    ...
    // using Im1ImageInterpolatorType = Interp::SplineInterp<Im1ImageType, Math::UniformBSpline<typename
    // Im1ImageType::value_type>, Math::SplineProcessingType::ValueAndDerivative>; using Im2ImageInterpolatorType =
    // Interp::SplineInterp<Im2ImageType, Math::UniformBSpline<typename Im2ImageType::value_type>,
    // Math::SplineProcessingType::ValueAndDerivative>; using ProcessedImageInterpolatorType =
    // Interp::SplineInterp<ProcessedImageType, Math::UniformBSpline<typename ProcessedImageType::value_type>,
    // Math::SplineProcessingType::ValueAndDerivative>;
#else
    using Im1ImageInterpolatorType =
        Interp::LinearInterp<Im1ImageType, Interp::LinearInterpProcessingType::ValueAndDerivative>;
    using Im2ImageInterpolatorType =
        Interp::LinearInterp<Im2ImageType, Interp::LinearInterpProcessingType::ValueAndDerivative>;
    using ProcessedImageInterpolatorType =
    ...
    midway_image_header = compute_minimum_average_header(
        im1_image, im2_image, transform.get_transform_half_inverse(), transform.get_transform_half());
    ...
    for (size_t istage = 0; istage < stages.size(); istage++) {
      ...
      auto im1_smoothed =
          Registration::multi_resolution_lmax(im1_image, stage.scale_factor, do_reorientation, stage_contrasts);
      INFO("smoothing image 2");
      auto im2_smoothed = Registration::multi_resolution_lmax(
          im2_image, stage.scale_factor, do_reorientation, stage_contrasts, &stage_contrasts);
      ...
      DEBUG(fmt::format("neighbourhood kernel extent: {}", kernel_extent));
      ...
      DEBUG(fmt::format("coherence length: {}", coherence));
      DEBUG(fmt::format("stop length:      {}", stop));
      ...
      DEBUG(fmt::format("convergence slope threshold: {}", slope_threshold[0]));
      ...
      midway_image_header = compute_minimum_average_header(im1_image,
                                                           im2_image,
  ...
  //       Transform::reorient ("reorienting...", image_midway, image_midway, transformation.get_transform_half(),
  //       aPSF_directions);
  //   } else {
  //     Filter::reslice<Interp::Cubic> (input, image_midway, transformation.get_transform_half_inverse(),
  //     Adapter::AutoOverSample, 0.0); if (do_reorientation)
  //  //     60 directions by default!
  //       Transform::reorient ("reorienting...", image_midway, image_midway,
  //       transformation.get_transform_half_inverse(), aPSF_directions);
  //   }
  // }
