...
template <typename ImageType1, typename ImageType2> struct LCCPrecomputeFunctorMasked_Naive {
  template <typename MaskType, typename ImageType3> void operator()(MaskType &mask, ImageType3 &out) {
    ...
    const int nmax = extent[0] * extent[1] * extent[2];
    ...
    in1.index(1) = pos[1];
    in1.index(2) = pos[2];
    const value_type value_in1 = in1.value();
    ...
    in2.index(1) = pos[1];
    in2.index(2) = pos[2];
    const value_type value_in2 = in2.value();
    ...
    while (niter.loop()) {
      ...
      const value_type val1 = in1.value();
      if (val1 != val1) {
        continue;
      }
      ...
      const value_type val2 = in2.value();
      if (val2 != val2) {
        continue;
      }
    ...
    const default_type m1 = n1.sum() / cnt;
    const default_type m2 = n2.sum() / cnt;
    n1.array() -= m1;
    n2.array() -= m2;
...
public:
  ...
  /** requires_precompute int is_neighbourhood: type_trait to distinguish metric types that require a call to precompute
  ...
  template <class ParamType> default_type precompute(ParamType &parameters) {
    ...
    const std::vector<uint32_t> NoOversample;
    {
  ...
  default_type
  operator()(Params &params, const Iterator &iter, Eigen::Matrix<default_type, Eigen::Dynamic, 1> &gradient) {
    ...
    const default_type A = params.processed_image.value();
    params.processed_image.index(3) = 3;
    const default_type B = params.processed_image.value();
    params.processed_image.index(3) = 4;
    const default_type C = params.processed_image.value();
    const default_type A_BC = A / (B * C);
    ...
    // 2.0 * sfm / (sff * smm) * ((i2 - sfm / smm * i1 ) * im1_gradient.value() - (i1 - sfm / sff * i2 ) *
    // im2_gradient.value());

    // ITK:
    // derivWRTImage[dim] = 2.0 * sFixedMoving / (sFixedFixed_sMovingMoving) * (fixedI - sFixedMoving / sMovingMoving *
    // movingI) * movingImageGradient[dim];
