...
public:
  ...
  default_type operator()(Params &params,
    ...
    assert(params.im2_image.index(0) ==
           std::round((MR::Transform(params.midway_image).voxel2scanner.inverse() * midway_point)[0]));
    assert(params.im2_image.index(1) ==
           std::round((MR::Transform(params.midway_image).voxel2scanner.inverse() * midway_point)[1]));
    assert(params.im2_image.index(2) ==
           std::round((MR::Transform(params.midway_image).voxel2scanner.inverse() * midway_point)[2]));
...
template <class Im1Type, class Im2Type> class MeanSquared4D : public LinearBase {
public:
  template <class Params>
  default_type operator()(Params &params,
                          const Eigen::Vector3d &im1_point,
    ...
    const Eigen::Matrix<default_type, 4, 1> jacobian_vec =
        params.transformation.get_jacobian_vector_wrt_params(midway_point);
...
template <class Im1Type, class Im2Type> class MeanSquared4DNonSymmetric : public LinearBase {
public:
  template <class Params>
  default_type operator()(Params &params,
                          const Eigen::Vector3d &im1_point,
    ...
    const Eigen::Matrix<default_type, 4, 1> jacobian_vec =
        params.transformation.get_jacobian_vector_wrt_params(im2_point);
    diff_values = im1_values - im2_values;
...
template <class Im1Type, class Im2Type> class MeanSquaredVectorNoGradient4D : public LinearBase {
public:
  MeanSquaredVectorNoGradient4D() = delete;
  MeanSquaredVectorNoGradient4D(const Im1Type im1, const Im2Type im2) : volumes((im1.ndim() == 3) ? 1 : im1.size(3)) {
    assert(im1.ndim() == 4);
