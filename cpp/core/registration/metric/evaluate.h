...
template <class MetricType, typename U = void> struct metric_requires_precompute {
  using no = int;
};
...
template <class MetricType, typename U = void> struct metric_requires_initialisation {
  using no = int;
};
...
template <class MetricType, class ParamType> class Evaluate {
public:
  using TransformParamType = typename ParamType::TransformParamType;
  using value_type = default_type;

  template <class U = MetricType> Evaluate() = delete;
  ...
  Evaluate(const MetricType &metric_, ParamType &parameters, typename metric_requires_initialisation<U>::yes = 0)
      : metric(metric_), params(parameters), iteration(1) {
  ...
  template <class U = MetricType>
  default_type operator()(const Eigen::Matrix<default_type, Eigen::Dynamic, 1> &x,
    ...
    if (directions.cols()) {
...

      {
        if (params.mc_settings.size()) {
          ...
          Registration::Transform::reorient(params.im1_image,
                                            *im1_image_reoriented,
                                            params.transformation.get_transform_half(),
                                            directions,
                                            false,
                                            params.mc_settings);
          Registration::Transform::reorient(params.im2_image,
                                            *im2_image_reoriented,
                                            params.transformation.get_transform_half_inverse(),
                                            directions,
                                            false,
                                            params.mc_settings);
        } else {
          ...
          Registration::Transform::reorient(
              params.im1_image, *im1_image_reoriented, params.transformation.get_transform_half(), directions);
          Registration::Transform::reorient(
              params.im2_image, *im2_image_reoriented, params.transformation.get_transform_half_inverse(), directions);
        }
      }
  ...
  //       DEBUG ("stochastic gradient descent, density: {}", params.loop_density);
  //       Math::RNG rng;
  //       gradient.setZero();
  ...
  //       cost, gradient, rng, overlap_count); loop.run_outer (functor);
  //     }
  //     else {
  //       if (overlap_count)
  //         *overlap_count = 0;
  ...
  //         Adapter::Subset<Image<default_type>> midway_subset (params.midway_image,
  //         params.robust_estimate_subset_from, params.robust_estimate_subset_size); ThreadedLoop (midway_subset, 0,
  //         3).run (kernel);
  //       } else {
  ...
  size_t size() { return params.transformation.size(); }

  ssize_t overlap() { return overlap_count; }
  ...
  void set_directions(const Eigen::MatrixXd &dir) { directions = dir; }
