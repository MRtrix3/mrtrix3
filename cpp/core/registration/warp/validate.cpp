...
WarpFormat validate_header(const Header &H) {
  ...
  if (H.datatype().is_complex())
    throw Exception(
        fmt::format("Warp image \"{}\": expected a real data type (got {})", H.name(), H.datatype().description()));
  ...
  if (H.ndim() == 4) {
    if (H.size(3) != 3)
      throw Exception(fmt::format("Warp image \"{}\": a 4D warp image (displacement or deformation field) must have "
  ...
  if (H.ndim() == 5) {
    if (H.size(3) != 3)
      throw Exception(fmt::format("Warp image \"{}\": a 5D full warp image must have exactly 3 volumes in the 4th "
    ...
    if (H.keyval().find("linear1") == H.keyval().end() || //
      ...
      throw Exception(fmt::format("Warp image \"{}\": full warp image header must have both \"linear1\" and "
                                  "\"linear2\" linear transform fields in the associated metadata",
                                  H.name()));

    return WarpFormat::Full;
  }
  throw Exception(fmt::format("Warp image \"{}\": expected a 4D image (displacement or deformation field) or a 5D "
    ...
    if ((voxel_count(image) / 3) / sort_fills.begin()->first < minratio_finite_to_fill) {
      ...
      if (fill_counts.size() > 1) {
        ...
        if (sort_fills.begin()->first / second_fill->first < minratio_firstfill_to_secondfill)
          throw Exception(fmt::format(
              "Warp image \"{}\": Unable to unambiguously determine fill value due to presence of different voxels "
              "with different constant values; a single fill convention must be used throughout",
              image.name()));
      } else {
        DEBUG(fmt::format(
            "Warp image \"{}\": Fill value of {} chosen despite detection of multiple potential fill values",
            image.name(),
            str(*result.fill_value)));
      }
    } else {
      DEBUG(fmt::format("Warp image \"{}\": No explicit fill value chosen due to too few voxels ({} containing "
                        "candidate fill value {})",
                        image.name(),
                        str(sort_fills.begin()->first),
                        str(sort_fills.begin()->second)));
    }
  }

  if (finitemix_count > 0)
    throw Exception(fmt::format("Warp image \"{}\": {}{}{}{}",
                                image.name(), //
                                str(finitemix_count),
                                (finitemix_count > 1 ? " peaks contain" : "peak contains"),              //
...
template <typename ValueType> void debug_validate_image(Image<ValueType> image) {
  ...
  try {
    ...
    DEBUG(fmt::format("Warp image \"{}\": {} format", image.name(), fmt));
    DEBUG(fmt::format(
        "Warp image \"{}\": {}",
        image.name(),                                                                                               //
        (v.fill_value.has_value() ? fmt::format("fill value is {}", *v.fill_value) : "no fill value determined"))); //
  } catch (const Exception &e) {
    throw Exception(e, fmt::format("Warp image \"{}\" validation failed", image.name()));
  }
}
