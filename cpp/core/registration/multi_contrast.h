...
FORCE_INLINE void check_image_output(const std::filesystem::path &image_path, const Header &reference) {
  ...
  const std::vector<int> Pdim(parser.ndim());
...
struct MultiContrastSetting {
  ...
  MultiContrastSetting(size_t image_nvols,
                       bool do_reorientation = false,
                       ssize_t limit_lmax = std::numeric_limits<ssize_t>::max())
...
inline std::ostream &operator<<(std::ostream &o, const MultiContrastSetting &a) {
  o << "MultiContrast: [start:" << a.start << ", nvols:" << a.nvols << ", lmax:" << a.lmax
    << ", image_lmax:" << a.image_lmax << ", reorient:" << a.do_reorientation << ", weight:" << a.weight << "]";
  return o;
}
