...
bool AffineUpdate::operator()(Eigen::Matrix<default_type, Eigen::Dynamic, 1> &newx,
  ...
  if (control_points.size()) {
    ...
    while (true) {
      ...
      if ((X + Delta).determinant() <= 0.0) {
        step_size *= step_down_factor;
        continue;
      }
      Diff.noalias() = ((X + Delta) * P - X * P).cwiseAbs();
      if ((Diff.template block<3, 1>(0, 0) - coherence_distance).maxCoeff() > 0.0) {
        step_size *= step_down_factor;
        continue;
      }
      if ((Diff.template block<3, 1>(0, 1) - coherence_distance).maxCoeff() > 0.0) {
        step_size *= step_down_factor;
        continue;
      }
      if ((Diff.template block<3, 1>(0, 2) - coherence_distance).maxCoeff() > 0.0) {
        step_size *= step_down_factor;
        continue;
      }
      if ((Diff.template block<3, 1>(0, 3) - coherence_distance).maxCoeff() > 0.0) {
        step_size *= step_down_factor;
        continue;
      }

      if (projection_type == TransformProjectionType::affine) {
        DEBUG("symmetric affine update");
        A = X - Delta;
        A(3, 3) = 1.0;
      ...
      } else if (projection_type == TransformProjectionType::affine_nonsym ||
                 projection_type == TransformProjectionType::rigid_nonsym) {
        DEBUG("nonsymmetric affine update");
        newx = x - step_size * g;
        Registration::Transform::param_vec2mat(newx, Xnew);
        if (Xnew.determinant() <= 0.0) {
          step_size *= step_down_factor;
          continue;
        }
      } else {
        throw Exception("affine transformation update not implemented yet");
      }

      // project affine trafo to rigid trafo if requested
      if (projection_type == TransformProjectionType::rigid_nonsym) {
        DEBUG("projecting nonsymmetric affine update to rigid");
        ...
        Eigen::JacobiSVD<Eigen::Matrix<default_type, 3, 3>> const svd(L, Eigen::ComputeFullU | Eigen::ComputeFullV);
        ...
        m.col(0) /= x;
        R = m * svd.matrixV().adjoint();
        assert((R * R.transpose().eval())
                   .isApprox(Eigen::Matrix<default_type, Eigen::Dynamic, Eigen::Dynamic>::Identity(3, 3)) &&
               "projection of affine to rotation orthonormal?");
...
void AffineUpdate::set_control_points(const Eigen::Matrix<default_type, Eigen::Dynamic, Eigen::Dynamic> &points,
