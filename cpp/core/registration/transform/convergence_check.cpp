...
void DoubleExpSmoothSlopeCheck::set_parameters(const Eigen::Matrix<default_type, Eigen::Dynamic, 1> &slope_threshold,
...
void DoubleExpSmoothSlopeCheck::debug(const Eigen::Matrix<default_type, Eigen::Dynamic, 1> &control_points_vec) const {
  ...
  std::cout << fmt::format("{}", control_points_vec) << std::endl;
  if (len == 0) {
