...
template <class MetricType = Registration::Metric::MeanSquaredNoGradient> class ExhaustiveRotationSearch {
public:
  ExhaustiveRotationSearch(Image<default_type> &image1,
  ...
  using ParamType =
      Metric::Params<Registration::Transform::Rigid,
                     Image<default_type>,
                     Image<default_type>,
                     Header,
                     Image<default_type>,
...
private:
  FORCE_INLINE ParamType get_parameters() {
    ...
    midway_image_header = compute_minimum_average_header(
        im1, im2, local_trafo.get_transform_half_inverse(), local_trafo.get_transform_half());
  ...
  // ENH: less brute-force approach (for instance: fixed set of electrostatic repulsion directions, rotate all to gap
  // centre)
