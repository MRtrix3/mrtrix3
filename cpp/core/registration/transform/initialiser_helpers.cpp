...
template <class ImageType, class MaskType> class WeightedMomentsFunctor {
public:
...
template <class ImageType, class MaskType> class WeightedMassFunctor {
public:
...
void initialise_using_rotation_search(Image<default_type> &im1,
  ...
  RotationSearch::ExhaustiveRotationSearch<decltype(metric)>(im1,
                                                             im2,
...
void MomentsInitialiser::run() {
  ...
  // Maaref, A., & Aïssa, S. (2007). Eigenvalue distributions of wishart-type random matrices with application to the
  // performance analysis of MIMO MRC systems. IEEE Transactions on Wireless Communications, 6(7), 2678–2689.
  // doi:10.1109/TWC.2007.05990 Edelman, A. (1991). The distribution and moments of the smallest eigenvalue of a random
  // matrix of wishart type. Linear Algebra and Its Applications, 159, 55–80. doi:10.1016/0024-3795(91)90076-9
  ...
  im2_evec.array().rowwise() *= (im2_evec.array() * im1_evec.array())
                                    .colwise()
                                    .sum()
                                    .cwiseQuotient((im2_evec.array() * im1_evec.array()).colwise().sum().abs())
                                    .eval();
  ...
  Eigen::FullPivHouseholderQR<decltype(im2_evec_transpose)> const dec(im2_evec_transpose);
...
bool MomentsInitialiser::calculate_eigenvectors(Image<default_type> &image_1,
