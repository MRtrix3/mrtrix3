...
class CopyFunctor4D {
public:
  CopyFunctor4D(size_t out_start_vol, size_t nvols) : start_vol(out_start_vol), nvols(nvols) {}
  template <class VoxeltypeIn, class VoxeltypeOut> void operator()(VoxeltypeIn &in, VoxeltypeOut &out) {
    auto loop = Loop(3);
    auto l = loop(in);
...
class CopyFunctor3D {
public:
  CopyFunctor3D(size_t out_start_vol) : start_vol(out_start_vol) {}
  template <class VoxeltypeIn, class VoxeltypeOut> void operator()(VoxeltypeIn &in, VoxeltypeOut &out) {
...
void preload_data(std::vector<Header> &input,
...

  {
    ...
    images = Image<default_type>::scratch(h1);
    if (sumvols > 1)
      images = images.with_direct_io(3);
    else
      images = images.with_direct_io();
  }
