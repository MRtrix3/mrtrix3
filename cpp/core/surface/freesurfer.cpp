...
void read_label(std::string_view path, VertexList &vertices, Scalar &scalar) {
  ...
  if (line.substr(0, 13) != "#!ascii label")
    throw Exception("Error parsing FreeSurfer label file \"" + Path::basename(path) + "\":" + //
  ...
  try {
    ...
    throw Exception(e,
                    "Error parsing FreeSurfer label file \"" + Path::basename(path) + "\":" + //
                        " Bad second line vertex count");
  }
