...
template <class T, class Deleter = std::default_delete<T>> class copy_ptr : public std::unique_ptr<T, Deleter> {
public:
  ...
  template <class U, class E> copy_ptr(copy_ptr<U, E> &&u) noexcept : std::unique_ptr<T, Deleter>(std::move(u)) {}

  copy_ptr &operator=(const copy_ptr &u) {
    this->reset(u ? new T(*u) : nullptr);
    return *this;
  }
};
