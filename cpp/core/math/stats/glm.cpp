...
const std::string
    column_ones_description("In some software packages, a column of ones is automatically added to the "
...
App::OptionGroup glm_options(std::string_view element_name) {
  ...
  + Option("ttests",
           "perform one or more t-tests;"
           " input matrix text file should contain one row for each hypothesis,"
           " with each row performing a dot product with the GLM beta coefficients"
           " to form a contrast of interest")
    + Argument("path").type_file_in()

  + Option("ftest",
           "perform an F-test;"
           " input matrix text file should contain one or more rows,"
           " with each row specifying an undirected contrast to contribute to the F-test").allow_multiple()
    + Argument("path").type_file_in()
...
index_type batch_size() {
  ...
  static const index_type value = File::Config::get_int("GLMBatchSize", 16384);
  return value;
}
...
void check_design(const matrix_type &design, const bool extra_factors) {
  ...
  if (decomp.rank() < design.cols()) {
    ...
    if (cond > condnumber_warning_threshold) {
      if (extra_factors) {
        CONSOLE(fmt::format("Design matrix conditioning is poor (condition number: {:.6g}) before the addition of element-wise columns", cond));
      } else {
        WARN(fmt::format("Design matrix conditioning is poor (condition number: {:.6g}); model fitting may be highly influenced by noise", cond));
      }
    } else {
      CONSOLE(fmt::format("Design matrix condition number{}: {:.6g}", extra_factors ? " (without element-wise columns)" : "", cond));
    }
  }
}
...
index_array_type load_variance_groups(const index_type num_inputs) {
  ...
  try {
    ...
    if (max_coeff == min_coeff) {
      WARN(fmt::format("Only a single variance group is defined in file \"{}\"; variance groups will not be used", opt[0][0]));
      return index_array_type();
    }
    ...
    throw Exception(e, fmt::format("unable to read file \"{}\" as variance group data", opt[0][0]));
  }
}
...
std::vector<Hypothesis> load_hypotheses(const ssize_t num_factors) {
  ...
  if (!opt.empty()) {
    ...
    if (contrast_matrix.cols() != num_factors)
      throw Exception(fmt::format("Number of columns in T-test matrix file \"{}\"{}{}){}{})", opt[0][0], //
  ...
  for (size_t i = 0; i != opt.size(); ++i) {
    ...
    if (ftest_matrix.cols() != num_factors)
      throw Exception(fmt::format("Number of columns in F-test matrix \"{}\"{}{}){}{})", opt[i][0], //
...
vector_type std_effect_size(
  ...
  if (hypothesis.is_F())
    return vector_type::Constant(measurements.cols(), std::numeric_limits<vector_type::Scalar>::quiet_NaN());
...
void all_stats(const measurements_matrix_type &measurements,
  ...
  for (index_type ic = 0; ic != hypotheses.size(); ++ic) {
    if (hypotheses[ic].is_F()) {
      abs_effect_size.col(ic).fill(std::numeric_limits<matrix_type::Scalar>::quiet_NaN());
    } else {
    ...
    void zero() {
      ...
      for (index_type ih = 0; ih != hypotheses.size(); ++ih) {
        if (hypotheses[ih].is_F())
          local_abs_effect_size(0, ih) = local_std_effect_size(0, ih) =
...
template <class MatrixType> Hypothesis::Partition Hypothesis::partition(const MatrixType &design) const {
  ...
  const matrix_type Z = Cv.isZero() ? matrix_type::Zero(design.rows(), 1)
...
SharedFixedBase::SharedFixedBase(
    const measurements_matrix_type &measurements, const matrix_type &design, const std::vector<Hypothesis> &hypotheses)
...
SharedVariableBase::SharedVariableBase(
    const std::vector<CohortDataImport> &importers, const bool nans_in_data, const bool nans_in_columns)
    : importers(importers), nans_in_data(nans_in_data), nans_in_columns(nans_in_columns) {}

SharedHeteroscedasticBase::SharedHeteroscedasticBase(const std::vector<Hypothesis> &hypotheses,
                                                     const index_array_type &variance_groups)
    : VG(variance_groups),
...
TestFixedHomoscedastic::Shared::Shared(
    const measurements_matrix_type &measurements, const matrix_type &design, const std::vector<Hypothesis> &hypotheses)
...
TestFixedHomoscedastic::TestFixedHomoscedastic(
    const measurements_matrix_type &measurements, const matrix_type &design, const std::vector<Hypothesis> &hypotheses)
...
void TestFixedHomoscedastic::operator()(
    const shuffle_matrix_type &shuffling_matrix, matrix_type &stats, matrix_type &zstats) {
  assert(static_cast<index_type>(shuffling_matrix.rows()) == num_inputs());
  ...
  for (index_type ih = 0; ih != num_hypotheses(); ++ih) {
    ...
    for (index_type iestart = 0; iestart < num_elements(); iestart += batch_size()) {
      ...
      Sy.leftCols(this_batch_size).noalias() =
          shuffling_matrix.cast<default_type>() * S().partitions[ih].Rz *
          y.block(0, iestart, num_inputs(), this_batch_size).cast<default_type>();
#ifndef NDEBUG
...
#else
  sse_per_vg = decltype(sse_per_vg)::Constant(num_variance_groups(),
                                              std::min(num_elements(), batch_size()),
                                              std::numeric_limits<default_type>::signaling_NaN());
  Wterms = decltype(Wterms)::Constant(num_variance_groups(),
                                      std::min(num_elements(), batch_size()),
                                      std::numeric_limits<default_type>::signaling_NaN());
#endif
}
...
TestFixedHeteroscedastic::TestFixedHeteroscedastic(const TestFixedHeteroscedastic &that)
    : TestBase(that),
      ...
      sq_residuals(decltype(sq_residuals)::Constant(
          num_inputs(), std::min(num_elements(), batch_size()), std::numeric_limits<default_type>::signaling_NaN())),
      sse_per_vg(decltype(sse_per_vg)::Constant(num_variance_groups(),
                                                std::min(num_elements(), batch_size()),
                                                std::numeric_limits<default_type>::signaling_NaN())),
...
void TestFixedHeteroscedastic::operator()(
  ...
  for (index_type ih = 0; ih != num_hypotheses(); ++ih) {
    ...
    for (index_type iestart = 0; iestart < num_elements(); iestart += batch_size()) {
...
#ifndef NDEBUG
      sq_residuals.rightCols(sq_residuals.cols() - this_batch_size)
          .setConstant(std::numeric_limits<default_type>::signaling_NaN());
#endif
#ifdef GLM_TEST_DEBUG
      // VAR (sq_residuals);
#endif
...
#ifndef NDEBUG
      sse_per_vg.rightCols(sse_per_vg.cols() - this_batch_size)
          .setConstant(std::numeric_limits<default_type>::signaling_NaN());
#endif
#ifdef GLM_TEST_DEBUG
      // VAR (sse_per_vg);
      ...
      Wterms.leftCols(this_batch_size) =
          sse_per_vg.leftCols(this_batch_size).array().inverse().colwise() * S().Rnn_sums;
#ifndef NDEBUG
      Wterms.rightCols(Wterms.cols() - this_batch_size)
          .setConstant(std::numeric_limits<default_type>::signaling_NaN());
#endif
      ...
      for (index_type ie = 0; ie != this_batch_size; ++ie) {
        ...
        const default_type numerator =
            lambdas.col(ie).transpose() * c[ih].matrix().transpose() *
            (c[ih].matrix() * (M.transpose() * W.asDiagonal() * M).inverse() * c[ih].matrix().transpose()).inverse() *
            c[ih].matrix() * lambdas.col(ie);
#ifdef GLM_TEST_DEBUG
        VAR(numerator);
#endif
        ...
        if (!std::isfinite(G)) {
          ...
          stats(ie + iestart, ih) =
              c[ih].is_F() ? G : std::sqrt(G) * ((c[ih].matrix() * lambdas.col(ie)).sum() > 0.0 ? 1.0 : -1.0);
...
#endif
            zstats(ie + iestart, ih) =
                c[ih].is_F() ? Math::G2z(G, c[ih].rank(), dof) : Math::v2z(stats(ie + iestart, ih), dof);
          }
        }
      }
    }
...
#else
      dof(matrix_type::Constant(
          measurements.cols(), hypotheses.size(), std::numeric_limits<default_type>::signaling_NaN())),
      extra_column_data(measurements_matrix_type::Constant(
          measurements.rows(), importers.size(), std::numeric_limits<measurements_value_type>::signaling_NaN())),
      element_mask(element_mask_type::Zero(num_inputs())),
      permuted_mask(measurements.rows()),
      intermediate_shuffling_matrix(shuffle_matrix_type::Constant(
          measurements.rows(), measurements.rows(), std::numeric_limits<int8_t>::min())),
      shuffling_matrix_masked(shuffle_matrix_type::Constant(
          measurements.rows(), measurements.rows(), std::numeric_limits<int8_t>::min())),
      Mfull_masked(matrix_type::Constant(
          measurements.rows(), design.cols() + importers.size(), std::numeric_limits<default_type>::signaling_NaN())),
      pinvMfull_masked(matrix_type::Constant(
          design.cols() + importers.size(), measurements.rows(), std::numeric_limits<default_type>::signaling_NaN())),
      Rm(matrix_type::Constant(
          measurements.rows(), measurements.rows(), std::numeric_limits<default_type>::signaling_NaN())),
      y_masked(measurements_vector_type::Constant(measurements.rows(),
                                                  std::numeric_limits<default_type>::signaling_NaN())),
      Sy(vector_type::Constant(measurements.rows(), std::numeric_limits<default_type>::signaling_NaN())),
      lambda(
          vector_type::Constant(design.cols() + importers.size(), std::numeric_limits<default_type>::signaling_NaN()))
#endif
{
}
...
TestVariableBase::TestVariableBase(const TestVariableBase &that)
    : TestBase(that),
      ...
      intermediate_shuffling_matrix(that.intermediate_shuffling_matrix.rows(),
                                    that.intermediate_shuffling_matrix.cols()),
...
#else
      dof(matrix_type::Constant(
          that.dof.rows(), that.dof.cols(), std::numeric_limits<default_type>::signaling_NaN())),
      extra_column_data(
          measurements_matrix_type::Constant(that.extra_column_data.rows(),
                                              that.extra_column_data.cols(),
                                              std::numeric_limits<measurements_value_type>::signaling_NaN())),
      element_mask(element_mask_type::Zero(num_inputs())),
      permuted_mask(that.permuted_mask.size()),
      intermediate_shuffling_matrix(shuffle_matrix_type::Constant(that.intermediate_shuffling_matrix.rows(),
                                                                  that.intermediate_shuffling_matrix.cols(),
                                                                  std::numeric_limits<int8_t>::min())),
      shuffling_matrix_masked(shuffle_matrix_type::Constant(that.shuffling_matrix_masked.rows(),
                                                            that.shuffling_matrix_masked.cols(),
                                                            std::numeric_limits<int8_t>::min())),
      Mfull_masked(matrix_type::Constant(
          that.Mfull_masked.rows(), that.Mfull_masked.cols(), std::numeric_limits<default_type>::signaling_NaN())),
      pinvMfull_masked(matrix_type::Constant(that.pinvMfull_masked.rows(),
                                              that.pinvMfull_masked.cols(),
                                              std::numeric_limits<default_type>::signaling_NaN())),
      Rm(matrix_type::Constant(that.Rm.rows(), that.Rm.cols(), std::numeric_limits<default_type>::signaling_NaN())),
      y_masked(measurements_vector_type::Constant(that.y_masked.size(),
                                                  std::numeric_limits<measurements_value_type>::signaling_NaN())),
...
template <class SharedType> void TestVariableBase::set_mask(const SharedType &s, const index_type ie) {
  element_mask.setOnes();
  if (s.nans_in_data) {
...
TestVariableHomoscedastic::TestVariableHomoscedastic(const TestVariableHomoscedastic &that) : TestVariableBase(that) {
  for (index_type ih = 0; ih != num_hypotheses(); ++ih) {
#ifdef NDEBUG
    XtX.emplace_back(c[ih].matrix().rows(), c[ih].matrix().rows());
    beta.emplace_back(c[ih].matrix().rows(), 1);
#else
    XtX.emplace_back(matrix_type::Constant(
        c[ih].matrix().rows(), c[ih].matrix().rows(), std::numeric_limits<default_type>::signaling_NaN()));
    beta.emplace_back(
        matrix_type::Constant(c[ih].matrix().rows(), 1, std::numeric_limits<default_type>::signaling_NaN()));
#endif
  }
...
TestVariableHeteroscedastic::Shared::Shared(const std::vector<Hypothesis> &hypotheses,
                                            const index_array_type &variance_groups,
                                            const std::vector<CohortDataImport> &importers,
                                            const bool nans_in_data,
                                            const bool nans_in_columns)
