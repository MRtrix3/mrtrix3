...
template <typename T> inline T I0_scaled(const T x) {
  ...
  if (x * x < 4.0 * std::numeric_limits<T>::epsilon())
    return (1.0 - x);
  if (x <= 3.0)
    return (exp(-x) * (2.75 + Chebyshev::eval(coef_aI0, 11, -1.0, 1.0, x * x / 4.5 - 1.0)));
  if (x <= 8.0)
    return ((0.375 + Chebyshev::eval(coef_bI0, (sizeof(T) > 4 ? 20 : 13), -1.0, 1.0, (48.0 / x - 11.0) / 5.0)) /
            sqrt(x));
...
template <typename T> inline T I1_scaled(const T x) {
  ...
  if (x * x < 8.0 * std::numeric_limits<T>::epsilon())
    return (0.5 * x);
  if (x <= 3.0)
    return (x * exp(-x) * (0.875 + Chebyshev::eval(coef_aI1, 10, -1.0, 1.0, x * x / 4.5 - 1.0)));
  if (x <= 8.0) {
