...
template <class Function, class UpdateFunctor = LinearUpdate> class GradientDescent {
public:
  using value_type = typename Function::value_type;

  GradientDescent(Function &function,
                  UpdateFunctor update_functor = LinearUpdate(),
                  value_type step_size_upfactor = 3.0,
                  value_type step_size_downfactor = 0.1,
                  bool verbose = false)
      : func(function),
  ...
  void run(const size_t max_iterations = 1000,
    ...
    if (log_os) {
      ...
      for (ssize_t a = 0; a < x.size(); a++)
        log_os << fmt::format("{}x_{}", delim, a + 1);
      for (ssize_t a = 0; a < x.size(); a++)
        log_os << fmt::format("{}g_{}", delim, a + 1);
      log_os << "\n" << std::flush;
    }
    init(log_os);
  ...
  bool iterate(std::ostream &log_os) {
    ...
    while (normg != 0.0) {
      ...
      if (quadratic_minimum < step_down)
        quadratic_minimum = step_down;
      if (quadratic_minimum > step_up)
        quadratic_minimum = step_up;
