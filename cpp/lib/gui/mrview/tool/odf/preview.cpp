...
ODF_Preview::ODF_Preview(ODF *parent)
    : QWidget(&window(), Qt::Tool), parent(parent), render_frame(new RenderFrame(this)) {
...
void ODF_Preview::interpolation_slot(int) { parent->update_preview(); }

void ODF_Preview::show_axes_slot(int) { render_frame->set_show_axes(show_axes_box->isChecked()); }

void ODF_Preview::level_of_detail_slot(int) { render_frame->set_LOD(level_of_detail_selector->value()); }
