...
void Font::initGL(bool with_shadow) {
  ...
  gl::TexImage2D(gl::TEXTURE_2D, 0, gl::RG, tex_width, font_height, 0, gl::RG, gl::FLOAT, tex_data);
...
void Font::render(const std::string &text, int x, int y) const {
  ...
  for (size_t n = 0; n < text.size(); ++n) {
    ...
    pos[2] = x;
    pos[3] = y + font_height;
    pos[4] = x + font_width[c] + 2;
    pos[5] = y + font_height;
    pos[6] = x + font_width[c] + 2;
    pos[7] = y;

    GLfloat *tex = &tex_pos[8 * n];
    tex[0] = font_tex_pos[c];
    tex[1] = 1.0;
    tex[2] = font_tex_pos[c];
    tex[3] = 0.0;
    tex[4] = font_tex_pos[c] + font_tex_width[c];
    tex[5] = 0.0;
    tex[6] = font_tex_pos[c] + font_tex_width[c];
    tex[7] = 1.0;
