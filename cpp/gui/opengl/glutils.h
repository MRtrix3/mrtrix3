...
#define GLGETBOOL(x, n)                                                                                                \
  {                                                                                                                    \
    GLboolean __v[n];                                                                                                  \
    gl::GetBooleanv(x, __v);                                                                                           \
    std::cerr << #x " = ";                                                                                             \
    for (auto i : __v)                                                                                                 \
      std::cerr << int(i) << " ";                                                                                      \
    std::cerr << "\n";                                                                                                 \
...
public:
  ...
  void gen(GLenum target, GLint interp_type = gl::LINEAR) {
    if (!id) {
      ...
      tex_type = target;
      gl::GenTextures(1, &id);
      GL_DEBUG(fmt::format("created OpenGL texture ID {}", str(id)));
      bind();
  ...
  void clear() {
    if (id) {
      check_context();
      GL_DEBUG(fmt::format("deleting OpenGL texture ID {}", str(id)));
  ...
  void bind() const {
    assert(id);
    check_context();
    GL_DEBUG(fmt::format("binding OpenGL texture ID {}", str(id)));
...
public:
  ...
  void gen() {
    if (!id) {
      check_context.set();
      gl::GenBuffers(1, &id);
      GL_DEBUG(fmt::format("created OpenGL vertex buffer ID {}", str(id)));
    }
  }
  void clear() {
    if (id) {
      check_context();
      GL_DEBUG(fmt::format("deleting OpenGL vertex buffer ID {}", str(id)));
  ...
  void bind(GLenum target) const {
    assert(id);
    check_context();
    GL_DEBUG(fmt::format("binding OpenGL vertex buffer ID {}", str(id)));
...
public:
  ...
  void gen() {
    if (!id) {
      ...
      GL_DEBUG(fmt::format("created OpenGL vertex array ID {}", str(id)));
    }
  }
  void clear() {
    if (id) {
      check_context();
      GL_DEBUG(fmt::format("deleting OpenGL vertex array ID {}", str(id)));
  ...
  void bind() const {
    assert(id);
    check_context();
    GL_DEBUG(fmt::format("binding OpenGL vertex array ID {}", str(id)));
...
public:
  ...
  void gen() {
    if (!id) {
      check_context.set();
      gl::GenBuffers(1, &id);
      GL_DEBUG(fmt::format("created OpenGL index buffer ID {}", str(id)));
    }
  }
  void clear() {
    if (id) {
      check_context();
      GL_DEBUG(fmt::format("deleting OpenGL index buffer ID {}", str(id)));
      gl::DeleteBuffers(1, &id);
      id = 0;
    }
  }
  void bind() const {
    assert(id);
    check_context();
    GL_DEBUG(fmt::format("binding OpenGL index buffer ID {}", str(id)));
...
public:
  ...
  void gen() {
    if (!id) {
      ...
      GL_DEBUG(fmt::format("created OpenGL framebuffer ID {}", str(id)));
    }
  }
  void clear() {
    if (id) {
      check_context();
      GL_DEBUG(fmt::format("deleting OpenGL framebuffer ID {}", str(id)));
  ...
  void bind() const {
    assert(id);
    check_context();
    GL_DEBUG(fmt::format("binding OpenGL framebuffer ID {}", str(id)));
  ...
  void attach_color(Texture &tex, size_t attachment) const {
    ...
    bind();
    GL_DEBUG(fmt::format(
        "texture ID {} attached to framebuffer ID {} at color attachement {}", str(tex), str(id), str(attachment)));
