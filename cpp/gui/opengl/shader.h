...
public:
  ...
  void clear() {
    if (index_) {
      GL_DEBUG("deleting OpenGL shader ID {}", index_);
  ...
  void compile(std::string_view source) {
    ...
    if (!index_) {
      ...
      GL_DEBUG("created OpenGL {} shader ID {}", this->type(), index_);
    }
  ...
  static const std::string type() {
    return TYPE == gl::VERTEX_SHADER ? "vertex" : (TYPE == gl::FRAGMENT_SHADER ? "fragment" : "geometry");
  }
...
public:
  ...
  void clear() {
    if (index_) {
      GL_DEBUG("deleting OpenGL shader program {}", index_);
  ...
  template <GLint TYPE> void attach(const Object<TYPE> &object) {
    if (!index_) {
      ...
      GL_DEBUG("created OpenGL shader program ID {}", index_);
    }
    gl::AttachShader(index_, object.index_);
    GL_DEBUG("attached OpenGL {} shader ID {} to program ID {}", Object<TYPE>::type(), object.index_, index_);
  }
  template <GLint TYPE> void detach(const Object<TYPE> &object) {
    assert(index_);
    assert(object.index_);
    gl::DetachShader(index_, object.index_);
    GL_DEBUG("detached OpenGL {} shader ID {} from program ID {}", Object<TYPE>::type(), object.index_, index_);
  }
  void link() {
    GL_DEBUG("linking OpenGL shader program ID {}...", index_);
    assert(index_);
  ...
  void start() const {
    ...
    GL_DEBUG("using OpenGL shader program ID {}", index_);
  }
  static void stop() {
    gl::UseProgram(0);
