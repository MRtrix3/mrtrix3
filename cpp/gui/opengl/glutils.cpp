...
void init() {
  INFO(fmt::format("GL renderer:  {}", (const char *)gl::GetString(gl::RENDERER)));
  INFO(fmt::format("GL version:   {}", (const char *)gl::GetString(gl::VERSION)));
  INFO(fmt::format("GL vendor:    {}", (const char *)gl::GetString(gl::VENDOR)));
  ...
  /*
          ...
          switch (status) {
            ...
            case gl::FRAMEBUFFER_INCOMPLETE_MISSING_ATTACHMENT: throw Exception ("default framebuffer has no image
     attached!"); case gl::FRAMEBUFFER_INCOMPLETE_DRAW_BUFFER: throw Exception ("default framebuffer has incomplete draw
     buffer!"); case gl::FRAMEBUFFER_UNSUPPORTED: throw Exception ("default framebuffer does not support requested
     format!"); case gl::FRAMEBUFFER_COMPLETE: break; default: throw Exception ("default framebuffer cannot be used for
     unknown reason!");
          }
          */
}
