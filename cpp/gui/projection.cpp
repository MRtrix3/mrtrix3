...
public:
  ...
  Eigen::Vector3f dir;
  std::string label;
  bool operator<(const OrientationLabel &R) const { return dir.squaredNorm() < R.dir.squaredNorm(); }
};
} // namespace
...
void Projection::draw_orientation_labels() const {
  ...
  for (size_t i = 2; i < labels.size(); ++i) {
    ...
    float dist = std::min(width() / std::fabs(pos[0]), height() / std::fabs(pos[1])) / 2.0;
