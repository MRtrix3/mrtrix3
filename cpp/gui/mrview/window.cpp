...
void Window::image_select_slot(QAction *action) {
  ...
  set_image_navigation_menu();
  image()->set_allowed_features(mode->features & Mode::ShaderThreshold,
...
#include "mrview/tool/list.h"
    ...
    if (opt.opt->is("volume")) {
      if (image()) {
        auto pos = parse_ints<Axes::index_type>(opt[0]);
        for (size_t n = 0; n < std::min(pos.size(), static_cast<size_t>(image()->image.ndim())); ++n) {
