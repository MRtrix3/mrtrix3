...
Qt::KeyboardModifiers get_modifier(std::string_view key, Qt::KeyboardModifiers default_key) {
  const std::string value = lowercase(MR::File::Config::get(key));
  if (value.empty())
    return default_key;
...
Window::GLArea::GLArea(Window &parent) : GL::Area(&parent) {
...
QSize Window::GLArea::sizeHint() const {
  ...
  const std::string init_size_string = lowercase(MR::File::Config::get("MRViewInitWindowSize"));
...
void Window::GLArea::dragLeaveEvent(QDragLeaveEvent *event) { event->accept(); }
...
void Window::GLArea::initializeGL() { main->initGL(); }
void Window::GLArea::paintGL() { main->paintGL(); }
...
Window::Window()
  ...
  {
    const int iconsize = MR::File::Config::get_int("IconSize", 30);
  ...
  {
    const std::string toolbar_pos_spec = lowercase(MR::File::Config::get("InitialToolBarPosition"));
    if (!toolbar_pos_spec.empty()) {
      if (toolbar_pos_spec == "bottom")
        toolbar_position = Qt::BottomToolBarArea;
      else if (toolbar_pos_spec == "left")
        toolbar_position = Qt::LeftToolBarArea;
      else if (toolbar_pos_spec == "right")
        toolbar_position = Qt::RightToolBarArea;
  ...
  const auto button_style = static_cast<Qt::ToolButtonStyle>(MR::File::Config::get_int("ToolbarStyle", 2));
  ...
  next_image_volume_group_action =
      image_menu->addAction(tr("Next volume group"), this, SLOT(image_next_volume_group_slot()));
  ...
  prev_image_volume_group_action =
      image_menu->addAction(tr("Previous volume group"), this, SLOT(image_previous_volume_group_slot()));
  ...
  goto_image_volume_group_action =
      image_menu->addAction(tr("Go to volume group..."), this, SLOT(image_goto_volume_group_slot()));
...
#undef MODE_OPTION
  ...
  snap_to_image_action = toolbar->addAction(QIcon(":/lock.svg"), tr("Snap to image"), this, SLOT(snap_to_image_slot()));
  snap_to_image_action->setToolTip(tr("Snap focus and view orientation to\n"
...
void Window::select_mouse_mode_slot(QAction *action) {
  const bool rotate_button_checked = mode_action_group->actions().indexOf(action) == 2;
...
void Window::create_tool(QAction *action, bool show) {
  ...
  if (!tools_floating) {
    ...
    for (int i = 0; i < tool_group->actions().size(); ++i) {
      ...
      if ((other_tool != nullptr) && other_tool != tool) {
...
void Window::on_scaling_changed() { emit scalingChanged(); }

void Window::wrap_volumes_slot() { set_image_navigation_menu(); }

void Window::updateGL() { glarea->update(); }

void Window::drawGL() { glarea->repaint(); }
...
void Window::select_plane_slot(QAction *action) {
  if (action == axial_action)
    set_plane(2);
  else if (action == sagittal_action)
    set_plane(0);
  else if (action == coronal_action)
    set_plane(1);
  else
    assert(0);
  glarea->update();
...
void Window::hide_image_slot() {
  const bool visible = !image_hide_action->isChecked();
...
void Window::image_next_slot() {
  ...
  const int N = image_group->actions().size();
  const int n = image_group->actions().indexOf(action);
...
void Window::image_next_volume_slot() {
  const ssize_t vol = image()->image.index(3) + 1;
...
void Window::image_goto_volume_slot() {
  const size_t maxvol = image()->image.size(3) - 1;
...
void Window::image_goto_volume_group_slot() {
  const size_t maxvolgroup = image()->image.size(4) - 1;
...
void Window::image_next_volume_group_slot() {
  const ssize_t vol = image()->image.index(4) + 1;
...
void Window::image_select_slot(QAction *action) {
  ...
  const size_t cmap_index = image()->colourmap;
  ...
  set_image_navigation_menu();
  image()->set_allowed_features((mode->features & Mode::ShaderThreshold) != 0,
                                (mode->features & Mode::ShaderTransparency) != 0,
                                (mode->features & Mode::ShaderLighting) != 0);
  emit imageChanged();
  glarea->update();
}
...
void Window::toggle_annotations_slot() {
  ...
  if (show_crosshairs())
    current_annotations |= 0x00000001;
  if (show_comments())
    current_annotations |= 0x00000002;
  if (show_voxel_info())
    current_annotations |= 0x00000004;
  if (show_orientation_labels())
    current_annotations |= 0x00000008;
  if (show_colourbar())
    current_annotations |= 0x00000010;
  ...
  if (current_annotations != 0) {
    ...
    show_crosshairs_action->setChecked((annotations & 0x00000001) != 0);
    show_comments_action->setChecked((annotations & 0x00000002) != 0);
    show_voxel_info_action->setChecked((annotations & 0x00000004) != 0);
    show_orientation_labels_action->setChecked((annotations & 0x00000008) != 0);
    show_colourbar_action->setChecked((annotations & 0x00000010) != 0);
  }
  glarea->update();
}
...
void Window::set_cursor() {
  ...
  if ((tool_has_focus != nullptr) && modifiers_ == Qt::NoModifier) {
...
void Window::set_mode_features() {
  mode_action_group->actions()[0]->setEnabled((mode->features & Mode::FocusContrast) != 0);
  mode_action_group->actions()[1]->setEnabled((mode->features & Mode::MoveTarget) != 0);
  mode_action_group->actions()[2]->setEnabled((mode->features & Mode::TiltRotate) != 0);
...
void Window::about_slot() {
  const std::string message = std::string("<h1>MRView</h1>The MRtrix viewer, version ") + MR::App::mrtrix_version +
                              "<br>"
                              "<em>" +
                              str(8 * sizeof(size_t)) +
                              " bit "
...
void Window::mousePressEventGL(QMouseEvent *event) {
  ...
  const int group = get_mouse_mode();
...
void Window::mouseReleaseEventGL(QMouseEvent *) {
  ...
  if ((tool_has_focus != nullptr) && mouse_action == NoAction)
...
bool Window::gestureEventGL(QGestureEvent *event) {
  ...
  if (QGesture *pinch = event->gesture(Qt::PinchGesture)) {
    ...
    const QPinchGesture::ChangeFlags changeFlags = e->changeFlags();
...
void Window::register_camera_interactor(Tool::CameraInteractor *agent) {
  if ((camera_interactor != nullptr) && camera_interactor != agent)
...
#include "mrview/tool/list.h"
    ...
    if (opt.opt->is("size")) {
      ...
      const QSize oldsize = glarea->size();
