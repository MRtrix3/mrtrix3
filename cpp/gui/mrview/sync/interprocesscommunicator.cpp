...
InterprocessCommunicator::InterprocessCommunicator() : QObject(0) {
  ...
  for (int i_attempt = 0; i_attempt < 100; i_attempt++) {
    ...
    for (int i = 0; i < MAX_NO_ALLOWED; i++) {
      ...
      if (state != QLocalSocket::ConnectedState) {
        ...
        QLocalServer::removeServer(
            serverName); // this can be needed when previous processes used this but were forcibly killed, leaving a
                         // temporary file. If that file exists, then we will get an exception ("address in use") when
                         // trying to use this Id
        break;
      }
    }
    if (freeEntry == -1) {
    ...
    // else we didn't connect OK. This can occur when two processes start simultaneously and both try for the same
    // number. Loop back and re-try
  }
