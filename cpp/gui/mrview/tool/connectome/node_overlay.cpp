...
void NodeOverlay::update_texture2D(const int plane, const int slice) {
  ...
  gl::TexImage3D(
      gl::TEXTURE_3D, 0, internal_format, xsize, ysize, 1, 0, format, type, reinterpret_cast<void *>(&texture_data[0]));
  need_update = false;
}
...
void NodeOverlay::update_texture3D() {
  ...
  // upload_data ({ { 0, 0, 0 } }, { { data.size(0), data.size(1), data.size(2) } }, reinterpret_cast<void*>
  // (data.address(0)));
  need_update = false;
}
