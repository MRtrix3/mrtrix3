...
bool NodeShader::need_update(const Connectome &parent) const {
  if (crop_to_slab != parent.crop_to_slab)
    return true;
  if (is_3D != parent.is_3D)
    return true;
  if (use_lighting != parent.use_lighting())
    return true;
  if (geometry != parent.node_geometry)
    return true;
  if (colour != parent.node_colour)
    return true;
...
void NodeShader::update(const Connectome &parent) {
  ...
  if (crop_to_slab) {
    vertex_shader_source += "uniform vec3 screen_normal;\n";
    if (is_3D) {
      vertex_shader_source += "out float include;\n"
                              "uniform float crop_var;\n"
                              "uniform float slab_thickness;\n";
    } else {
      vertex_shader_source += "out float depth;\n"
  ...
  vertex_shader_source += "  gl_Position = MVP * vec4 (pos, 1);\n";

  if (crop_to_slab) {
    if (is_3D) {
      vertex_shader_source += "  include = (dot (pos, screen_normal) - crop_var) / slab_thickness;\n";
    } else {
      vertex_shader_source += "  depth = dot (pos, screen_normal) - depth_offset;\n";
    }
  }
  ...
  fragment_shader_source = "uniform vec3 node_colour;\n";
  ...
  if (crop_to_slab && is_3D) {
    fragment_shader_source += "in float include;\n";
  }

  fragment_shader_source += "void main() {\n";

  if (crop_to_slab && is_3D) {
    fragment_shader_source += "  if (include < 0 || include > 1) discard;\n";
  }

  if (use_alpha) {
    fragment_shader_source += "  color.rgb = node_colour;\n";
  } else {
...
bool EdgeShader::need_update(const Connectome &parent) const {
  ...
  if (use_lighting != parent.use_lighting())
    return true;
  if (geometry != parent.edge_geometry)
    return true;
  if (colour != parent.edge_colour)
    return true;
  if (colour == edge_colour_t::MATRIX_FILE && colourmap_index != parent.edge_colourmap_index)
    return true;
...
void EdgeShader::update(const Connectome &parent) {
  ...
  case edge_geometry_t::CYLINDER:
    vertex_shader_source += "  vec3 centre = centre_one;\n"
  ...
  fragment_shader_source = "uniform vec3 edge_colour;\n";

  if (use_alpha) {
    fragment_shader_source += "uniform float edge_alpha;\n"
                              "out vec4 color;\n";
  } else {
    fragment_shader_source += "out vec3 color;\n";
  }
  ...
  if (use_lighting) {
    if (geometry == edge_geometry_t::CYLINDER || geometry == edge_geometry_t::STREAMTUBE) {
      ...
      fragment_shader_source += "  vec3 t = normalize (tangent" + GS_out +
                                ...
                                "  color += specular * sqrt(cos2_theta) * pow (clamp (-l_dot_t*n_dot_t + "
                                "length(l_perp)*length(n_perp), 0, 1), shine);\n";
    }
  }
