...
protected:
  ...
  public:
    InterpolateCheckBox(const QString &text, QWidget *parent = nullptr) : QCheckBox(text, parent) {}

  protected:
    // We don't want a click to cycle to a partially checked state
    // So explicitly specify the allowed clickable states
    void nextCheckState() override {
      checkState() == Qt::Unchecked ? setCheckState(Qt::Checked) : setCheckState(Qt::Unchecked);
    }
  };
