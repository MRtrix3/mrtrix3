...
ODF::ODF(Dock *parent) : Base(parent), preview(nullptr), renderer(nullptr), lighting_dock(nullptr), lmax(0) {
  ...
  main_grid_box->setToolTip(tr("Show individual ODFs at the spatial resolution of the main image instead of the ODF "
                               "image's own spatial resolution"));
  ...
  colour_relative_to_projection_box->setToolTip(tr("Colour ODFs according their direction relative to the "
...
void ODF::draw(const Projection &projection, bool is_3D, int, int) {
  ...
  if (!hide_all_button->isChecked()) {
    ...
    renderer->start(projection,
                    *lighting,
                    settings->scale,
                    use_lighting_box->isChecked(),
                    settings->color_by_direction,
                    settings->hide_negative,
                    true,
                    rotation);
    ...
    const Eigen::Vector3f x_width =
        projection.screen_to_model_direction(projection.width() / 2.0, 0.0, projection.depth_of(pos));
    const int nx = std::ceil(x_width.norm() / x_dir.norm());
    const Eigen::Vector3f y_width =
        projection.screen_to_model_direction(0.0, projection.height() / 2.0, projection.depth_of(pos));
    ...
    for (int y = -ny; y <= ny; ++y) {
      for (int x = -nx; x <= nx; ++x) {
        ...
        if (settings->odf_type == odf_type_t::DIXEL &&
            settings->dixel->dir_type == ODF_Item::DixelPlugin::dir_t::DW_SCHEME)
          values.resize(settings->image.header().size(3));
  ...
  if (interp) {
    ...
    if (linear_interp.scanner(pos)) {
      for (linear_interp.index(3) = 0;
           linear_interp.index(3) < std::min(static_cast<VoxelIndex>(values.size()), linear_interp.size(3));
    ...
    if (nearest_interp.scanner(pos)) {
      for (nearest_interp.index(3) = 0;
           nearest_interp.index(3) < std::min(static_cast<VoxelIndex>(values.size()), nearest_interp.size(3));
...
void ODF::closeEvent(QCloseEvent *) { window().disconnect(this); }
...
void ODF::hide_all_slot() { window().updateGL(); }
