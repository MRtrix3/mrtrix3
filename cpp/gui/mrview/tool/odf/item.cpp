...
ODF_Item::ODF_Item(
  ...
  try {
    ...
    DEBUG("Image {} initialised as dixel ODF using DW scheme", image.header().path());
  } catch (...) {
    try {
      dixel->set_header();
      DEBUG("Image {} initialised as dixel ODF using header directions field", image.header().path());
    } catch (...) {
      try {
        dixel->set_internal(image.header().size(3));
        DEBUG("Image {} initialised as dixel ODF using internal direction set", image.header().path());
      } catch (...) {
        DEBUG("Image {} left uninitialised in ODF tool", image.header().path());
      }
    }
  }
}
  ...
  if (entry != H.keyval().end()) {
    try {
      ...
      if (lines.size() != static_cast<size_t>(H.size(3)))
        throw Exception("malformed directions field in image \"{}\" - incorrect number of rows", H.path());
      for (size_t row = 0; row < lines.size(); ++row) {
        const auto values = parse_floats(lines[row]);
        if (!header_dirs.rows()) {
          if (values.size() != 2 && values.size() != 3)
            throw Exception("malformed directions field in image \"{}\" - should have 2 or 3 columns", H.path());
          header_dirs.resize(lines.size(), values.size());
        } else if (values.size() != static_cast<size_t>(header_dirs.cols())) {
          header_dirs.resize(0, 0);
          throw Exception("malformed directions field in image \"{}\" - variable number of columns", H.path());
        }
