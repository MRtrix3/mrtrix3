...
std::string BaseFixel::Shader::vertex_shader_source(const Displayable &) {
  std::string source = "layout (location = 0) in vec3 centre;\n"
...
std::string BaseFixel::Shader::geometry_shader_source(const Displayable &object) {
  ...
  source += "    vec4 start = MVP * (gl_in[0].gl_Position";
...
std::string BaseFixel::Shader::fragment_shader_source(const Displayable &) {
  std::string source = "out vec3 outColour;\n"
...
void BaseFixel::render(const Projection &projection) {
  ...
  if (!fixel_tool.is_cropped_to_slab()) {
    ...
    if (GLsizei const points_count = regular_grid_buffer_pos.size())
...
void BaseFixel::update_interp_image_buffer(const Projection &projection,
  ...
  gl::BufferData(gl::ARRAY_BUFFER,
                 regular_grid_buffer_pos.size() * sizeof(Eigen::Vector3f),
  ...
  gl::BufferData(gl::ARRAY_BUFFER,
                 regular_grid_buffer_dir.size() * sizeof(Eigen::Vector3f),
  ...
  if (has_val) {
    ...
    gl::BufferData(gl::ARRAY_BUFFER,
                   regular_grid_buffer_threshold.size() * sizeof(float),
