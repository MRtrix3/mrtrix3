...
ROI_UndoEntry::Shared::Shared() : count(1) {
  ...
  const GL::Shader::Vertex vertex_shader("layout(location = 0) in ivec3 vertpos;\n"
                                         "void main() {\n"
                                         "  gl_Position = vec4 (vertpos,1);\n"
                                         "}\n");
  const GL::Shader::Fragment fragment_shader("uniform usampler3D tex;\n"
...
void ROI_UndoEntry::draw_line(ROI_Item &roi,
...
void ROI_UndoEntry::draw_thick_line(ROI_Item &roi,
                                    const Eigen::Vector3f &prev_pos,
                                    const Eigen::Vector3f &pos,
                                    const bool insert_mode_value,
...
void ROI_UndoEntry::draw_circle(ROI_Item &roi,
                                const Eigen::Vector3f &pos,
                                const bool insert_mode_value,
                                const float diameter) {
  ...
  for (int k = a[2]; k < b[2]; ++k)
    for (int j = a[1]; j < b[1]; ++j)
      for (int i = a[0]; i < b[0]; ++i)
        if (Math::pow2(roi.header().spacing(0) * (vox[0] - i)) + Math::pow2(roi.header().spacing(1) * (vox[1] - j)) +
                Math::pow2(roi.header().spacing(2) * (vox[2] - k)) <
            radius_sq)
          after[i - from[0] + size[0] * (j - from[1] + size[1] * (k - from[2]))] = value;
...
void ROI_UndoEntry::draw_rectangle(ROI_Item &roi,
                                   const Eigen::Vector3f &from_pos,
                                   const Eigen::Vector3f &to_pos,
                                   const bool insert_mode_value) {
