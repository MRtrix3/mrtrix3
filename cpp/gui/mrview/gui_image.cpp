...
void ImageBase::render2D(Displayable::Shader &shader_program,
...
void Image::update_texture2D(int plane, int slice) {
  ...
  if (cmap_name == "RGB") {
      ...
      for (size_t n = 0; n < 3; ++n) {
        ...
        for (image.index(y) = 0; image.index(y) < ysize; ++image.index(y)) {
          for (image.index(x) = 0; image.index(x) < xsize; ++image.index(x)) {
            cfloat const val = image.value();
            float const mag = MR::abs(val.real());
    ...
    if (tex_positions[plane] < 0 || tex_positions[plane] >= header().size(plane)) {
      ...
      for (image.index(y) = 0; image.index(y) < ysize; ++image.index(y)) {
        for (image.index(x) = 0; image.index(x) < xsize; ++image.index(x)) {
          cfloat const val = image.value();
          size_t const idx = 2 * (image.index(x) + image.index(y) * xsize);
          data[idx] = val.real();
          data[idx + 1] = val.imag();
          float const mag = MR::abs(val);
          if (std::isfinite(mag))
            slice_max[plane] = std::max(slice_max[plane], mag);
        }
      }
  ...
  gl::TexImage3D(
      gl::TEXTURE_3D, 0, internal_format, xsize, ysize, 1, 0, format, type, reinterpret_cast<void *>(&data[0]));
}
