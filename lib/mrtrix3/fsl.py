...
def checkFirst(prefix, structures): #pylint: disable=unused-variable
  import os
  from mrtrix3 import app, file, MRtrixException, path # pylint: disable=redefined-builtin
  ...
  if existing_file_count != len(vtk_files):
    if 'SGE_ROOT' in os.environ:
      ...
      raise MRtrixException('FSL FIRST has failed; only ' + str(existing_file_count) + ' of ' + str(len(vtk_files)) + ' structures were segmented successfully (check ' + path.toTemp('first.logs', False) + ')')
