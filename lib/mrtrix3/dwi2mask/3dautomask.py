...
def usage(base_parser, subparsers): #pylint: disable=unused-variable
  ...
  options.add_argument('-clfrac', type=app.Parser.Float(0.1, 0.9), help='Set the \'clip level fraction\', must be a number between 0.1 and 0.9. A small value means to make the initial threshold for clipping smaller, which will tend to make the mask larger.')
  ...
  options.add_argument('-peels', type=app.Parser.Int(0), help='Peel (erode) the mask n times, then unpeel (dilate).')
  options.add_argument('-nbhrs', type=app.Parser.Int(6, 26), help='Define the number of neighbors needed for a voxel NOT to be eroded.  It should be between 6 and 26.')
  ...
  options.add_argument('-SI', type=app.Parser.Float(0.0), help='After creating the mask, find the most superior voxel, then zero out everything more than SI millimeters inferior to that. 130 seems to be decent (i.e., for Homo Sapiens brains).')
  options.add_argument('-dilate', type=app.Parser.Int(0), help='Dilate the mask outwards n times')
  options.add_argument('-erode', type=app.Parser.Int(0), help='Erode the mask outwards n times')
