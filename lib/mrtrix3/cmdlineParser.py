...
def initialise(desc):
  ...
  if not app.author:
    ...
    sys.exit(1)
  app.parser = Parser(description=desc, add_help=False)
  standard_options = app.parser.add_argument_group('Standard options')
...
class Parser(argparse.ArgumentParser):
  ...
  def print_help(self):
    ...
    from mrtrix3 import app, mrtrix
    ...
    s += '\n'
    s += bold('COPYRIGHT') + '\n'
    s += w.fill(app.copyright) + '\n'
    if app.citationList:
      s += '\n'
      s += bold('REFERENCES') + '\n'
      s += '\n'
      for entry in app.citationList:
        if entry[0]:
          s += w.fill('* ' + entry[0] + ':') + '\n'
        s += w.fill(entry[1]) + '\n'
        s += '\n'
