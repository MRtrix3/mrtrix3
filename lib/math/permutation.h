...
namespace MR {
  namespace Math {
    namespace {
      template <typename T> inline void permute (const gsl_permutation* p, VectorView<T>& V);
      template <> inline void permute (const gsl_permutation* p, VectorView<double>& V) { gsl_permute_vector (p, &V); }
      template <> inline void permute (const gsl_permutation* p, VectorView<float>& V) { gsl_permute_vector_float (p, &V); }
    }
	...
	~Permutation () { gsl_permutation_free (p); }
        ...
        template <typename T> VectorView<T>& apply (VectorView<T>& V) const { permute (p, V); return (V); }
