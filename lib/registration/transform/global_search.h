...
namespace MR
{
  namespace Registration
  {
    namespace GlobalSearch
    {
      ...
      //   void crossover(const default_type& t, const TransformType& P1, const TransformType& P2, TransformType& F1, TransformType& F2) {
      //     assert (t > 0);
      //     assert (t < 1.0);
      //     Eigen::MatrixXd M1 = P1.linear();
      //     Eigen::MatrixXd M2 = P2.linear();
      //     assert (M1.determinant() * M2.determinant() > 0);

      //     Eigen::Matrix3d R1 = P1.rotation();
      //     Eigen::Matrix3d R2 = P2.rotation();
      ...
      //     F1.translation() = ((1.0 - t) * P1.translation() + t * P2.translation());
      //     Qout = Q1.slerp(t, Q2);
      //     F1.linear() = Qout * ((1.0 - t) * S1 + t * S2);

      //     F2.translation() = ( t * P1.translation() + (1.0 - t) * P2.translation());
      //     Qout = Q1.slerp(1.0 - t, Q2);
      //     F2.linear() = Qout * (t * S1 + (1.0 - t) * S2);
      //   }
      ...
      bool better (const Candidate<ComputeType>& a, const Candidate<ComputeType>& b) {
      ...
      class GlobalSearch
      {
          ...
          void run_masked (
            ...
            // typedef typename TransformType::ParameterType ParameterType;
              ...
              while (parental.size() < pool_size){
                ...
                parental.back().run_sgd (metric, transform, im1_smoothed, im2_smoothed, im1_mask, im2_mask);
                if (parental.back().get_overlap() == 0)
                  parental.pop_back();
              ...
              // cross parental generation --> filial generation
              for (size_t iter = 0; iter < max_iter; ++iter){
                ...
                std::sort(std::begin(filial), std::end(filial));
              ...
              std::sort(std::begin(geneology), std::end(geneology), better<default_type>);
              VAR(median_overlap);
              ...
              for(auto const& c: geneology) {
                INFO(str(c.get_overlap() > median_overlap) + " ------- " + str(c));
              }

              TrafoType winner = best->get_trafo();
              CONSOLE("global search result: " + str(*best));
        ...
        // private:
          ...
          // template <class MetricType, class TransformType,  class Im1ImageType, class Im2ImageType, class Im1MaskType, class Im2MaskType, class TrafoType>
