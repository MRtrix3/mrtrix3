        ...
        class Linear : public VoxelType, public Transform
      {
        public:
          ...
          value_type value () {
            ...
            value_type val = 0.0;
            if (faaa) val  = faaa * value_type (VoxelType::value());
            (*this)[2]++;
            if (faab) val += faab * value_type (VoxelType::value());
            (*this)[1]++;
            if (fabb) val += fabb * value_type (VoxelType::value());
            (*this)[2]--;
            if (faba) val += faba * value_type (VoxelType::value());
            (*this)[0]++;
            if (fbba) val += fbba * value_type (VoxelType::value());
            (*this)[1]--;
            if (fbaa) val += fbaa * value_type (VoxelType::value());
            (*this)[2]++;
            if (fbab) val += fbab * value_type (VoxelType::value());
            (*this)[1]++;
            if (fbbb) val += fbbb * value_type (VoxelType::value());
            (*this)[0]--;
            (*this)[1]--;
            (*this)[2]--;
            return val;
