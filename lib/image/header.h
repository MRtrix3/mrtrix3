    ...
    class Header : public std::map<std::string, std::string> {
      public:
        ...
        Header& operator= (const Header& H) 
        {
          ...
          format_ = NULL; offset_ = H.offset_; scale_ = H.scale_; readwrite_ = false;
        ...
        bool operator! () const { return (handler_); }
        ...
        const DataType& datatype () const { return (dtype_); }
        bool is_complex () const { return (dtype_.is_complex()); }

        float data_offset () const { return (offset_); }
        float data_scale () const { return (scale_); }

        size_t ndim () const { return (axes_.size()); }
        int dim (size_t index) const { return (axes_[index].dim); } 
        float vox (size_t index) const { return (axes_[index].vox); }
        ssize_t stride (size_t axis) const { return (axes_[axis].stride); }
        const std::string& description (size_t axis) const { return (axes_[axis].description); }
        const std::string& units (size_t axis) const { return (axes_[axis].units); }

        const Math::Matrix<float>& DW_scheme () const { return (DW_scheme_); }
        ...
        void set_datatype (DataType new_datatype) { check_not_finalised(); dtype_ = new_datatype; }
        void set_datatype (const std::string& specifier) { check_not_finalised(); dtype_.parse (specifier); }
        void set_transform (const Math::Matrix<float>& new_transform) { check_not_finalised(); transform_ = new_transform; }
        void set_DW_scheme (const Math::Matrix<float>& new_DW_scheme) { check_not_finalised(); DW_scheme_ = new_DW_scheme; }

        void add_comment (const std::string& text) { check_not_finalised(); comments_.push_back (text); }

        Math::Matrix<float>& get_DW_scheme () { check_not_finalised(); return (DW_scheme_); }
        Math::Matrix<float>& get_transform () { check_not_finalised(); return (transform_); }
        std::vector<std::string>& get_comments () { check_not_finalised(); return (comments_); }

        void set_ndim (size_t new_ndim) { check_not_finalised(); axes_.resize (new_ndim); }
        void set_dim (size_t axis, int new_dim) { check_not_finalised(); axes_[axis].dim = new_dim; }
        void set_vox (size_t axis, float new_vox) { check_not_finalised(); axes_[axis].vox = new_vox; }
        void set_stride (size_t axis, ssize_t new_stride) { check_not_finalised(); axes_[axis].stride = new_stride; }
        void set_description (size_t axis, const std::string& new_description) { check_not_finalised(); axes_[axis].description = new_description; }
        void set_units (size_t axis, const std::string& new_units) { check_not_finalised(); axes_[axis].units = new_units; }

        void clear () {
          std::map<std::string, std::string>::clear(); 
          name_.clear(); axes_.clear(); comments_.clear(); dtype_ = DataType();
          offset_ = 0.0; scale_ = 1.0; readwrite_ = false; format_ = NULL;
          transform_.clear(); DW_scheme_.clear();
        }

        void  set_scaling (float scaling = 1.0, float bias = 0.0) { check_not_finalised(); offset_ = bias; scale_ = scaling; }
        void  reset_scaling () { set_scaling(); }
        void  apply_scaling (float scaling, float bias = 0.0) { check_not_finalised(); scale_ *= scaling; offset_ = scaling * offset_ + bias; }


        // USED IN BACKEND:

        void  set_handler (Handler::Base* handler) { handler_ = handler; }
        Handler::Base* get_handler () { return (handler_.get()); }
        void add_file (const File::Entry& entry) { files_.push_back (entry); }
        const std::vector<File::Entry>& get_files () const { return (files_); }
      ...
      private:
        ...
        void check_not_finalised () const { assert (files_.empty() && !handler_); }

        int64_t footprint_for_count (int64_t count) { return (dtype_ == DataType::Bit ? (count+7)/8 : count * dtype_.bytes()); }
