...
namespace MR
{
   ...
   *
   * The StochasticThreadedLoop class provides a simple and powerful interface
   * for rapid development of multi-threaded image processing applications.
   * For full details, please refer the StochasticThreadedLoop class
   * documentation.
   *
   * @} */
   ...
   *
   * The StochasticThreadedLoop class is generally used by first initialising
   ...
   * To do this, the StochasticThreadedLoop class defines a set of axes that
   ...
   * of the StochasticThreadedLoop using the inner_axes() method. By default,
   ...
   * remaining axes are managed by the StochasticThreadedLoop class: each
   * invocation of the thread's functor is given a fresh position to operate
   * from, in the form of an Iterator class.
   *
   * Conceptually, the StochasticThreadedLoop performs something akin to the
   * following:
   *
   * \code
   ...
   *
   * The StochasticThreadedLoop constructors can be used to set up any
   ...
   *
   * The run() methods will run the StochasticThreadedLoop, invoking the
   ...
   *
   * MyVoxelType vox;
   * StochasticThreadedLoop ("computing exponential in-place...", vox)
   *   .run (my_function, vox);
   * \endcode
   */
   ...
   * ...
   *
   * StochasticThreadedLoop (vox1).run (MyAdd(), vox_out, vox1, vox2); 
   * \endcode 
   *
   ...
   *   out.value() = in1.value() + in2.value();
   * }
   * StochasticThreadedLoop (vox1).run (f, vox_out, vox1, vox2);
   * \endcode
   * 
   */ 
   ...
   *
   * double SoS = 0.0;
   * StochasticThreadedLoop ("computing RMS of \"" + vox.name() + "\"...", vox)
   *     .run (RMS(SoS), vox);
   *
