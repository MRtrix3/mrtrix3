...
namespace MR
{
  namespace File
  {
    namespace NIfTI
    {
      ...
      {
        bool is_BE = false;
        if (Raw::fetch_<int32_t> (&NH.sizeof_hdr, is_BE) != ver1_hdr_size) {
          is_BE = true;
          if (Raw::fetch_<int32_t> (&NH.sizeof_hdr, is_BE) != ver1_hdr_size) {
            std::cerr << "GDB break here\n";
            throw Exception ("image \"" + H.name() + "\" is not in NIfTI format (sizeof_hdr != " + str(ver1_hdr_size) + ")");
          }
        }
        ...
        switch (Raw::fetch_<int16_t> (&NH.datatype, is_BE)) {
          ...
          case DT_COMPLEX128:
            ...
            break;
          default:
            throw Exception ("unknown data type for NIfTI image \"" + H.name() + "\"");
        }
      ...
      {
        bool is_BE = false;
        if (Raw::fetch_<int32_t> (&NH.sizeof_hdr, is_BE) != ver2_hdr_size) {
          is_BE = true;
          if (Raw::fetch_<int32_t> (&NH.sizeof_hdr, is_BE) != ver2_hdr_size)
            throw Exception ("image \"" + H.name() + "\" is not in NIfTI-2 format (sizeof_hdr != " + str(ver2_hdr_size) + ")");
        }
        ...
        if (memcmp (NH.magic+4, ver2_sig_extra, 4))
      ...
      void write (nifti_1_header& NH, const Header& H, bool has_nii_suffix)
      {
        ...
        // magic number:
        Raw::store<int32_t> (ver1_hdr_size, &NH.sizeof_hdr, is_BE);
        ...
        Raw::store<float32> (float32(ver1_hdr_with_ext_size), &NH.vox_offset, is_BE);
      ...
      void write (nifti_2_header& NH, const Header& H, const bool has_nii_suffix)
      {
        if (H.ndim() > 7)
        ...
        // magic number:
        Raw::store<int32_t> (ver2_hdr_size, &NH.sizeof_hdr, is_BE);

        strncpy ( (char*) &NH.magic, has_nii_suffix ? "n+2\0" : "ni2\0", 4);
        strncpy ( (char*) &NH.magic+4, ver2_sig_extra, 4);
        ...
        Raw::store<int64_t> (int64_t(ver2_hdr_with_ext_size), &NH.vox_offset, is_BE);
        ...
        // TODO Test to see whether this is indeed endian-flipped
