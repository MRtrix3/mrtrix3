...
#include "image/registration_symmetric/utils.h"
...
namespace MR
{
  namespace Image
  {
    namespace RegistrationSymmteric
    {
      namespace Transform
      {
          ...
          template <class DFVoxelType>
            class ThreadKernel {
              ...
              public:
                ...
                template <class InvDFVoxelType>
                  void operator() (InvDFVoxelType& inv_warp) {

                    Point<float> truth = transform.voxel2scanner (inv_warp);
                    Point<float> current;
          ...
          public:
            ...
            template <class DFVoxelType, class InvDFVoxelType>
              void operator() (DFVoxelType& warp, InvDFVoxelType& inv_warp) {
                ...
                ThreadKernel<BufferScratch<float>::voxel_type> 
                  warp_updator (warped_positions_vox, inv_warp, max_iter, error_tolerance);
                ThreadedLoop ("inverting warp field...", inv_warp, 0, 3)
                  .run (warp_updator, inv_warp);
              }
