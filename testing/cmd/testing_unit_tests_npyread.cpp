...
template <bool isboolean, bool is1D>
bool verify_basic (const std::string& filepath)
{
  if (is1D) {
...
namespace {
  ...
  bool check_shape (void* address, const File::NPY::ReadInfo& info)
  {
    ...
    Eigen::Map<Eigen::Array<T, Eigen::Dynamic, Eigen::Dynamic, Major>> map (reinterpret_cast<T*> (address), info.shape[0], info.shape[1]);
    if (info.data_type == DataType::Bit) {
      std::cerr << map << "\n";
    }
  ...
  template <typename T>
  bool check_major (void* address, const File::NPY::ReadInfo& info)
  {
    return info.column_major ?
           check_shape<T, Eigen::ColMajor> (address, info) :
  ...
  bool check_datatype (void* address, const File::NPY::ReadInfo& info)
  {
    ...
    if (type == DataType::Bit) return check_major<int8_t> (address, info);
    if (type == DataType::Int8) return check_major<int8_t> (address, info);
    if (type == DataType::UInt8) return check_major<uint8_t> (address, info);
    if (type == DataType::Int16) return check_major<int16_t> (address, info);
    if (type == DataType::UInt16) return check_major<uint16_t> (address, info);
    if (type == DataType::Float16) return check_major<half_float::half> (address, info);
    if (type == DataType::Int32) return check_major<int32_t> (address, info);
    if (type == DataType::UInt32) return check_major<uint32_t> (address, info);
    if (type == DataType::Float32) return check_major<float> (address, info);
    if (type == DataType::Int64) return check_major<int64_t> (address, info);
    if (type == DataType::UInt64) return check_major<uint64_t> (address, info);
    if (type == DataType::Float64) return check_major<double> (address, info);
    assert (false);
    return false;
  }
}
