...
void run() {
  ...
  for (size_t i = 0; i < buffer1.ndim(); ++i) {
    if (std::isfinite(buffer1.spacing(i)))
      if (buffer1.spacing(i) != buffer2.spacing(i))
        throw Exception("images \"" + buffer1.name() + "\" and \"" + buffer2.name() +
                        "\" do not have matching voxel spacings " + str(buffer1.spacing(i)) + " vs " +
                        str(buffer2.spacing(i)));
  }
  for (size_t i = 0; i < 3; ++i) {
    for (size_t j = 0; j < 4; ++j) {
      if (abs(buffer1.transform()(i, j) - buffer2.transform()(i, j)) > 0.001)
        throw Exception("images \"" + buffer1.name() + "\" and \"" + buffer2.name() +
                        "\" do not have matching header transforms " + "\n" + str(buffer1.transform().matrix()) +
  ...
  ThreadedLoop(buffer1).run(
      [&tol](decltype(buffer1) &a, decltype(buffer2) &b) {
