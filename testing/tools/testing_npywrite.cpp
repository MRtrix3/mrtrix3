...
template <typename T> void save_1d(const std::filesystem::path &path) {
  File::Matrix::save_vector(reference_1d.cast<T>(), argument[0] / path);
}

template <typename T> void save_2d(const std::filesystem::path &path) {
  File::Matrix::save_matrix(reference_2d.cast<T>(), argument[0] / path);
}
