...
public:
  UnitTest(string unit_name_arg) { unit_name = unit_name_arg; }
...
public:
  ...
  */
  void check(bool pass, std::string message = "(no message provided)") {
    if (!pass) {
      // We write info on the error here becasue C++ assert doesn't accept a string and other ways around this are
      // horrible language hacks
