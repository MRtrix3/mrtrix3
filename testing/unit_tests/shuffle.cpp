...
void run() {
  ...
  auto test_kernel = [&](const size_t requested_number,
    ...
    test(temp.size() == expected_number,
         "Incorrect number of shuffles; " + error_string + "; " + eb_string + "; " + test_string);
    if (eb_within.size())
  ...
  for (size_t exchange_index = 0; exchange_index != 3; ++exchange_index) {
    ...
    const index_array_type eb_within(exchange_t(exchange_index) == exchange_t::WITHIN ? block_indices
                                                                                      : index_array_type());
    const index_array_type eb_whole(exchange_t(exchange_index) == exchange_t::WHOLE ? block_indices
                                                                                    : index_array_type());
    ...
    for (size_t error_index = 0; error_index != 2; ++error_index) {
      ...
      test_kernel(max_num / 2,
                  ...
                  error_string,
                  eb_string,
                  "less than max shuffles",
                  true);
      test_kernel(
          max_num, max_num, error_type, eb_within, eb_whole, error_string, eb_string, "exactly max shuffles", true);
      test_kernel(2 * max_num,
                  max_num,
                  error_type,
                  eb_within,
                  eb_whole,
                  error_string,
                  eb_string,
                  "more than max shuffles",
                  true);
    }

    // BOTH
    {
      test_kernel(max_num_signflips / 2,
                  max_num_signflips / 2,
                  Shuffler::error_t::BOTH,
                  eb_within,
                  eb_whole,
                  "BOTH",
                  eb_string,
                  "less than max signflips",
                  true);
      test_kernel(max_num_signflips,
                  max_num_signflips,
                  Shuffler::error_t::BOTH,
                  eb_within,
                  eb_whole,
                  "BOTH",
                  eb_string,
                  "exactly max signflips",
                  true);
      test_kernel((max_num_signflips + max_num_permutations) / 2,
                  ...
                  "BOTH",
                  eb_string,
                  "between max signflips and max permutations",
                  true);
      test_kernel(max_num_permutations,
      ...
      test_kernel((max_num_permutations + max_num_combined) / 2,
                  ...
                  "BOTH",
                  eb_string,
                  "between max permutations and max shuffles",
                  false);
      test_kernel(max_num_combined,
                  max_num_combined,
                  Shuffler::error_t::BOTH,
                  eb_within,
                  eb_whole,
                  "BOTH",
                  eb_string,
                  "exactly max shuffles",
                  true);
      test_kernel(2 * max_num_combined,
                  max_num_combined,
                  Shuffler::error_t::BOTH,
                  eb_within,
                  eb_whole,
