...
void run() {
  ...
  for (size_t bits = 0; bits != 16; ++bits) {
    ...
    for (size_t iter_fillvalue = 0; iter_fillvalue != 2; ++iter_fillvalue) {
      ...
      case 0:
        test(data.empty(), "Zero-filled set of size " + str(bits) + " reported non-empty");
        break;
      case 1:
        test(data.full(), "One-filled set of size " + str(bits) + " reported non-full");
        break;
      default:
        assert(0);
      }
      for (size_t i = 0; i != bits; ++i) {
        ...
        switch (iter_fillvalue) {
        case 0:
          test(!data.full(), "Incompletely filled set of size " + str(bits) + " reported full");
          break;
        case 1:
          test(!data.empty(), "Incompletely zeroed set of size " + str(bits) + " reported empty");
          break;
        default:
          assert(0);
        }
        ...
        test(data.count() == target_count,
             "Data " + str(data) + " (count " + str(target_count) + ") erroneously reported as count " +
                 str(data.count()));
      }
      switch (iter_fillvalue) {
      case 0:
        test(data.full(), "Progressively filled set of size " + str(bits) + " reported non-full");
        break;
      case 1:
        test(data.empty(), "Progressively zeroed set of size " + str(bits) + " reported non-empty");
        break;
      default:
        assert(0);
      }
      ...
      test(identical(data, duplicate, bits),
           "Duplicated sets pre-resize of size " + str(bits) + " did not lead to identical data: " + str(data) + " " +
               str(duplicate));
      ...
      if (bits) {
        ...
        test(data != duplicate,
             "Change of one bit in size " + str(bits) + " not reported as inequal: " + str(data) + " " +
                 str(duplicate));
        duplicate[index_to_toggle] = set_value;
        test(data == duplicate,
             "Reversion of one changed bit in size " + str(bits) + " not reported as equal: " + str(data) + " " +
                 str(duplicate));
      }
      ...
      test(identical(data, duplicate, bits),
           "Duplicated sets post-resize of size " + str(bits) + " did not lead to identical data: " + str(data) + " " +
               str(duplicate));
      switch (iter_fillvalue) {
      case 0:
        test(valid_last(data, bits, false),
             "Resized (0's) full set of size " + str(bits) + " contains invalid zeroed excess data: " + str(data));
        test(valid_last(duplicate, bits, true),
             "Resized (1's) full set of size " + str(bits) + " contains invalid filled excess data: " + str(duplicate));
        break;
      case 1:
        test(valid_last(data, bits, false),
             "Resized (0's) empty set of size " + str(bits) + " contains invalid zeroed excess data: " + str(data));
        test(valid_last(duplicate, bits, true),
             "Resized (1's) empty set of size " + str(bits) +
                 " contains invalid filled excess data: " + str(duplicate));
        break;
      default:
        assert(0);
      }
