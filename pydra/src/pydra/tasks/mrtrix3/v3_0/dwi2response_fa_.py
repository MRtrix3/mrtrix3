...
input_fields = [
    ...
    (
        ...
        int,
        {
            "help_string": "Number of brain mask erosion steps to apply prior to threshold (not used if mask is provided manually)",
            "argstr": "-erode",
        },
    ),
    (
        "number",
        int,
        {"help_string": "The number of highest-FA voxels to use", "argstr": "-number"},
    ),
    (
        "threshold",
        float,
        {
            "help_string": "Apply a hard FA threshold, rather than selecting the top voxels",
            "argstr": "-threshold",
        },
    ),
    (
...
dwi2response_fa_input_spec = specs.SpecInfo(
    name="dwi2response_fa_input", fields=input_fields, bases=(specs.ShellSpec,)
)
...
dwi2response_fa_output_spec = specs.SpecInfo(
    name="dwi2response_fa_output", fields=output_fields, bases=(specs.ShellOutSpec,)
)


class dwi2response_fa(ShellCommandTask):
    """
            References
        ----------
    ...
    input_spec = dwi2response_fa_input_spec
    output_spec = dwi2response_fa_output_spec
    executable = ("dwi2response", "fa")
