...
input_fields = [
    ...
    (
        ...
        bool,
        {
            "help_string": "(EXPERIMENTAL) Iteratively refine the weights for combination of per-shell trace-weighted images prior to thresholding",
            "argstr": "-iterative",
        },
    ),
    (
        ...
        int,
        {
            "help_string": "Set the maximum number of iterations for the algorithm (default: 10)",
            "argstr": "-max_iters",
        },
    ),
    (
        ...
        typing.Any,
        {
            "help_string": "Comma separated list of shells used to generate trace-weighted images for masking",
            "argstr": "-shells",
        },
    ),
    (
        ...
        int,
        {
            "help_string": "the maximum scale used to cut bridges. A certain maximum scale cuts bridges up to a width (in voxels) of 2x the provided scale. Setting this to 0 disables the mask cleaning step. (Default: 2)",
            "argstr": "-clean_scale",
        },
    ),
    (
...
dwi2mask_trace_input_spec = specs.SpecInfo(
    name="dwi2mask_trace_input", fields=input_fields, bases=(specs.ShellSpec,)
)

output_fields = [
    ("output", FsObject, {"help_string": "The output mask image", "mandatory": True})
]
dwi2mask_trace_output_spec = specs.SpecInfo(
    name="dwi2mask_trace_output", fields=output_fields, bases=(specs.ShellOutSpec,)
)


class dwi2mask_trace(ShellCommandTask):
    """
            References
        ----------
    ...
    input_spec = dwi2mask_trace_input_spec
    output_spec = dwi2mask_trace_output_spec
