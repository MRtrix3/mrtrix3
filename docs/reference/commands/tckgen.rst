...
- BallSticks: A probabilistic streamlines algorithm that operates on data from the ball-and-sticks model; specifically, the multiple bootstrap realisations of that model that are generated by the FSL "bedpostx" command. While the empirical behaviour of this algorithm is intended to mimic that of the tractography algorithm implemented in the FSL "probtrackx" command, equivalence of the two is in no way guaranteed, and default parameters differ between the two implementations. This algorithm expects as input the path to the directory that is generated by the FSL command "bedpostx".
...
-  **-algorithm name** specify the tractography algorithm to use. Valid choices are: BallSticks, FACT, iFOD1, iFOD2, Nulldist1, Nulldist2, SD_Stream, Seedtest, Tensor_Det, Tensor_Prob (default: iFOD2).
...
-  **-angle theta** set the maximum angle in degrees between successive steps (defaults: 60 for deterministic algorithms & BallSticks; 15 for iFOD1 / nulldist1; 45 for iFOD2 / nulldist2)
...
-  **-cutoff value** set the FOD amplitude / fixel size / tensor FA cutoff for terminating tracks (defaults: 0.01 volume fraction for BallSticks; 0.1 for FOD-based algorithms; 0.1 for fixel-based algorithms; 0.1 for tensor-based algorithms; threshold multiplied by 0.5 when using ACT).

-  **-trials number** set the maximum number of sampling trials at each point (only used for iFOD1 / iFOD2) (default: 1000).

-  **-noprecomputed** do NOT pre-compute legendre polynomial values for FOD amplitude sampling. Warning: this will slow down the algorithm by a factor of approximately 4.
...
Options specific to the BallSticks tracking algorithms
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

-  **-seed_fixel choice** control which fixel is chosen for commencement of tracking from a given seed location; options are: first, largest, proportional, random_fixel, random_dir
...
* BallSticks: |br|
  Behrens, T. E. J.; Woolrich, M. W.; Jenkinson, M.; Johansen-Berg, H.; Nunes, R. G.; Clare, S.; Matthews, P. M.; Brady, J. M. & Smith, S. M. Characterization and propagation of uncertainty in diffusion-weighted MR imaging. Magnetic Resonance in Medicine, 2003, 50, 1077-1088  |br|
  Behrens, T. E. J.; Johansen-Berg, H.; Jbabdi, S.; Rushworth, M. F. S & Woolrich, M. W. Probabilistic diffusion tractography with multiple fibre orientations: What can we gain? NeuroImage, 2007, 34, 144-155
