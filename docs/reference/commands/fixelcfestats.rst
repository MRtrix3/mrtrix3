...
-  *connectivity*: the data used to determine fixel-fixel connectivity; either a pre-computed fixel-fixel connectivity matrix, or a whole-brain tractogram (see Description)
-  *out_fixel_directory*: the output directory where results will be saved. Will be created if it does not exist

Description
-----------

Fixel connectivity information for this command can be initialised in one of two ways, depending on what data are provided as the fifth input to the command. If a track file is provided here (as is the original functionality of the fixelcfestats command), then the command will use this track file to construct two fixel-fixel connectivity matrices: One for the purpose of smoothing the input data (which it will automatically perform prior to permutation testing), and one for the Connectivity-based Fixel Enhancement (CFE) algorithm. If a text file is instead provided, this will be interpreted as a pre-computed fixel-fixel connectivity matrix (e.g. as generated by the MRtrix3 command fixelconnectivity), and used directly; furthermore, it will be assumed that the data provided as input to the command have already had any desired smoothing applied (e.g. using the MRtrix3 command fixelfilter).
...
Options for fixel-fixel connectivity matrix generation & usage from an input track file
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
