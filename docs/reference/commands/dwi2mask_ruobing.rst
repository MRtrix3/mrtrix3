...
.. Generating a brain mask from Synthstrip method (a skull-stripping method that extracts brain signal from a landscape of image types byleveraging anatomical
.. label maps to synthesize a broad set of training images, optimizing a
.. robust convolutional neural network that is agnostic to MRI contrasts
.. and acquisition schemes)
...
.. Options for Synthstrip function arguments
.. ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

.. - **-h** Display the help message and exit.

.. - **-i filename.nii** Input image to skullstrip.

.. - **-o filename.nii** Output/save the stripped image to current path.

.. - **-m filename.nii** Output/save the binary brain mask from stripping to current path.

.. - **-g** Use the GPU.

.. - **-b BORDER**  Mask border threshold, unit in mm. Default value is 1mm.

.. - **--no-csf** Excluede CSF from brain border.

.. - **_i filename.nii** Alternative model weights.
