...
For -in_format and -out_format options, the choices are: - "unitspherical": Each orientation is represented using 2 sequential volumes encoded as azimuth and inclination angles in radians; - "spherical": Each orientation and associated value is represented using 3 sequential volumes, with associated value ("radius") first, followed by aximuth and inclination angles in radians; - "unitcartesian": Each orientation is represented using 3 sequential volumes encoded as three dot products with respect to three orthogonal reference axes; - "cartesian": Each orientation and associated non-negative value is represented using 3 sequential volumes, with the norm of that 3-vector encoding the associated value and the unit-normalised vector encoding the three dot products with respect to three orthogonal reference axes. The default behaviour throughout MRtrix3 is to interpret data as either "unitcartesian" or "cartesian" depending upon the context and/or presence of non-unit norm vectors in the data.
