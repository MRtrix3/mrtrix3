...
-  *operation*: the operation to apply, one of: mean, median, sum,
   product, rms, var, std, min, max, absmax, magmax.
...
along a specified axis for a single image. Supported operations are:

mean, median, sum, product, rms (root-mean-square value), var (unbiased
variance), std (unbiased standard deviation), min, max, absmax (maximum
absolute value), magmax (value with maximum absolute value, preserving
its sign).
...
-  **-axis index** perform operation along a specified axis of a single
   input image
