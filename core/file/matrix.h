  ...
  namespace File
  {
    ...
    namespace Matrix
    {
      ...
      namespace
      {
        ...
        void save_matrix_text (const MatrixType& M, const std::string& filename, const KeyValues& keyvals = KeyValues(), const bool add_to_command_history = true)
        {
          ...
          while (getline (stream, sbuf)) {
            ...
            if (V.size() > 1)
              if (V.back().size() != V[0].size())
                throw Exception ("uneven rows in matrix text file \"" + filename + "\" " +
        ...
        void save_vector_text (const VectorType& V, const std::string& filename, const KeyValues& keyvals, const bool add_to_command_history)
        {
