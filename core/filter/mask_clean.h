...
public:
  template <class HeaderType> MaskClean(const HeaderType &in) : Base(in), scale(2) {
  ...
  void set_scale(unsigned int scales) { scale = scales; }
...
protected:
  ...
  template <class InputImageType, class OutputImageType> bool differ(InputImageType &ima, OutputImageType &imb) {
