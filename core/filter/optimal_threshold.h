...
template <class ImageType, class MaskType> class ImageCorrelationCostFunction {
...
typename ImageType::value_type estimate_optimal_threshold(ImageType &input, MaskType &mask) {
  ...
  input_value_type optimal_threshold = 0.0;
  {
    ...
    optimal_threshold = Math::golden_section_search(
        cost_function, "optimising threshold", min + 0.001 * (max - min), 0.5 * (min + max), max - 0.001 * (max - min));
  }

  return optimal_threshold;
}

template <class ImageType> inline typename ImageType::value_type estimate_optimal_threshold(ImageType &input) {
  Image<bool> mask;
...
public:
  OptimalThreshold(const Header &H) : Base(H) { datatype_ = DataType::Bit; }
