...
std::unique_ptr<ImageIO::Base> TIFF::read(Header &H) const {
  if (!(Path::has_suffix(H.name(), ".tiff") || Path::has_suffix(H.name(), ".tif") ||
        Path::has_suffix(H.name(), ".TIFF") || Path::has_suffix(H.name(), ".TIF")))
...
std::unique_ptr<ImageIO::Base> TIFF::create(Header &H) const { return std::unique_ptr<ImageIO::Base>(); }
