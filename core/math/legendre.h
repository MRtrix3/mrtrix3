...
template <typename ValueType> inline ValueType double_factorial(const ValueType n) {
...
namespace {
template <typename ValueType> inline ValueType Plm_sph_helper(const ValueType x, const ValueType m) {
  return (m < 1.0 ? 1.0 : x * (m - 1.0) / m * Plm_sph_helper(x, m - 2.0));
}
} // namespace

template <typename ValueType> inline ValueType Plm_sph(const int l, const int m, const ValueType x) {
  ValueType x2 = pow2(x);
  if (m && x2 >= 1.0)
    return (0.0);
  ValueType v0 = 0.282094791773878;
  if (m)
    v0 *= std::sqrt((2 * m + 1) * Plm_sph_helper(1.0 - x2, 2.0 * m));
  if (m & 1)
    v0 = -v0;
  if (l == m)
    return (v0);
...
/** \note upon completion, the (l,m) value will be stored in \c array[l]. Entries in \a array for l<m will be left
 * undefined. */
...
inline void Plm_sph_deriv(VectorType &array, const int lmax, const int m, const typename VectorType::Scalar x) {
  ...
  for (int n = lmax; n > m; n--)
    array[n] = x2 * (n * x * array[n] -
                     (n + m) * std::sqrt((2.0 * n + 1.0) * (n - m) / ((2.0 * n - 1.0) * (n + m))) * array[n - 1]);
  array[m] *= x2 * m * x;
}
