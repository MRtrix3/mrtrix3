...
namespace MR
{
  namespace Math
  {
    namespace SH
    {
      ...
      template <typename ValueType>
      class ApodizedBase
      { MEMALIGN(ApodizedBase<ValueType>)
        public:
          ApodizedBase (const size_t lmax) :
              lmax (lmax),
              RH (Eigen::Matrix<ValueType,Eigen::Dynamic,1>::Zero (lmax/2 + 1)) { }
      ...
      { MEMALIGN(aPSF<ValueType>)
        public:
          aPSF (const size_t lmax) :
              ApodizedBase<ValueType> (lmax)
          {
            switch (lmax) {
              case 2:
                ApodizedBase<ValueType>::RH << 1.00000000,
                                               0.41939279;
                break;
              case 4:
                ApodizedBase<ValueType>::RH << 1.00000000,
