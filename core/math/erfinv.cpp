...
default_type erfinv(const default_type p) {
  ...
  public:
    Shared() : m_Y(0.0891314744949340820313) {
      ...
      m_Q << 1.0, -0.970005043303290640362, -1.56574558234175846809, 1.56221558398423026363, 0.662328840472002992063,
...
default_type erfcinv(const default_type q) {
  ...
  public:
    Shared() : N(6), m_Y(N), m_P(N), m_Q(N) {
      ...
      m_P[0].resize(9);
      m_P[0] << -0.202433508355938759655, 0.105264680699391713268, 8.37050328343119927838, 17.6447298408374015486,
          -18.8510648058714251895, -44.6382324441786960818, 17.445385985570866523, 21.1294655448340526258,
          -3.67192254707729348546;
      m_Q[0].resize(9);
      m_Q[0] << 1.0, 6.24264124854247537712, 3.9713437953343869095, -28.6608180499800029974, -20.1432634680485188801,
          48.5609213108739935468, 10.8268667355460159008, -22.6436933413139721736, 1.72114765761200282724;
      // Region 1: x < 3
      m_Y[1] = 0.807220458984375;
      m_P[1].resize(11);
      m_P[1] << -0.131102781679951906451, -0.163794047193317060787, 0.117030156341995252019, 0.387079738972604337464,
      ...
      m_Q[1].resize(8);
      m_Q[1] << 1.0, 3.46625407242567245975, 5.38168345707006855425, 4.77846592945843778382, 2.59301921623620271374,
          0.848854343457902036425, 0.152264338295331783612, 0.01105924229346489121;
      // Region 2: x < 6
      m_Y[2] = 0.93995571136474609375;
      m_P[2].resize(9);
      m_P[2] << -0.0350353787183177984712, -0.00222426529213447927281, 0.0185573306514231072324,
          0.00950804701325919603619, 0.00187123492819559223345, 0.000157544617424960554631, 0.460469890584317994083e-5,
          -0.230404776911882601748e-9, 0.266339227425782031962e-11;
      m_Q[2].resize(7);
      m_Q[2] << 1.0, 1.3653349817554063097, 0.762059164553623404043, 0.220091105764131249824, 0.0341589143670947727934,
          0.00263861676657015992959, 0.764675292302794483503e-4;
      // Region 3: x < 18
      m_Y[3] = 0.98362827301025390625;
      m_P[3].resize(9);
      m_P[3] << -0.0167431005076633737133, -0.00112951438745580278863, 0.00105628862152492910091,
      ...
      m_Q[3].resize(7);
      m_Q[3] << 1.0, 0.591429344886417493481, 0.138151865749083321638, 0.0160746087093676504695,
          0.000964011807005165528527, 0.275335474764726041141e-4, 0.282243172016108031869e-6;
      // Region 4: x < 44
      m_Y[4] = 0.99714565277099609375;
      m_P[4].resize(8);
      m_P[4] << -0.0024978212791898131227, -0.779190719229053954292e-5, 0.254723037413027451751e-4,
      ...
      m_Q[4].resize(7);
      m_Q[4] << 1.0, 0.207123112214422517181, 0.0169410838120975906478, 0.000690538265622684595676,
          0.145007359818232637924e-4, 0.144437756628144157666e-6, 0.509761276599778486139e-9;
      // Region 5: x >= 44
      m_Y[5] = 0.99941349029541015625;
      m_P[5].resize(8);
      m_P[5] << -0.000539042911019078575891, -0.28398759004727721098e-6, 0.899465114892291446442e-6,
      ...
      m_Q[5].resize(7);
      m_Q[5] << 1.0, 0.0845746234001899436914, 0.00282092984726264681981, 0.468292921940894236786e-4,
