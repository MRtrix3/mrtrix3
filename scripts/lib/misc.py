...
def haveBinary(cmdname):
  ...
  for path_dir in os.environ["PATH"].split(os.pathsep):
    ...
    if os.path.isfile(full_path) and os.access(full_path, os.X_OK):
      lib.message.debug('Command \'' + cmdname + '\' found in PATH')
      return True
  lib.message.debug('Command \'' + cmdname + '\' NOT found in PATH')
  return False
...
def isWindows():
  ...
  lib.message.debug('System = ' + system + '; is Windows? ' + str(result))
  return result
