...
def getInputFiles():
  ...
  if os.path.exists(mask_path):
    lib.message.warn('-mask option is ignored by algorithm \'manual\'')
    os.remove(mask_path)
  runCommand('mrconvert ' + lib.path.fromUser(lib.app.args.in_voxels, True) + ' ' + os.path.join(lib.app.tempDir, 'in_voxels.mif'))
...
def execute():
  ...
  if lib.app.args.lmax:
    ...
    if not len(lmax) == len(shells):
      lib.message.error('Number of manually-defined lmax\'s (' + str(len(lmax)) + ') does not match number of b-value shells (' + str(len(shells)) + ')')
    for l in lmax:
      if l%2:
