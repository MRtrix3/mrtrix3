...
ppi <- 800
png("/home/dave/Gdrive/Documents/JournalPapers/CFE/Figures/Artificial/effect.png", width=7*ppi, height=4*ppi, res = ppi)
all_data <- read.csv('/raid1/CFE_backup/aucdata.csv');
sub <- subset(all_data, (SNR == 1 | SNR == 2 | SNR == 3) & smoothing == 10 & C == 0.5, select = c (smoothing, ROI, SNR, C, E, H, IQR25, AUC, IQR75))
sub$H <- factor(sub$H)
sub$E <- factor(sub$E)
...
print(ggplot(data=sub,  aes(x=E, y=H)) + geom_tile(aes(fill = AUC), colour = "white") 
      ...
      + facet_grid(SNR ~ ROI, labeller = mf_labeller) 
