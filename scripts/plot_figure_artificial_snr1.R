...
ppi <- 800
png("/home/dave/Gdrive/Documents/JournalPapers/CFE/Figures/Artificial/snr1", width=7*ppi, height=6*ppi, res = ppi)
all_data <- read.csv('/raid1/CFE_backup/aucdata.csv');
sub <- subset(all_data, SNR == 1 & smoothing == 10, select = c (smoothing, ROI, SNR, C, E, H, IQR25, AUC, IQR75))
sub$H <- factor(sub$H)
sub$E <- factor(sub$E)
