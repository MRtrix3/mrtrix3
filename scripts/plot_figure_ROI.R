...
source('multiplot.R')
png("/home/dave/Gdrive/Documents/JournalPapers/CFE/Figures/AUC_SNR1_smooth10.png", width=210, height=260, units = 'mm', res = 350)
all_data <- read.csv('/data/dave/cfe/experiment_2_sims/artificial2/all_data.csv');
sub <- subset(all_data, SNR == 1 & smoothing == 10, select = c (ROI, C, E, H, IQR25, AUC, IQR75))
sub$H <- factor(sub$H)
