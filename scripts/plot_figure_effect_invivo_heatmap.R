...
source('multiplot.R')
ppi <- 400
png("/home/dave/Gdrive/Documents/JournalPapers/CFE/Figures/invivo2/AUC_heat_effect1_smooth10.png", width=9*ppi, height=5*ppi, res = ppi)
all_data <- read.csv('/data/dave/cfe/experiment_2_sims/invivo2/aucdata.csv');
sub <- subset(all_data, smoothing == 10 & C == 0, select = c (smoothing, ROI, effect, C, E, H, IQR25, AUC, IQR75))
sub$H <- factor(sub$H)
sub$E <- factor(sub$E)
...
print(ggplot(data=sub,  aes(x=E, y=H)) + geom_tile(aes(fill = AUC), colour = "white") 
      ...
      + facet_grid(effect ~ ROI, labeller = mf_labeller) 
