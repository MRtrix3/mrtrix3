...
mf_labeller <- function(var, value){ 
  ...
  if (var=="ROI") {
    ...
    value[value=="ad"]   <- "Alzheimers-like"
  }
  if (var=="C") {
    value[value=="0"]   <- "C=0"
...
source('multiplot.R')
ppi <- 400
png("/home/dave/Gdrive/Documents/JournalPapers/CFE/Figures/invivo2/AUC_heat_effect1_smooth10.png", width=7*ppi, height=6*ppi, res = ppi)
...
print(ggplot(data=sub,  aes(x=E, y=H)) + geom_tile(aes(fill = AUC), colour = "white") 
      + scale_fill_gradientn(colours=c("black","blue","cyan","yellow","red"), breaks=seq(0,.3,by=0.1)) 
