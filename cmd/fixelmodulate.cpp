...
void usage ()
{
  ...
  DESCRIPTION
  + "Compute the log of the modulation scale factor for each fixel using input jacobian matrices. Used for fixel-based morphometry";
  ...
  + Argument ("jacobian", "the image of jacobian matrices").type_image_in()
...
void run() {
  ...
  for (loop.start (mask_vox, output_vox, jacobian_vox); loop.ok(); loop.next (mask_vox, output_vox, jacobian_vox)) {
    ...
    for (size_t f = 0; f != mask_vox.value().size(); ++f) {
      ...
      output_vox.value()[f].value = Math::determinant (jacobian_matrix) / Math::norm (v_transformed);
    }
  }
}
