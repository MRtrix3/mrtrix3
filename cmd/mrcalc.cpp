...
BINARY_OP(
    ...
    "(%1 == %2)",
    COMPLEX_MAPS_TO_REAL,
    "equal-to operator (true=1, false=0)",
    { return a == b; },
    { return a == b; })
BINARY_OP(
    neq,
    "(%1 != %2)",
    COMPLEX_MAPS_TO_REAL,
    "not-equal-to operator (true=1, false=0)",
    { return a != b; },
    { return a != b; })
...
TERNARY_OP(
    ...
    { return a ? b : c; },
    { return is_true(a) ? b : c; })
TERNARY_OP(
    replace,
    "(%1, %2 -> %3)",
    NORMAL,
    "Wherever first operand is equal to the second operand, replace with third operand",
    { return (a == b) ? c : a; },
    { return (a == b) ? c : a; })
...
UNARY_OP(
    ...
    "!%1",
    NORMAL,
    "NOT operator: true (1) if operand is false (i.e. zero)",
    { return !v; },
    { return !is_true(v); })
BINARY_OP(
        and,
        "(%1 && %2)",
        NORMAL,
        "AND operator: true (1) if both operands are true (i.e. non-zero)",
        { return a && b; },
...
BINARY_OP(
    ...
    "(%1 || %2)",
    NORMAL,
    "OR operator: true (1) if either operand is true (i.e. non-zero)",
    { return a || b; },
    { return is_true(a) || is_true(b); })
BINARY_OP(
    xor,
    "(%1 ^^ %2)",
    NORMAL,
    "XOR operator: true (1) if only one of the operands is true (i.e. non-zero)",
    {
      return (!a) != (!b);
    },
    { return is_true(a) != is_true(b); })
...
UNARY_OP(
    ...
    "isnan (%1)",
    COMPLEX_MAPS_TO_REAL,
    "true (1) if operand is not-a-number (NaN)",
    { return std::isnan(v); },
...
UNARY_OP(
    ...
    "isinf (%1)",
    COMPLEX_MAPS_TO_REAL,
    "true (1) if operand is infinite (Inf)",
    { return std::isinf(v); },
...
UNARY_OP(
    ...
    "finite (%1)",
    COMPLEX_MAPS_TO_REAL,
    "true (1) if operand is finite (i.e. not NaN or Inf)",
...
public:
  ...
  Chunk &next() {
    ...
    if (item.image)
      load(item.chunk, *item.image);
    return item.chunk;
  }

  void reset(const Iterator &current_position) {
    current = 0;
...
class LoadedImage {
public:
  LoadedImage(std::shared_ptr<Image<complex_type>> &i, const bool c) : image(i), image_is_complex(c) {}
...
inline Chunk &StackEntry::evaluate(ThreadLocalStorage &storage) const {
  if (evaluator)
    return evaluator->evaluate(storage);
  if (rng) {
...
public:
  ...
  virtual Chunk &evaluate(Chunk &a, Chunk &b) const {
    Chunk &out(a.empty() ? b : a);
    if (operands[0].is_complex() || operands[1].is_complex()) {
      for (size_t n = 0; n < out.size(); ++n)
        out[n] = op.Z(a.empty() ? a.value : a[n], b.empty() ? b.value : b[n]);
    } else {
      for (size_t n = 0; n < out.size(); ++n)
        out[n] = op.R(a.empty() ? a.value.real() : a[n].real(), b.empty() ? b.value.real() : b[n].real());
    }
    return out;
  }
};
    ...
    if (operands[0].is_complex() || operands[1].is_complex() || operands[2].is_complex()) {
      ...
      for (size_t n = 0; n < out.size(); ++n)
        out[n] = op.R(a.empty() ? a.value.real() : a[n].real(),
                      b.empty() ? b.value.real() : b[n].real(),
                      c.empty() ? c.value.real() : c[n].real());
    }
    return out;
  }
};
...
void binary_operation(const std::string &operation_name, std::vector<StackEntry> &stack, Operation operation) {
  ...
  if (a.evaluator || a.image || a.rng || b.evaluator || b.image || b.rng) {
    ...
    a.value = (a.value.imag() == 0.0 && b.value.imag() == 0.0 ? operation.R(a.value.real(), b.value.real())
