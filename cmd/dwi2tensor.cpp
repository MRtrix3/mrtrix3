...
public:
  Processor(const Eigen::MatrixXd &A,
            const Eigen::MatrixXd &Aneq,
            const bool ols,
            const int iter,
            const MASKType &mask_image,
            const B0Type &b0_image,
            const DTType &dt_image,
            const DKTType &dkt_image,
            const PredictType &predict_image)
  ...
  template <class DWIType> void operator()(DWIType &dwi_image) {
    if (mask_image.valid()) {
...
inline Processor<MASKType, B0Type, DTType, DKTType, PredictType> processor(const Eigen::MatrixXd &A,
                                                                           const Eigen::MatrixXd &Aneq,
                                                                           const bool ols,
                                                                           const int iter,
                                                                           const MASKType &mask_image,
...
void run() {
  ...
  if (constrain) {
    ...
    if (dki) {
      ...
      // Aneq.block(tmp.rows()*0,1,tmp.rows(), 6) =                tmp.block(0,1,tmp.rows(), 6); --> redundant
      // constraint
