...
void usage ()
{
  ...
  DESCRIPTION
  + "generate an image for visualisation purposes from a four-tissue-type segmented anatomical image";
...
void run ()
{
  ...
  Image::Buffer<float> in (argument[0]);
  if (in.ndim() != 4 || in.dim (3) != 4)
    throw Exception ("Input image is not a true four-tissue-type segmented image");
  ...
  for (loop.start (v_in, v_out); loop.ok(); loop.next (v_in, v_out)) {
    ...
    const float bg = 1.0 - (t.get_cgm() + t.get_sgm() + t.get_wm() + t.get_csf());
    v_out.value() = (bg_multiplier * bg) + (cgm_multiplier * t.get_cgm()) + (sgm_multiplier * t.get_sgm()) + (wm_multiplier * t.get_wm()) + (csf_multiplier * t.get_csf());
  }
