...
enum MetricType {MeanSquared, CrossCorrelation, AngularCorrelation};
const char* metric_choices[] = { "diff", "ncc", "ac", NULL };
...
void usage ()
{
  ...
  + Option ("space",
    ...
    + Option ("metric",
        ...
        "Choices: diff (squared differences), ncc (normalised negative cross correlation), ac (angular cross correlation). Default: diff). "
        "ncc and ac are only implemented for -space average and -interp linear and cubic.")
    ...
    + Option ("lmax",
        "lmax for assessment of AC. Default: 4.")
...
void run ()
{
  ...
  if (opt.size()) {
    lmax = opt[0][0];
    selected_lmax_for_ac_metric = true;
  }

  MetricType metric_type = MetricType::MeanSquared;
  opt = get_options ("metric");
  if (opt.size()) {
    if (int(opt[0][0]) == 1) { // NCC
      if (space != 3)
        throw Exception ("NCC metric only implemented for use in average space");
      if (interp != 1 and interp != 2)
        throw Exception ("NCC metric only implemented for use with linear and cubic interpolation");
      metric_type = MetricType::CrossCorrelation;
    }
    if (int(opt[0][0]) == 2) { // AC
      if (space != 3)
        throw Exception ("AC metric only implemented for use in average space");
      if (interp != 1 and interp != 2)
        throw Exception ("AC metric only implemented for use with linear and cubic interpolation");
      metric_type = MetricType::AngularCorrelation;
    }
  }
  ...
  bool is_possible_fod_image = input1.ndim() == 4 && input1.size(3) >= 6 &&
  input1.size(3) == (int) Math::SH::NforL (Math::SH::LforN (input1.size(3)));
  const size_t nvols_1 = input1.ndim() == 3 ? 1 : input1.size(3);
  size_t image_lmax = Math::SH::LforN (nvols_1);
  size_t order_for_ac = Math::SH::NforL (lmax);
    
  if (is_possible_fod_image) {
    INFO ("detected FOD image");
    if (lmax > image_lmax || lmax < 2)
      throw Exception ("selected lmax should not exceed the input image lmax or be smaller than 2");
  } else {
    if (metric_type == MetricType::AngularCorrelation)
      throw Exception ("AC metric requires input FOD images");
  }
  if (selected_lmax_for_ac_metric) {
      if (metric_type != MetricType::AngularCorrelation)
          throw Exception ("lmax should be specified only for AC metric");
  }
  ...
  if (space==0) {
    ...
    if (space == 3) {
      ...
      Header midway_image_header = compute_minimum_average_header (input1, input2, transform.get_transform_half_inverse(), transform.get_transform_half());
      ...
      using CubicParamTypeWithGradient = Registration::Metric::Params <
        ...
        if (interp == 1 or interp == 2) {
          ...
          } else if ( metric_type == MetricType::AngularCorrelation) {
            nonormalisation = true;
            cost = Eigen::Matrix<value_type, Eigen::Dynamic, 1>::Zero (1, 1);
            Registration::Metric::GlobalAngularCorrelation4D metric;
            if (interp == 1) {
              LinearParamTypeWithGradient parameters (transform, input1, input2, midway_image, mask1, mask2);
              metric.precompute (parameters, order_for_ac);
              cost[0] = metric.get_ac();
            } else if (interp == 2) {
              CubicParamTypeWithGradient parameters (transform, input1, input2, midway_image, mask1, mask2);
              metric.precompute (parameters, order_for_ac);
              cost[0] = metric.get_ac();
  ...
  if (n_voxels==0 && metric_type != MetricType::AngularCorrelation && metric_type != MetricType::CrossCorrelation)
