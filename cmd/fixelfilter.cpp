...
void usage() {
  ...
  +Argument("input", "the input: either a fixel data file, or a fixel directory (see Description)").type_various() +
      Argument("filter", "the filtering operation to perform; options are: " + join(filters, ", "))
          .type_choice(filters) +
  ...
  +Option("matrix", "provide a fixel-fixel connectivity matrix for filtering operations that require it").required() +
      ...
      Option("threshold_value",
             "specify a threshold for the input fixel data file values "
             "(default = " +
                 str(DEFAULT_FIXEL_CONNECT_VALUE_THRESHOLD) + ")") +
      Argument("value").type_float() +
      Option("threshold_connectivity",
             "specify a fixel-fixel connectivity threshold for connected-component analysis "
             "(default = " +
                 str(DEFAULT_FIXEL_CONNECT_CONNECTIVITY_THRESHOLD) + ")") +
      Argument("value").type_float(0.0)

      + OptionGroup("Options specific to the \"smooth\" filter") +
      Option("fwhm",
             "the full-width half-maximum (FWHM) of the spatial component of the smoothing filter "
             "(default = " +
                 str(DEFAULT_FIXEL_SMOOTHING_FWHM) + "mm)") +
      Argument("value").type_float(0.0) +
      Option("minweight",
             "apply a minimum threshold to smoothing weights "
             "(default = " +
                 str(DEFAULT_FIXEL_SMOOTHING_MINWEIGHT) + ")") +
      Argument("value").type_float(0.0) +
      Option("mask", "only perform smoothing within a specified binary fixel mask") + Argument("image").type_image_in();
}
