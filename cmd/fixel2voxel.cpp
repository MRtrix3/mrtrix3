...
struct LoopFixelsInVoxelWithMax {
  ...
  template <class... DataType> FORCE_INLINE Run<DataType...> operator()(DataType &...data) const {
    return {num_fixels, max_fixels, offset, std::tie(data...)};
  }
};
...
public:
  Base(FixelDataType &data, const index_type max_fixels, const bool pad = false, const float pad_value = 0.0)
      : data(data), max_fixels(max_fixels), pad(pad), pad_value(pad_value) {}

protected:
  FORCE_INLINE LoopFixelsInVoxelWithMax Loop(FixelIndexType &index) {
    index.index(3) = 0;
...
public:
  Mean(FixelDataType &data, const index_type max_fixels, FixelDataType &vol) : Base(data, max_fixels), vol(vol) {}
...
public:
  Sum(FixelDataType &data, const index_type max_fixels, FixelDataType &vol) : Base(data, max_fixels), vol(vol) {}
...
public:
  Product(FixelDataType &data, const index_type max_fixels) : Base(data, max_fixels) {}
...
public:
  Min(FixelDataType &data, const index_type max_fixels) : Base(data, max_fixels) {}
...
public:
  Max(FixelDataType &data, const index_type max_fixels) : Base(data, max_fixels) {}
...
public:
  Complexity(FixelDataType &data, const index_type max_fixels) : Base(data, max_fixels) {}
...
public:
  SF(FixelDataType &data, const index_type max_fixels) : Base(data, max_fixels) {}
...
public:
  DEC_unit(FixelDataType &data, const index_type max_fixels, FixelDataType &vol, Image<float> &dir)
      : Base(data, max_fixels), vol(vol), dir(dir) {}
