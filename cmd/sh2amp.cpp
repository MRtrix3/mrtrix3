...
void usage ()
{
  ...
  DESCRIPTION
    ...
    + "If a full DW encoding is provided, the number of shells needs to match "
      ...
      "dimension). If needed, the -shells option can be used to pick out the "
  ...
  ARGUMENTS
    ...
    + Argument ("directions",
                "the source of the set of directions along which the SH functions "
                "will be sampled (see Description)").type_various()
    + Argument ("output",
...
void run ()
{
  ...
  if (directions.cols() == 2 || directions.cols() == 3) { // single-shell:
