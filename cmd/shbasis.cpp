...
template <typename value_type> void check_and_update(Header &H, const conv_t conversion) {
  ...
  for (size_t l = 2; l <= lmax; l += 2) {
    ...
    for (image.index(3) = ssize_t(Math::SH::NforL(l - 2)); image.index(3) != ssize_t(Math::SH::NforL(l));
         ++image.index(3)) {
      double sum = 0.0;
  ...
  DEBUG("Power ratio for assessing SH basis is " + str(power_ratio) + " as " +
        (lmax < 8 ? "derived from" : "regressed to") + " l=" + str(l_for_decision));
...
void run() {
  ...
  if (!opt.empty()) {
    ...
    case 1:
      ...
      break;
    case 2:
      conversion = FORCE_OLDTONEW;
      break;
    case 3:
      conversion = FORCE_NEWTOOLD;
      break;
    default:
      assert(0);
      break;
