...
void run() {
  ...
  if (count) {
    if (weights_provided) {
      ...
      while (sum > 0.5 * sum_weights) {
        sum -= all_lengths[++median_index].get_weight();
      }
  ...
  if (!fields.empty()) {
    ...
    std::cout << " " << std::setw(width) << std::right << (mean_length) << " " << std::setw(width) << std::right
              << (median_length) << " " << std::setw(width) << std::right << (stdev) << " " << std::setw(width)
              << std::right << (min_length) << " " << std::setw(width) << std::right << (max_length) << " "
              << std::setw(width) << std::right << (count) << "\n";
  }
