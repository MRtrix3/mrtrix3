...
void run () {
  ...
  if (contrast == TDI || contrast == ENDPOINT || contrast == LENGTH || contrast == INVLENGTH || (contrast == CURVATURE && stat_tck != GAUSSIAN)) {
    ...
    if (colour) {
      ...
      Thread::run_queue_threaded_pipe (loader, Tractography::Streamline<float>(), mapper, SetVoxelDEC(), writer);
    } else {
      ...
      Thread::run_queue_threaded_pipe (loader, Tractography::Streamline<float>(), mapper, SetVoxel(), *writer);
    }
    ...
    if (colour) {
      ...
      Thread::run_queue_threaded_pipe (loader, Tractography::Streamline<float>(), mapper, SetVoxelDir(), writer);
    } else {
      TrackMapperTWI <       SetVoxelDir> mapper (header, upsample_ratio, map_zero, step_size, contrast, stat_tck);
    ...
    if (colour) {
      ...
      Thread::run_queue_threaded_pipe (loader, Tractography::Streamline<float>(), mapper, SetVoxelDECFactor(), writer);
    } else {
      ...
      Thread::run_queue_threaded_pipe (loader, Tractography::Streamline<float>(), mapper, SetVoxelFactor(), *writer);
    }
